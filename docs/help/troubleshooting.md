...
You can also set `SANI_VERBOSE=1` instead of the flag.
...
### `docs.sani.ai` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.sani.ai` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.sani.ai` to the allowlist, then retry.
...
- Check the allowlist: `sani config get agents.defaults.models`
...
If you can, include the relevant log tail from `sani logs --follow`.
