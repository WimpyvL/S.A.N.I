...
- Jobs persist under `~/.sani/cron/` so restarts don’t lose schedules.
...
sani cron list
sani cron run <job-id> --force
sani cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.sani/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `sani cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, SANI defaults to `announce`.
...
- Job store: `~/.sani/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.sani/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
sani cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
sani cron edit <jobId> --agent ops
sani cron edit <jobId> --clear-agent
```
...
```bash
sani system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`sani system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `SANI_SKIP_CRON`.
