...
2. Configure WhatsApp in `~/.sani/sani.json`.
3. Run `sani channels login` to scan the QR code (Linked Devices).
4. Start the gateway.
...
WhatsApp requires a real mobile number for verification. VoIP and virtual numbers are usually blocked. There are two supported ways to run SANI on WhatsApp:

### Dedicated number (recommended)

Use a **separate phone number** for SANI. Best UX, clean routing, no self-chat quirks. Ideal setup: **spare/old Android phone + eSIM**. Leave it on Wi‑Fi and power, and link it via QR.

**WhatsApp Business:** You can use WhatsApp Business on the same device with a different number. Great for keeping your personal WhatsApp separate — install WhatsApp Business and register the SANI number there.
...
Quick fallback: run SANI on **your own number**. Message yourself (WhatsApp “Message yourself”) for testing so you don’t spam contacts. Expect to read verification codes on your main phone during setup and experiments. **Must enable self-chat mode.**
...
Self-chat replies default to `[{identity.name}]` when set (otherwise `[sani]`)
...
- Early SANI builds supported Twilio’s WhatsApp Business integration.
...
- Login command: `sani channels login` (QR via Linked Devices).
- Multi-account login: `sani channels login --account <id>` (`<id>` = `accountId`).
- Default account (when `--account` is omitted): `default` if present, otherwise the first configured account id (sorted).
- Credentials stored in `~/.sani/credentials/whatsapp/<accountId>/creds.json`.
- Backup copy at `creds.json.bak` (restored on corruption).
- Legacy compatibility: older installs stored Baileys files directly in `~/.sani/credentials/`.
- Logout: `sani channels logout` (or `--account <id>`) deletes WhatsApp auth state (but keeps shared `oauth.json`).
...
- **DM policy**: `channels.whatsapp.dmPolicy` controls direct chat access (default: `pairing`).
  - Pairing: unknown senders get a pairing code (approve via `sani pairing approve whatsapp <code>`; codes expire after 1 hour).
...
If you run SANI on your **personal WhatsApp number**, enable `channels.whatsapp.selfChatMode` (see sample above).
...
**Will SANI message random contacts when I link WhatsApp?**  
No. Default DM policy is **pairing**, so unknown senders only get a pairing code and their message is **not processed**. SANI only replies to chats it receives, or to sends you explicitly trigger (agent/CLI).

**How does pairing work on WhatsApp?**  
Pairing is a DM gate for unknown senders:

- First DM from a new sender returns a short code (message is not processed).
- Approve with: `sani pairing approve whatsapp <code>` (list with `sani pairing list whatsapp`).
- Codes expire after 1 hour; pending requests are capped at 3 per channel.

**Can multiple people use different SANI instances on one WhatsApp number?**  
...
- Media:
  ...
  - Animated GIFs: WhatsApp expects MP4 with `gifPlayback: true` for inline looping.
    - CLI: `sani message send --media <mp4> --gif-playback`
...
- Best results: OGG/Opus. SANI rewrites `audio/ogg` to `audio/ogg; codecs=opus`.
...
- Log file: `/tmp/sani/sani-YYYY-MM-DD.log` (configurable).
...
- Fix: run `sani channels login` on the gateway host and scan the QR (WhatsApp → Settings → Linked Devices).

**Linked but disconnected / reconnect loop**

- Symptom: `channels status` shows `running, disconnected` or warns “Linked but disconnected”.
- Fix: `sani doctor` (or restart the gateway). If it persists, relink via `channels login` and inspect `sani logs --follow`.
