...
- Talk to SANI via Discord DMs or guild channels.
...
3. Configure SANI with `channels.discord.token` (or `DISCORD_BOT_TOKEN` as a fallback).
...
7. Direct chats: secure by default via `channels.discord.dm.policy` (default: `"pairing"`). Unknown senders get a pairing code (expires after 1 hour); approve via `sani pairing approve discord <code>`.
...
This is the “Discord Developer Portal” setup for running SANI in a server (guild) channel like `#help`.
...
### 2) Enable the gateway intents SANI needs
...
Discord uses numeric ids everywhere; SANI config prefers ids.
...
- First: run `sani doctor` and `sani channels status --probe` (actionable warnings + quick audits).
...
When enabled, SANI uses the member identity for allowlists and labels the
...
- On startup, SANI resolves channel/user names in allowlists to IDs (when the bot can search members)
  and logs the mapping; unresolved entries are kept as typed.

Native command notes:

- The registered commands mirror SANI’s chat commands.
...
- Slash commands may still be visible in Discord UI to users who aren’t allowlisted; SANI enforces allowlists on execution and replies “not authorized”.
...
- If the bot is stuck or rate limited, restart the gateway (`sani gateway --force`) after confirming no other processes own the Discord session.
