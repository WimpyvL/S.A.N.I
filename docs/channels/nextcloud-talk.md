...
```bash
sani plugins install @sani/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
sani plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "SANI" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `sani pairing list nextcloud-talk`
  - `sani pairing approve nextcloud-talk <CODE>`
