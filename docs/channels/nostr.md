...
read_when:
  - You want SANI to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables SANI to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`sani onboard`) and `sani channels add` list optional channel plugins.
...
```bash
sani plugins install @sani/nostr
```

Use a local checkout (dev workflows):

```bash
sani plugins install --link <path-to-sani>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "sani@example.com",
        "lud16": "sani@example.com"
      }
    }
  }
}
