...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects SANI to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
If you just installed SANI, run the wizard:
...
2. Configuring app credentials in SANI
...
1. You already ran `sani channels add` for Feishu
2. The gateway is running (`sani gateway status`)
...
```bash
sani pairing approve feishu <CODE>
```
...
2. Run `sani logs --follow` and look for `chat_id`
...
2. Run `sani logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `sani gateway status`  | Show gateway status           |
| `sani gateway install` | Install/start gateway service |
| `sani gateway stop`    | Stop gateway service          |
| `sani gateway restart` | Restart gateway service       |
| `sani logs --follow`   | Tail gateway logs             |
...
5. Ensure the gateway is running: `sani gateway status`
6. Check logs: `sani logs --follow`
