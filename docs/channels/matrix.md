...
Matrix is an open, decentralized messaging protocol. SANI connects as a Matrix **user**
...
1. Install the Matrix plugin:
   - From npm: `sani plugins install @sani/matrix`
   - From a local checkout: `sani plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: SANI calls the same
     login endpoint, stores the access token in `~/.sani/credentials/matrix/credentials.json`,
...
- On first connection, SANI requests device verification from your other sessions.
...
`~/.sani/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- On startup, SANI resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
