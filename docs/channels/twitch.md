...
read_when:
  - Setting up Twitch chat integration for SANI
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. SANI connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
sani plugins install @sani/twitch
```

Local checkout (when running from a git repo):

```bash
sani plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `SANI_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "sani", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use SANI_TWITCH_ACCESS_TOKEN env var)
...
```bash
SANI_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
