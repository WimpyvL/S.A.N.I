...
SANI can stream partial replies in Telegram DMs using `sendMessageDraft`.
...
- If Telegram rejects the HTML payload, SANI retries the same message as plain text.

## Commands (native + custom)

SANI registers native commands (like `/status`, `/reset`, `/model`) with Telegram’s bot menu on startup.
...
- Custom commands are **menu entries only**; SANI does not implement them unless you handle them elsewhere.
...
**Privacy note:** `@userinfobot` is a third-party bot. If you prefer, add the bot to the group, send a message, and use `sani logs --follow` to read `chat.id`, or use the Bot API `getUpdates`.
...
- A group is upgraded to a supergroup and Telegram emits `migrate_to_chat_id` (chat ID changes). SANI can migrate `channels.telegram.groups` automatically.
...
Telegram forum topics include a `message_thread_id` per message. SANI:
...
Private chats can include `message_thread_id` in some edge cases. SANI keeps the DM session key unchanged, but still uses the thread id for replies/draft streaming when it is present.
...
- Approve via:
  ...
  - `sani pairing approve telegram <CODE>`
...
2. Run `sani logs --follow` and look for `from.id`.
...
SANI defaults to audio files for backward compatibility.
...
SANI supports receiving and sending Telegram stickers with intelligent caching.

### Receiving stickers

When a user sends a sticker, SANI handles it based on the sticker type:
...
Stickers are processed through the AI's vision capabilities to generate descriptions. Since the same stickers are often sent repeatedly, SANI caches these descriptions to avoid redundant API calls.
...
**Cache location:** `~/.sani/telegram/sticker-cache.json`
...
SANI uses Bot API `sendMessageDraft` (not real messages) and then sends the
...
**How reactions work:**
Telegram reactions arrive as **separate `message_reaction` events**, not as properties in message payloads. When a user adds a reaction, SANI:
...
- Telegram bots must explicitly request `message_reaction` in `allowed_updates` (configured automatically by SANI)
...
- Example: `sani message send --channel telegram --target 123456789 --message "hi"`.
...
- `sani channels status` shows a warning when config expects unmentioned group messages.
- `sani channels status --probe` can additionally check membership for explicit numeric group IDs (it can’t audit wildcard `"*"` rules).
...
- Check gateway logs: `sani logs --follow` (look for "skipping group message")
...
- Upgrade to a SANI build that normalizes abort signals, or run the gateway on Node 20 until you can upgrade.
