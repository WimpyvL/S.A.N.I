...
Tlon is a decentralized messenger built on Urbit. SANI connects to your Urbit ship and can
...
```bash
sani plugins install @sani/tlon
```

Local checkout (when running from a git repo):

```bash
sani plugins install ./extensions/tlon
```
...
Use these with `sani message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, SANI replies in-thread.
