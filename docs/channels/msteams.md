...
```bash
sani plugins install @sani/msteams
```

Local checkout (when running from a git repo):

```bash
sani plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
SANI will offer the local install path automatically.
...
3. Configure SANI with those credentials.
...
- Talk to SANI via Teams DMs, group chats, or channels.
...
- On startup, SANI resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.sani/sani.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring SANI, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `sani-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `sani plugins install @sani/msteams`
   - From a local checkout: `sani plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "SANI in Teams", "full": "SANI in Teams" },
...
SANI handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, SANI only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/SANIShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

SANI sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `sani message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.sani/msteams-polls.json`.
...
```bash
sani message send --channel msteams \
...
sani message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
sani message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
sani message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
sani message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
