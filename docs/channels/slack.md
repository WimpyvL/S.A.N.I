...
3. Set tokens for SANI and start the gateway.
...
7. Slash Commands → create `/sani` if you use `channels.slack.slashCommand`. If you enable native commands, add one slash command per built-in command (same names as `/help`). Native defaults to off for Slack unless you set `channels.slack.commands.native: true` (global `commands.native` is `"auto"` which leaves Slack off).
...
SANI can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, SANI falls back to the user token.
...
```json
{
  "display_information": {
    "name": "SANI",
    "description": "Slack connector for SANI"
  },
  "features": {
    "bot_user": {
      "display_name": "SANI",
    ...
    "slash_commands": [
      {
        "command": "/sani",
        "description": "Send a message to SANI",
        "should_escape": false
      }
    ]
  },
...
By default, SANI replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, SANI infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `sani pairing approve slack <code>`.
...
- On startup, SANI resolves channel/user names in allowlists to IDs (when tokens allow)
