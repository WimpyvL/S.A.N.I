...
SANI 包含三个 Vitest 测试套件（单元/集成、端到端、实时）以及一小组 Docker 运行器。
...
- 文件：`src/**/*.live.test.ts`
- 默认：通过 `pnpm test:live` **启用**（设置 `SANI_LIVE_TEST=1`）
- 范围：
...
- 预期：
  ...
  - Anthropic 密钥轮换：设置 `SANI_LIVE_ANTHROPIC_KEYS="sk-...,sk-..."`（或 `SANI_LIVE_ANTHROPIC_KEY=sk-...`）或多个 `ANTHROPIC_API_KEY*` 变量；测试会在遇到速率限制时重试
...
- 如何启用：
  - `pnpm test:live`（或直接调用 Vitest 时使用 `SANI_LIVE_TEST=1`）
- 设置 `SANI_LIVE_MODELS=modern`（或 `all`，modern 的别名）以实际运行此套件；否则会跳过以保持 `pnpm test:live` 专注于 Gateway 网关冒烟测试
- 如何选择模型：
  - `SANI_LIVE_MODELS=modern` 运行现代允许列表（Opus/Sonnet/Haiku 4.5、GPT-5.x + Codex、Gemini 3、GLM 4.7、MiniMax M2.1、Grok 4）
  - `SANI_LIVE_MODELS=all` 是现代允许列表的别名
  - 或 `SANI_LIVE_MODELS="openai/gpt-5.2,anthropic/claude-opus-4-5,..."`（逗号分隔的允许列表）
- 如何选择提供商：
  - `SANI_LIVE_PROVIDERS="google,google-antigravity,google-gemini-cli"`（逗号分隔的允许列表）
- 密钥来源：
  - 默认：配置文件存储和环境变量回退
  - 设置 `SANI_LIVE_REQUIRE_PROFILE_KEYS=1` 以强制**仅使用配置文件存储**
- 为什么存在这个测试：
  - 将"提供商 API 损坏/密钥无效"与"Gateway 网关智能体管道损坏"分离
  - 包含小型、隔离的回归测试（例如：OpenAI Responses/Codex Responses 推理重放 + 工具调用流程）

### 第二层：Gateway 网关 + 开发智能体冒烟测试（"@sani"实际做的事）
...
- 如何选择模型：
  ...
  - `SANI_LIVE_GATEWAY_MODELS=all` 是现代允许列表的别名
  - 或设置 `SANI_LIVE_GATEWAY_MODELS="provider/model"`（或逗号分隔列表）来缩小范围
- 如何选择提供商（避免"OpenRouter 全部"）：
  - `SANI_LIVE_GATEWAY_PROVIDERS="google,google-antigravity,google-gemini-cli,openai,anthropic,zai,minimax"`（逗号分隔的允许列表）
- 工具 + 图像探测在此实时测试中始终开启：
...
- 令牌来源（选择一个）：
  - 配置文件：`SANI_LIVE_SETUP_TOKEN_PROFILE=anthropic:setup-token-test`
  - 原始令牌：`SANI_LIVE_SETUP_TOKEN_VALUE=sk-ant-oat01-...`
- 模型覆盖（可选）：
...
- 覆盖（可选）：
  ...
  - `SANI_LIVE_CLI_BACKEND_IMAGE_PROBE=1` 发送真实图像附件（路径注入到提示中）。
  - `SANI_LIVE_CLI_BACKEND_IMAGE_ARG="--image"` 将图像文件路径作为 CLI 参数传递而非提示注入。
  - `SANI_LIVE_CLI_BACKEND_IMAGE_MODE="repeat"`（或 `"list"`）控制设置 `IMAGE_ARG` 时如何传递图像参数。
  - `SANI_LIVE_CLI_BACKEND_RESUME_PROBE=1` 发送第二轮并验证恢复流程。
- `SANI_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG=0` 保持 Claude Code CLI MCP 配置启用（默认使用临时空文件禁用 MCP 配置）。
...
- Google 专项（Gemini API 密钥 + Antigravity）：
  - Gemini（API 密钥）：`SANI_LIVE_GATEWAY_MODELS="google/gemini-3-flash-preview" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
- Gemini API 与 Gemini CLI：
  - API：SANI 通过 HTTP 调用 Google 托管的 Gemini API（API 密钥/配置文件认证）；这是大多数用户说的"Gemini"。
  - CLI：SANI 调用本地 `gemini` 二进制文件；它有自己的认证，行为可能不同（流式传输/工具支持/版本差异）。
...
在 `SANI_LIVE_GATEWAY_MODELS` 中至少包含一个支持图像的模型（Claude/Gemini/OpenAI 视觉能力变体等）以测试图像探测。

### 聚合器/替代 Gateway 网关

如果你启用了密钥，我们也支持通过以下方式测试：

- OpenRouter：`openrouter/...`（数百个模型；使用 `sani models scan` 查找支持工具+图像的候选模型）
...
- 如果实时测试说"无凭证"，用调试 `sani models list`/模型选择相同的方式调试。

- 配置文件存储：`~/.sani/credentials/`（首选；测试中"配置文件密钥"的含义）
- 配置：`~/.sani/sani.json`（或 `SANI_CONFIG_PATH`）
...
- `SANI_CONFIG_DIR=...`（默认：`~/.sani`）挂载到 `/home/node/.sani`
- `SANI_WORKSPACE_DIR=...`（默认：`~/.sani/workspace`）挂载到 `/home/node/.sani/workspace`
- `SANI_PROFILE_FILE=...`（默认：`~/.profile`）挂载到 `/home/node/.profile` 并在运行测试前加载
- `SANI_LIVE_GATEWAY_MODELS=...` / `SANI_LIVE_MODELS=...` 用于缩小运行范围
- `SANI_LIVE_REQUIRE_PROFILE_KEYS=1` 确保凭证来自配置文件存储（而非环境变量）
