...
当你需要切换不常用的设置而不编辑 `sani.json` 时，这非常方便。
...
- **全局 `--dev`（配置文件）：** 将状态隔离到 `~/.sani-dev` 下，并将 Gateway 网关端口默认为 `19001`（派生端口随之移动）。
...
如果你还没有全局安装，请通过 `pnpm sani ...` 运行 CLI。
...
1. **配置文件隔离**（全局 `--dev`）
   ...
   - `SANI_GATEWAY_PORT=19001`（浏览器/画布相应移动）
...
2. **Dev 引导**（`gateway --dev`）
   ...
   - 默认身份：**C3‑PO**（礼仪机器人）。
   - 在 dev 模式下跳过渠道提供商（`SANI_SKIP_CHANNELS=1`）。
