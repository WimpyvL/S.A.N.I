...
所有日志配置都在 `~/.sani/sani.json` 的 `logging` 下。
...
- SANI 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `sani plugins enable diagnostics-otel` 启用插件。
...
- `sani.tokens`（计数器，属性：`sani.token`、`sani.channel`、`sani.provider`、`sani.model`）
- `sani.cost.usd`（计数器，属性：`sani.channel`、`sani.provider`、`sani.model`）
- `sani.run.duration_ms`（直方图，属性：`sani.channel`、`sani.provider`、`sani.model`）
- `sani.context.tokens`（直方图，属性：`sani.context`、`sani.channel`、`sani.provider`、`sani.model`）

消息流：

- `sani.webhook.received`（计数器，属性：`sani.channel`、`sani.webhook`）
- `sani.webhook.error`（计数器，属性：`sani.channel`、`sani.webhook`）
- `sani.webhook.duration_ms`（直方图，属性：`sani.channel`、`sani.webhook`）
- `sani.message.queued`（计数器，属性：`sani.channel`、`sani.source`）
- `sani.message.processed`（计数器，属性：`sani.channel`、`sani.outcome`）
- `sani.message.duration_ms`（直方图，属性：`sani.channel`、`sani.outcome`）

队列 + 会话：

- `sani.queue.lane.enqueue`（计数器，属性：`sani.lane`）
- `sani.queue.lane.dequeue`（计数器，属性：`sani.lane`）
- `sani.queue.depth`（直方图，属性：`sani.lane` 或 `sani.channel=heartbeat`）
- `sani.queue.wait_ms`（直方图，属性：`sani.lane`）
- `sani.session.state`（计数器，属性：`sani.state`、`sani.reason`）
- `sani.session.stuck`（计数器，属性：`sani.state`）
- `sani.session.stuck_age_ms`（直方图，属性：`sani.state`）
- `sani.run.attempt`（计数器，属性：`sani.attempt`）
...
- `sani.webhook.error`
  - `sani.channel`、`sani.webhook`、`sani.chatId`、`sani.error`
- `sani.message.processed`
  - `sani.channel`、`sani.outcome`、`sani.chatId`、`sani.messageId`、`sani.sessionKey`、`sani.sessionId`、`sani.reason`
- `sani.session.stuck`
  - `sani.state`、`sani.ageMs`、`sani.queueDepth`、`sani.sessionKey`、`sani.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `sani doctor`。
