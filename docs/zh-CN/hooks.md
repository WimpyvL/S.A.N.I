...
Hooks 提供了一个可扩展的事件驱动系统，用于响应智能体命令和事件自动执行操作。Hooks 从目录中自动发现，可以通过 CLI 命令管理，类似于 SANI 中 Skills 的工作方式。
...
- **Webhooks**：外部 HTTP webhooks，让其他系统触发 SANI 中的工作。参见 [Webhook Hooks](/automation/webhook) 或使用 `sani webhooks` 获取 Gmail 助手命令。
...
- 记录所有命令以供审计
- 在智能体生命周期事件上触发自定义自动化
- 在不修改核心代码的情况下扩展 SANI 的行为

## 入门

### 捆绑的 Hooks

SANI 附带四个自动发现的捆绑 hooks：

- **💾 session-memory**：当你发出 `/new` 时将会话上下文保存到智能体工作区（默认 `~/.sani/workspace/memory/`）
- **📝 command-logger**：将所有命令事件记录到 `~/.sani/logs/commands.log`
...
在新手引导期间（`sani onboard`），你将被提示启用推荐的 hooks。向导会自动发现符合条件的 hooks 并呈现供选择。
...
2. **托管 hooks**：`~/.sani/hooks/`（用户安装，跨工作区共享）
3. **捆绑 hooks**：`<sani>/dist/hooks/bundled/`（随 SANI 附带）
...
Hook 包是标准的 npm 包，通过 `package.json` 中的 `sani.hooks` 导出一个或多个 hooks。使用以下命令安装：
...
Hook 包可以附带依赖；它们将安装在 `~/.sani/hooks/<id>` 下。
...
这些 hooks 不是事件流监听器；它们让插件在 SANI 持久化工具结果之前同步调整它们。
...
**输出**：`<workspace>/memory/YYYY-MM-DD-slug.md`（默认为 `~/.sani/workspace`）
...
- [捆绑 Hooks README](https://github.com/sani/sani/tree/main/src/hooks/bundled)
