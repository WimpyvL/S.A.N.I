...
- **会话存储**（聊天历史 + 路由状态）位于 `~/.sani/agents/<agentId>/sessions` 下。
...
Skills 通过每个工作区的 `skills/` 文件夹实现每智能体独立，共享的 Skills 可从 `~/.sani/skills` 获取。参见 [Skills：每智能体 vs 共享](/tools/skills#per-agent-vs-shared-skills)。
...
- 状态目录：`~/.sani`（或 `SANI_STATE_DIR`）
- 工作区：`~/.sani/workspace`（或 `~/.sani/workspace-<agentId>`）
- 智能体目录：`~/.sani/agents/<agentId>/agent`（或 `agents.list[].agentDir`）
- 会话：`~/.sani/agents/<agentId>/sessions`

### 单智能体模式（默认）

如果你什么都不做，SANI 运行单个智能体：

- `agentId` 默认为 **`main`**。
- 会话键为 `agent:main:<mainKey>`。
- 工作区默认为 `~/.sani/workspace`（或当设置了 `SANI_PROFILE` 时为 `~/.sani/workspace-<profile>`）。
- 状态默认为 `~/.sani/agents/main/agent`。
...
```js
{
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // 可选覆盖。默认：~/.sani/credentials/whatsapp/personal
