...
当启用沙箱隔离且 `workspaceAccess` 不是 `"rw"` 时，工具在 `~/.sani/sandboxes` 下的沙箱工作区内操作，而不是你的主机工作区。

## 默认位置

- 默认：`~/.sani/workspace`
- 如果设置了 `SANI_PROFILE` 且不是 `"default"`，默认值变为
  `~/.sani/workspace-<profile>`。
- 在 `~/.sani/sani.json` 中覆盖：
...
`sani onboard`、`sani configure` 或 `sani setup` 将创建工作区并在缺失时填充引导文件。
...
旧版安装可能创建了 `~/sani`。保留多个工作区目录可能会导致混乱的认证或状态漂移，因为同一时间只有一个工作区是活动的。

**建议：** 保持单个活动工作区。如果你不再使用额外的文件夹，请归档或移至废纸篓（例如 `trash ~/sani`）。
如果你有意保留多个工作区，请确保 `agents.defaults.workspace` 指向活动的那个。

`sani doctor` 在检测到额外工作区目录时会发出警告。

## 工作区文件映射（每个文件的含义）

这些是 SANI 在工作区内期望的标准文件：
...
如果任何引导文件缺失，SANI 会在会话中注入"缺失文件"标记并继续。大型引导文件在注入时会被截断；使用 `agents.defaults.bootstrapMaxChars` 调整限制（默认：20000）。
`sani setup` 可以重新创建缺失的默认值而不覆盖现有文件。

## 工作区中不包含的内容

这些位于 `~/.sani/` 下，不应提交到工作区仓库：

- `~/.sani/sani.json`（配置）
- `~/.sani/credentials/`（OAuth token、API 密钥）
- `~/.sani/agents/<agentId>/sessions/`（会话记录 + 元数据）
- `~/.sani/skills/`（托管的 Skills）
...
如果你必须存储敏感引用，请使用占位符并将真正的密钥保存在其他地方（密码管理器、环境变量或 `~/.sani/`）。
...
1. 将仓库克隆到所需路径（默认 `~/.sani/workspace`）。
2. 在 `~/.sani/sani.json` 中将 `agents.defaults.workspace` 设置为该路径。
3. 运行 `sani setup --workspace <path>` 来填充任何缺失的文件。
4. 如果你需要会话，请单独从旧机器复制 `~/.sani/agents/<agentId>/sessions/`。
