...
read_when:
  - 添加或修改配置字段时
summary: ~/.sani/sani.json 的所有配置选项及示例
title: 配置
x-i18n:
...
SANI 从 `~/.sani/sani.json` 读取可选的 **JSON5** 配置（支持注释和尾逗号）。

如果文件不存在，SANI 使用安全的默认值（内置 Pi 智能体 + 按发送者分会话 + 工作区 `~/.sani/workspace`）。通常只在以下情况需要配置：
...
SANI 只接受完全匹配 schema 的配置。
未知键、类型错误或无效值会导致 Gateway 网关 **拒绝启动**以确保安全。

验证失败时：

- Gateway 网关不会启动。
- 只允许诊断命令（例如：`sani doctor`、`sani logs`、`sani health`、`sani status`、`sani service`、`sani help`）。
- 运行 `sani doctor` 查看具体问题。
- 运行 `sani doctor --fix`（或 `--yes`）应用迁移/修复。
...
请使用 `config.patch` 或 `sani config set`。请备份 `~/.sani/sani.json`。
...
{ "$include": "/etc/sani/base.json5" } // 绝对路径
...
SANI 从父进程（shell、launchd/systemd、CI 等）读取环境变量。

此外，它还会加载：

- 当前工作目录中的 `.env`（如果存在）
- `~/.sani/.env`（即 `$SANI_STATE_DIR/.env`）作为全局回退 `.env`
...
可选便利功能：如果启用且预期键均未设置，SANI 会运行你的登录 shell 并仅导入缺失的预期键（不会覆盖）。
...
SANI 在以下位置存储**每个智能体的**认证配置文件（OAuth + API 密钥）：

- `<agentDir>/auth-profiles.json`（默认：`~/.sani/agents/<agentId>/agent/auth-profiles.json`）

另请参阅：[/concepts/oauth](/concepts/oauth)

旧版 OAuth 导入：

- `~/.sani/credentials/oauth.json`（或 `$SANI_STATE_DIR/credentials/oauth.json`）
...
- `~/.sani/agent/*`（由 `sani doctor` 迁移到 `~/.sani/agents/<defaultAgentId>/agent/*`）

覆盖：

- OAuth 目录（仅旧版导入）：`SANI_OAUTH_DIR`
- 智能体目录（默认智能体根目录覆盖）：`SANI_AGENT_DIR`（推荐）、`PI_CODING_AGENT_DIR`（旧版）

首次使用时，SANI 会将 `oauth.json` 条目导入到 `auth-profiles.json` 中。
...
- 默认日志文件：`/tmp/sani/sani-YYYY-MM-DD.log`
- 如需稳定路径，将 `logging.file` 设为 `/tmp/sani/sani.log`。
- 控制台输出可通过以下方式单独调整：
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // 可选覆盖。默认：~/.sani/credentials/whatsapp/biz
...
- 旧版单账号 Baileys 认证目录由 `sani doctor` 迁移到 `whatsapp/default`。
...
- `agents.list[]`：每智能体覆盖。
  ...
  - `name`：智能体的显示名称。
  - `workspace`：默认 `~/.sani/workspace-<agentId>`（对于 `main`，回退到 `agents.defaults.workspace`）。
  - `agentDir`：默认 `~/.sani/agents/<agentId>/agent`。
...
- `commands.config: true` 启用 `/config`（读写 `sani.json`）。
...
SANI 仅在存在 `channels.telegram` 配置段时启动 Telegram。机器人 token 从 `channels.telegram.botToken`（或 `channels.telegram.tokenFile`）解析，`TELEGRAM_BOT_TOKEN` 作为默认账号的回退。
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["sani-dm"], // 可选群组私聊白名单
      },
      guilds: {
        "123456789012345678": {
          // 服务器 id（推荐）或 slug
...
SANI 仅在存在 `channels.discord` 配置段时启动 Discord。token 从 `channels.discord.token` 解析，`DISCORD_BOT_TOKEN` 作为默认账号的回退（除非 `channels.discord.enabled` 为 `false`）。在为 cron/CLI 命令指定投递目标时，使用 `user:<id>`（私聊）或 `channel:<id>`（服务器频道）；裸数字 ID 有歧义会被拒绝。
...
SANI 在提供商启用且两个 token 都已设置时启动 Slack（通过配置或 `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`）。在为 cron/CLI 命令指定投递目标时使用 `user:<id>`（私聊）或 `channel:<id>`。
...
请先安装：`sani plugins install @sani/mattermost`（或从 git checkout 使用 `./extensions/mattermost`）。
...
SANI 在账号已配置（机器人 token + 基础 URL）且已启用时启动 Mattermost。token + 基础 URL 从 `channels.mattermost.botToken` + `channels.mattermost.baseUrl` 或默认账号的 `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` 解析（除非 `channels.mattermost.enabled` 为 `false`）。
...
SANI 会生成 `imsg rpc`（通过 stdio 的 JSON-RPC）。无需守护进程或端口。
...
在系统提示的 Runtime 行中显示的可选仓库根目录。如果未设置，SANI 会从工作区（和当前工作目录）向上查找 `.git` 目录进行检测。路径必须存在才能使用。
...
当文件超过此限制时，SANI 会记录警告并注入带标记的头尾截断内容。
...
设置用户时区用于**系统提示上下文**（不用于消息信封中的时间戳）。如果未设置，SANI 在运行时使用主机时区。
...
如果未设置 `messages.responsePrefix`，默认不应用前缀。WhatsApp 自聊天回复是例外：它们在设置时默认为 `[{identity.name}]`，否则为 `[sani]`，以保持同一手机上的对话可读性。
...
WhatsApp 入站前缀通过 `channels.whatsapp.messagePrefix` 配置（已弃用：`messages.messagePrefix`）。默认保持**不变**：当 `channels.whatsapp.allowFrom` 为空时为 `"[sani]"`，否则为 `""`（无前缀）。使用 `"[sani]"` 时，如果路由的智能体设置了 `identity.name`，SANI 会改用 `[{identity.name}]`。
...
为出站回复启用文字转语音。开启后，SANI 使用 ElevenLabs 或 OpenAI 生成音频并附加到回复中。Telegram 使用 Opus 语音消息；其他渠道发送 MP3 音频。
...
SANI 还内置了一些别名快捷方式。默认值仅在模型已存在于 `agents.defaults.models` 中时才应用：
...
如果省略提供商，SANI 目前假定 `anthropic` 作为临时弃用回退。
...
- `tools.web.search.apiKey`（推荐：通过 `sani configure --section web` 设置，或使用 `BRAVE_API_KEY` 环境变量）
...
- `group:nodes`：`nodes`
- `group:sani`：所有内置 SANI 工具（不包含提供商插件）
...
- 智能体工作区访问：`workspaceAccess: "none"`（默认）
  - `"none"`：在 `~/.sani/sandboxes` 下使用每范围的沙箱工作区
...
SANI 使用 **pi-coding-agent** 模型目录。你可以通过编写
`~/.sani/agents/<agentId>/agent/models.json` 或在 SANI 配置中的 `models.providers` 下定义相同的 schema 来添加自定义提供商（LiteLLM、本地 OpenAI 兼容服务器、Anthropic 代理等）。
按提供商的概述 + 示例：[/concepts/model-providers](/concepts/model-providers)。

当存在 `models.providers` 时，SANI 在启动时将 `models.json` 写入/合并到
`~/.sani/agents/<agentId>/agent/`：
...
OpenCode Zen 是一个具有每模型端点的多模型网关。SANI 使用
...
- 快捷方式：`sani onboard --auth-choice opencode-zen`。
...
快捷方式：`sani onboard --auth-choice zai-api-key`。
...
- 在环境中设置 `MOONSHOT_API_KEY` 或使用 `sani onboard --auth-choice moonshot-api-key`。
...
- 在环境中设置 `KIMI_API_KEY` 或使用 `sani onboard --auth-choice kimi-code-api-key`。
...
- 设置 `SYNTHETIC_API_KEY` 或使用 `sani onboard --auth-choice synthetic-api-key`。
...
- 设置 `MINIMAX_API_KEY` 环境变量或使用 `sani onboard --auth-choice minimax-api`。
...
- 如果你希望 `models.json` 存储在其他位置，请使用 `SANI_AGENT_DIR`（或 `PI_CODING_AGENT_DIR`）覆盖智能体配置根目录（默认：`~/.sani/agents/main/agent`）。
...
```json5
{
  session: {
    ...
    // 默认已按智能体存储在 ~/.sani/agents/<agentId>/sessions/sessions.json
...
- `resetByType`：`dm`、`group` 和 `thread` 的每会话覆盖。
  - 如果你只设置了旧版 `session.idleMinutes` 而没有任何 `reset`/`resetByType`，SANI 保持仅空闲模式以向后兼容。
...
控制内置白名单、安装偏好、额外 Skills 文件夹和每 Skills 覆盖。适用于**内置**Skills 和 `~/.sani/skills`（工作区 Skills 在名称冲突时仍然优先）。
...
控制插件发现、允许/拒绝和每插件配置。插件从 `~/.sani/extensions`、`<workspace>/.sani/extensions` 以及任何 `plugins.load.paths` 条目加载。**配置更改需要重启 Gateway 网关。**
...
SANI 可以为 SANI 启动一个**专用、隔离的** Chrome/Brave/Edge/Chromium 实例并暴露一个小型 local loopback 控制服务。
...
- 配置文件颜色：`#FF4500`（龙虾橙）
- 注意：控制服务器由运行中的 Gateway 网关（SANI.app 菜单栏或 `sani gateway`）启动。
...
- 当连接来自这些 IP 之一时，SANI 使用 `x-forwarded-for`（或 `x-real-ip`）来确定客户端 IP，用于本地配对检查和 HTTP 认证/本地检查。
- 仅列出你完全控制的代理，并确保它们**覆盖**传入的 `x-forwarded-for`。

说明：

- `sani gateway` 拒绝启动，除非 `gateway.mode` 设为 `local`（或你传递了覆盖标志）。
...
- 优先级：`--port` > `SANI_GATEWAY_PORT` > `gateway.port` > 默认 `18789`。
...
- SANI.app 监视 `~/.sani/sani.json`，当 `gateway.mode` 或 `gateway.remote.url` 变更时实时切换模式。
...
Gateway 网关监视 `~/.sani/sani.json`（或 `SANI_CONFIG_PATH`）并自动应用更改。
...
- `~/.sani/sani.json`（或 `SANI_CONFIG_PATH`）
...
- `SANI_STATE_DIR`（会话/凭据）
- `agents.defaults.workspace`（记忆）
- `gateway.port`（每实例唯一）

便利标志（CLI）：

- `sani --dev …` → 使用 `~/.sani-dev` + 端口从基础 `19001` 偏移
- `sani --profile <name> …` → 使用 `~/.sani-<name>`（端口通过配置/环境变量/标志）
...
Gmail 辅助配置（由 `sani webhooks gmail setup` / `run` 使用）：
...
默认根目录：`~/.sani/workspace/canvas`
默认端口：`18793`（选择此端口以避免 SANI 浏览器 CDP 端口 `18792`）
...
- 监视目录并通过 `/__sani__/ws` 的 WebSocket 端点广播重载
- 目录为空时自动创建起始 `index.html`（以便你立即看到内容）
- 同时在 `/__sani__/a2ui/` 提供 A2UI，并作为 `canvasHostUrl` 通告给节点
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // 省略时使用 ~/.sani/bridge/tls/bridge-{cert,key}.pem。
...
控制 LAN mDNS 发现广播（`_sani-gw._tcp`）。

- `minimal`（默认）：从 TXT 记录中省略 `cliPath` + `sshPort`
- `full`：在 TXT 记录中包含 `cliPath` + `sshPort`
- `off`：完全禁用 mDNS 广播
- 主机名：默认为 `sani`（通告 `sani.local`）。通过 `SANI_MDNS_HOSTNAME` 覆盖。
...
启用后，Gateway 网关在 `~/.sani/dns/` 下使用配置的发现域（示例：`sani.internal.`）为 `_sani-gw._tcp` 写入单播 DNS-SD 区域。
