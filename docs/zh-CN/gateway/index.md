...
- 替代旧版 `gateway` 命令。CLI 入口点：`sani gateway`。
...
- 配置热重载监视 `~/.sani/sani.json`（或 `SANI_CONFIG_PATH`）。
...
- 默认在 `canvasHost.port`（默认 `18793`）上启动 Canvas 文件服务器，从 `~/.sani/workspace/canvas` 提供 `http://<gateway-host>:18793/__sani__/canvas/`。使用 `canvasHost.enabled=false` 或 `SANI_SKIP_CANVAS_HOST=1` 禁用。
...
- 默认需要 Gateway 网关认证：设置 `gateway.auth.token`（或 `SANI_GATEWAY_TOKEN`）或 `gateway.auth.password`。客户端必须发送 `connect.params.auth.token/password`，除非使用 Tailscale Serve 身份。
- 向导现在默认生成令牌，即使在 loopback 上也是如此。
- 端口优先级：`--port` > `SANI_GATEWAY_PORT` > `gateway.port` > 默认 `18789`。
...
- macOS：`bot.molt.<profile>`（旧版 `com.sani.*` 可能仍然存在）
...
- `SANI_GATEWAY_PORT=19001`（Gateway 网关 WS + HTTP）
- 浏览器控制服务端口 = `19003`（派生：`gateway.port+2`，仅 loopback）
- `canvasHost.port=19005`（派生：`gateway.port+4`）
- 当你在 `--dev` 下运行 `setup`/`onboard` 时，`agents.defaults.workspace` 默认变为 `~/.sani/workspace-dev`。

派生端口（经验法则）：

- 基础端口 = `gateway.port`（或 `SANI_GATEWAY_PORT` / `--port`）
- 浏览器控制服务端口 = 基础 + 2（仅 loopback）
- `canvasHost.port = 基础 + 4`（或 `SANI_CANVAS_HOST_PORT` / 配置覆盖）
- 浏览器配置文件 CDP 端口从 `browser.controlPort + 9 .. + 108` 自动分配（按配置文件持久化）。

每个实例的检查清单：

- 唯一的 `gateway.port`
- 唯一的 `SANI_CONFIG_PATH`
- 唯一的 `SANI_STATE_DIR`
...
- `health` — 完整健康快照（与 `sani health --json` 形状相同）。
- `status` — 简短摘要。
...
- LaunchAgents 是按用户的，需要已登录的会话；对于无头设置，使用自定义 LaunchDaemon（未随附）。
  - `sani gateway install` 写入 `~/Library/LaunchAgents/bot.molt.gateway.plist`
    （或 `bot.molt.<profile>.plist`；旧版 `com.sani.*` 会被清理）。
  - `sani doctor` 审计 LaunchAgent 配置，可以将其更新为当前默认值。
...
- 如果检测到其他类似 Gateway 网关的服务，CLI 会发出警告，除非它们是 SANI 配置文件服务。
  我们仍然建议大多数设置**每台机器一个 Gateway 网关**；使用隔离的配置文件/端口进行冗余或救援机器人。参见[多个 Gateway 网关](/gateway/multiple-gateways)。
  - 清理：`sani gateway uninstall`（当前服务）和 `sani doctor`（旧版迁移）。
- `gateway install` 在已安装时是无操作的；使用 `sani gateway install --force` 重新安装（配置文件/env/路径更改）。

捆绑的 mac 应用：

- SANI.app 可以捆绑基于 Node 的 Gateway 网关中继并安装标记为
  `bot.molt.gateway`（或 `bot.molt.<profile>`；旧版 `com.sani.*` 标签仍能干净卸载）的按用户 LaunchAgent。
- 要干净地停止它，使用 `sani gateway stop`（或 `launchctl bootout gui/$UID/bot.molt.gateway`）。
- 要重启，使用 `sani gateway restart`（或 `launchctl kickstart -k gui/$UID/bot.molt.gateway`）。
  - `launchctl` 仅在 LaunchAgent 已安装时有效；否则先使用 `sani gateway install`。
  - 运行命名配置文件时，将标签替换为 `bot.molt.<profile>`。

## 监管（systemd 用户单元）

SANI 在 Linux/WSL2 上默认安装 **systemd 用户服务**。我们
建议单用户机器使用用户服务（更简单的 env，按用户配置）。
对于多用户或常驻服务器使用**系统服务**（无需 lingering，
共享监管）。

`sani gateway install` 写入用户单元。`sani doctor` 审计
单元并可以将其更新以匹配当前推荐的默认值。

创建 `~/.config/systemd/user/sani-gateway[-<profile>].service`：
...
创建 `/etc/systemd/system/sani-gateway[-<profile>].service`（复制上面的单元，
...
- `sani gateway health|status` — 通过 Gateway 网关 WS 请求 health/status。
- `sani message send --target <num> --message "hi" [--media ...]` — 通过 Gateway 网关发送（对 WhatsApp 是幂等的）。
- `sani agent --message "hi" --to <num>` — 运行智能体轮次（默认等待最终结果）。
- `sani gateway call <method> --params '{"k":"v"}'` — 用于调试的原始方法调用器。
- `sani gateway stop|restart` — 停止/重启受监管的 Gateway 网关服务（launchd/systemd）。
- Gateway 网关辅助子命令假设 `--url` 上有运行中的 Gateway 网关；它们不再自动生成一个。

## 迁移指南

- 淘汰 `sani gateway` 和旧版 TCP 控制端口的使用。
