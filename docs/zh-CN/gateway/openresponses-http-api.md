...
SANI 的 Gateway 网关可以提供兼容 OpenResponses 的 `POST /v1/responses` 端点。
...
底层实现中，请求作为正常的 Gateway 网关智能体运行执行（与 `sani agent` 相同的代码路径），因此路由/权限/配置与你的 Gateway 网关一致。
...
- 当 `gateway.auth.mode="token"` 时，使用 `gateway.auth.token`（或 `SANI_GATEWAY_TOKEN`）。
- 当 `gateway.auth.mode="password"` 时，使用 `gateway.auth.password`（或 `SANI_GATEWAY_PASSWORD`）。

## 选择智能体

无需自定义头：在 OpenResponses `model` 字段中编码智能体 id：

- `model: "sani:<agentId>"`（示例：`"sani:main"`、`"sani:beta"`）
- `model: "agent:<agentId>"`（别名）

或通过头指定特定的 SANI 智能体：

- `x-sani-agent-id: <agentId>`（默认：`main`）

高级：

- `x-sani-session-key: <sessionKey>` 完全控制会话路由。
