...
summary: 在同一主机上运行多个 SANI Gateway 网关（隔离、端口和配置文件）
title: 多 Gateway 网关
x-i18n:
...
- `SANI_CONFIG_PATH` — 每个实例的配置文件
- `SANI_STATE_DIR` — 每个实例的会话、凭证、缓存
...
配置文件自动限定 `SANI_STATE_DIR` + `SANI_CONFIG_PATH` 范围并为服务名称添加后缀。
