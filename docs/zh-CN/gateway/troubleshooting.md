...
| 命令                               | 它告诉你什么                                                                          | 何时使用                              |
| ---------------------------------- | ------------------------------------------------------------------------------------- | ------------------------------------- |
| `sani status`                  | 本地摘要：操作系统 + 更新、Gateway 网关可达性/模式、服务、智能体/会话、提供商配置状态 | 首次检查，快速概览                    |
| `sani status --all`            | 完整本地诊断（只读、可粘贴、相对安全）包括日志尾部                                    | 当你需要分享调试报告时                |
| `sani status --deep`           | 运行 Gateway 网关健康检查（包括提供商探测；需要可达的 Gateway 网关）                  | 当"已配置"不意味着"正常工作"时        |
| `sani gateway probe`           | Gateway 网关发现 + 可达性（本地 + 远程目标）                                          | 当你怀疑正在探测错误的 Gateway 网关时 |
| `sani channels status --probe` | 向运行中的 Gateway 网关查询渠道状态（并可选探测）                                     | 当 Gateway 网关可达但渠道异常时       |
| `sani gateway status`          | 监管程序状态（launchd/systemd/schtasks）、运行时 PID/退出、最后的 Gateway 网关错误    | 当服务"看起来已加载"但没有运行时      |
| `sani logs --follow`           | 实时日志（运行时问题的最佳信号）                                                      | 当你需要实际的故障原因时              |

**分享输出：** 优先使用 `sani status --all`（它会隐藏令牌）。如果你粘贴 `sani status`，考虑先设置 `SANI_SHOW_SECRETS=0`（令牌预览）。
...
- 优先：`sani logs --follow`
- 文件日志（始终）：`/tmp/sani/sani-YYYY-MM-DD.log`（或你配置的 `logging.file`）
- macOS LaunchAgent（如果已安装）：`$SANI_STATE_DIR/logs/gateway.log` 和 `gateway.err.log`
- Linux systemd（如果已安装）：`journalctl --user -u sani-gateway[-<profile>].service -n 200 --no-pager`
...
**还没有配置文件？** 运行 `sani setup` 创建初始配置，然后重新运行
Gateway 网关。
...
变量如 `DISPLAY` 应该放在 `~/.sani/.env` 中（由 Gateway 网关早期加载）。
...
服务是用 Bun 或版本管理的 Node 路径安装的，运行 `sani doctor`
迁移到系统 Node 安装。
...
- 或将密钥烘焙到你的自定义沙箱镜像中
- 然后运行 `sani sandbox recreate --agent <id>`（或 `--all`）
...
- `gateway.mode` 必须为 `local` 才能运行 `sani gateway` 和服务。
- 如果你设置了 `gateway.mode=remote`，**CLI 默认**使用远程 URL。服务可能仍在本地运行，但你的 CLI 可能在探测错误的位置。使用 `sani gateway status` 查看服务解析的端口 + 探测目标（或传递 `--url`）。
- `sani gateway status` 和 `sani doctor` 在服务看起来正在运行但端口关闭时会显示日志中的**最后 Gateway 网关错误**。
- 非本地回环绑定（`lan`/`tailnet`/`custom`，或本地回环不可用时的 `auto`）需要认证：
  `gateway.auth.token`（或 `SANI_GATEWAY_TOKEN`）。
- `gateway.remote.token` 仅用于远程 CLI 调用；它**不**启用本地认证。
- `gateway.token` 被忽略；使用 `gateway.auth.token`。

**如果 `sani gateway status` 显示配置不匹配**

- `Config (cli): ...` 和 `Config (service): ...` 通常应该匹配。
- 如果不匹配，你几乎肯定是在编辑一个配置而服务运行的是另一个。
- 修复：从你希望服务使用的相同 `--profile` / `SANI_STATE_DIR` 重新运行 `sani gateway install --force`。

**如果 `sani gateway status` 报告服务配置问题**

- 监管程序配置（launchd/systemd/schtasks）缺少当前默认值。
- 修复：运行 `sani doctor` 更新它（或 `sani gateway install --force` 完全重写）。
...
- 修复：设置 `gateway.auth.mode` + `gateway.auth.token`（或导出 `SANI_GATEWAY_TOKEN`）并重启服务。

**如果 `sani gateway status` 显示 `bind=tailnet` 但未找到 tailnet 接口**
...
症状：`pwd` 或文件工具显示 `~/.sani/sandboxes/...` 即使你
期望的是主机工作区。
...
SANI 有意拒绝**较旧/不安全的模型**（尤其是那些更容易受到提示词注入攻击的模型）。如果你看到此错误，该模型名称已不再支持。

**修复：**

- 为提供商选择**最新**模型并更新你的配置或模型别名。
- 如果你不确定哪些模型可用，运行 `sani models list` 或
  `sani models scan` 并选择一个支持的模型。
...
trash "${SANI_STATE_DIR:-$HOME/.sani}/credentials" # 如果 logout 无法完全清除所有内容
sani channels login --verbose       # 重新扫描二维码
```
...
当配置包含未知键、格式错误的值或无效类型时，SANI 现在拒绝启动。
这是为了安全而故意设计的。
...
- `sani doctor` 报告每个无效条目。
- `sani doctor --fix` 应用迁移/修复并重写配置。
- 诊断命令如 `sani logs`、`sani health`、`sani status`、`sani gateway status` 和 `sani gateway probe` 即使配置无效也能运行。

### "All models failed" — 我应该首先检查什么？

- **凭证**存在于正在尝试的提供商（认证配置文件 + 环境变量）。
- **模型路由**：确认 `agents.defaults.model.primary` 和回退是你可以访问的模型。
- `/tmp/sani/…` 中的 **Gateway 网关日志**以获取确切的提供商错误。
- **模型状态**：使用 `/model status`（聊天）或 `sani models status`（CLI）。
...
5. 运行 `sani channels status --probe` 获取审计提示。
...
端点接受 JSON Schema 的严格子集。SANI 在当前 `main` 中清理/规范化工具
...
如果重置不起作用，在 [`scripts/package-mac-app.sh`](https://github.com/sani/sani/blob/main/scripts/package-mac-app.sh) 中更改 `BUNDLE_ID`（例如，添加 `.test` 后缀）并重新构建。这会强制 macOS 将其视为新应用。
...
sani gateway status
sani gateway stop
# 或：launchctl bootout gui/$UID/bot.molt.gateway（用 bot.molt.<profile> 替换；旧版 com.sani.* 仍然有效）
```
...
| 日志                             | 位置                                                                                                                                                                                                                                                                                                                      |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gateway 网关文件日志（结构化）   | `/tmp/sani/sani-YYYY-MM-DD.log`（或 `logging.file`）                                                                                                                                                                                                                                                              |
| Gateway 网关服务日志（监管程序） | macOS：`$SANI_STATE_DIR/logs/gateway.log` + `gateway.err.log`（默认：`~/.sani/logs/...`；配置文件使用 `~/.sani-<profile>/logs/...`）<br />Linux：`journalctl --user -u sani-gateway[-<profile>].service -n 200 --no-pager`<br />Windows：`schtasks /Query /TN "SANI Gateway (<profile>)" /V /FO LIST` |
| 会话文件                         | `$SANI_STATE_DIR/agents/<agentId>/sessions/`                                                                                                                                                                                                                                                                          |
| 媒体缓存                         | `$SANI_STATE_DIR/media/`                                                                                                                                                                                                                                                                                              |
| 凭证                             | `$SANI_STATE_DIR/credentials/`                                                                                                                                                                                                                                                                                        |
...
sani channels login         # 重新配对 WhatsApp
sani gateway restart           # 或：sani gateway
```
...
1. 首先检查日志：`/tmp/sani/`（默认：`sani-YYYY-MM-DD.log`，或你配置的 `logging.file`）
2. 在 GitHub 上搜索现有问题
3. 提交新问题时包含：
   - SANI 版本
   - 相关日志片段
