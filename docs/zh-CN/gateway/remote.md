...
- 使用 macOS 应用的 **Remote over SSH** 模式（设置 → 通用 → "SANI runs"）。
...
- `sani health` 和 `sani status --deep` 现在通过 `ws://127.0.0.1:18789` 访问远程 Gateway 网关。
- `sani gateway {status,health,send,agent,call}` 在需要时也可以通过 `--url` 指定转发的 URL。

注意：将 `18789` 替换为你配置的 `gateway.port`（或 `--port`/`SANI_GATEWAY_PORT`）。
