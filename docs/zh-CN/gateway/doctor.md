...
`sani doctor` 是 SANI 的修复 + 迁移工具。它修复过时的配置/状态，检查健康状况，并提供可操作的修复步骤。
...
当配置包含已弃用的键时，其他命令会拒绝运行并要求你运行 `sani doctor`。

Doctor 将：

- 解释找到了哪些遗留键。
- 显示它应用的迁移。
- 使用更新后的 schema 重写 `~/.sani/sani.json`。
...
- 会话存储 + 记录：
  - 从 `~/.sani/sessions/` 到 `~/.sani/agents/<agentId>/sessions/`
- 智能体目录：
  - 从 `~/.sani/agent/` 到 `~/.sani/agents/<agentId>/agent/`
- WhatsApp 认证状态（Baileys）：
  - 从遗留的 `~/.sani/credentials/*.json`（除 `oauth.json` 外）
  - 到 `~/.sani/credentials/whatsapp/<accountId>/...`（默认账户 id：`default`）

这些迁移是尽力而为且幂等的；当 doctor 将任何遗留文件夹作为备份保留时会发出警告。Gateway 网关/CLI 也会在启动时自动迁移遗留会话 + 智能体目录，因此历史/认证/模型会落在每智能体路径中，无需手动运行 doctor。WhatsApp 认证有意仅通过 `sani doctor` 迁移。
...
- **多个状态目录**：当多个 `~/.sani` 文件夹存在于不同 home 目录或当 `SANI_STATE_DIR` 指向别处时发出警告（历史可能在安装之间分裂）。
- **远程模式提醒**：如果 `gateway.mode=remote`，doctor 会提醒你在远程主机上运行它（状态在那里）。
- **配置文件权限**：当 `~/.sani/sani.json` 对组/其他用户可读时发出警告，并提供收紧到 `600` 的选项。
...
Doctor 检测遗留的 Gateway 网关服务（launchd/systemd/schtasks），并提供删除它们并使用当前 Gateway 网关端口安装 SANI 服务的选项。它还可以扫描额外的类 Gateway 网关服务并打印清理提示。
配置文件命名的 SANI Gateway 网关服务被视为一等公民，不会被标记为"额外的"。
...
当本地 Gateway 网关缺少 `gateway.auth` 时，Doctor 会发出警告并提供生成 token 的选项。使用 `sani doctor --generate-gateway-token` 在自动化中强制创建 token。
...
- `sani doctor` 在重写 supervisor 配置前提示。
- `sani doctor --yes` 接受默认修复提示。
- `sani doctor --repair` 无需提示应用推荐的修复。
- `sani doctor --repair --force` 覆盖自定义 supervisor 配置。
- 你始终可以通过 `sani gateway install --force` 强制完全重写。
