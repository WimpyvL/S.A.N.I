...
来自社区的真实项目。看看大家正在用 SANI 构建什么。

<Info>
**想要展示你的项目？** 在 [Discord 的 #showcase 频道](https://discord.gg/clawd) 分享或在 [X 上 @sani](https://x.com/sani)。
</Info>
...
OpenCode 完成更改 → 打开 PR → SANI 审查差异并在 Telegram 中回复"小建议"加上明确的合并决定（包括需要先应用的关键修复）。
...
向"Robby"（@sani）请求一个本地酒窖 skill。它请求一个示例 CSV 导出 + 存储位置，然后快速构建/测试该 skill（示例中有 962 瓶酒）。
...
跨 Agents、Claude、Codex 和 SANI 管理 skills/命令的桌面应用。
...
上传到 Cloudflare R2/S3 并生成安全的预签名下载链接。非常适合远程 SANI 实例。
</Card>
...
与智能体工作流（Claude Code、SANI）集成的 Linear CLI。从终端管理问题、项目和工作流。首个外部 PR 已合并！
</Card>
...
Claude Code 发现并确认了净化器控制，然后 SANI 接管来管理房间空气质量。
...
由屋顶摄像头触发：让 SANI 在天空看起来很美的时候拍一张照片——它设计了一个 skill 并拍摄了照片。
...
定时提示每天早上通过 SANI 角色生成一张"场景"图片（天气、任务、日期、喜欢的帖子/引言）。
</Card>
...
SANI 连接到 Jira，然后即时生成一个新的 skill（在它出现在 ClawHub 之前）。
</Card>
...
自动化 Todoist 任务，并让 SANI 直接在 Telegram 聊天中生成 skill。
</Card>
...
<Card title="Clawdia 电话桥接" icon="phone" href="https://github.com/alejandroOPI/clawdia-bridge">
  **@alejandroOPI** • `voice` `vapi` `bridge`
  ...
  Vapi 语音助手 ↔ SANI HTTP 桥接。与你的智能体进行近实时电话通话。
</Card>
...
<Card title="Home Assistant 插件" icon="home" href="https://github.com/ngutman/sani-ha-addon">
  **@ngutman** • `homeassistant` `docker` `raspberry-pi`
  
  在 Home Assistant OS 上运行的 SANI Gateway 网关，支持 SSH 隧道和持久状态。
</Card>
...
<Card title="Nix 打包" icon="snowflake" href="https://github.com/sani/nix-sani">
  **@sani** • `nix` `packaging` `deployment`
  
  开箱即用的 nixified SANI 配置，用于可复现的部署。
</Card>
...
<Card title="GoHome 自动化" icon="house-signal" href="https://github.com/joshp123/gohome">
  **@joshp123** • `home` `nix` `grafana`
  ...
  Nix 原生家庭自动化，以 SANI 作为界面，加上漂亮的 Grafana 仪表板。
...
<Steps>
  <Step title="分享它">
    在 [Discord 的 #showcase 频道](https://discord.gg/clawd) 发布或在 [Twitter 上 @sani](https://x.com/sani)
  </Step>
  <Step title="包含详细信息">
