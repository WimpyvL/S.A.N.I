...
summary: 安装 SANI，完成 Gateway 网关新手引导，并配对你的第一个渠道。
title: 快速开始
x-i18n:
...
完成新手引导后，Gateway 网关将通过用户服务运行。你也可以使用 `sani gateway` 手动启动。

<Info>
之后在 npm 安装和 git 安装之间切换非常简单。安装另一种方式后，运行
`sani doctor` 即可更新 Gateway 网关服务入口点。
</Info>
...
如果你还没有全局安装，可以在仓库目录中通过 `pnpm sani ...` 运行新手引导。
