...
read_when:
  - 设计 macOS 新手引导助手
  - 实现认证或身份设置
summary: SANI 的首次运行新手引导流程（macOS 应用）
title: 新手引导
x-i18n:
...
- 打开浏览器进行 OAuth（PKCE）
- 要求用户粘贴 `code#state` 值
- 将凭证写入 `~/.sani/credentials/oauth.json`
...
应用可以通过 npm/pnpm 安装全局 `sani` CLI，以便终端工作流和 launchd 任务开箱即用。
...
在首次智能体运行时，SANI 会引导一个工作区（默认 `~/.sani/workspace`）：
