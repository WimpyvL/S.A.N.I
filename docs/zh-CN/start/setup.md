...
summary: 设置指南：在保持最新的同时保持你的 SANI 设置个性化
title: 设置
x-i18n:
...
- **个性化设置存放在仓库之外：** `~/.sani/workspace`（工作区）+ `~/.sani/sani.json`（配置）。
...
- **配置：** `~/.sani/sani.json`（JSON/JSON5 格式）
- **工作区：** `~/.sani/workspace`（Skills、提示、记忆；将其设为私有 git 仓库）
...
如果你还没有全局安装，通过 `pnpm sani setup` 运行它。

## 稳定工作流（macOS 应用优先）

1. 安装并启动 **SANI.app**（菜单栏）。
...
- 运行 `sani setup`，然后 `sani channels login`，然后手动启动 Gateway 网关（`sani gateway`）。
...
- **配对允许列表**：`~/.sani/credentials/<channel>-allowFrom.json`
- **模型认证配置文件**：`~/.sani/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.sani/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的设置）

- 将 `~/.sani/workspace` 和 `~/.sani/` 保持为"你的东西"；不要将个人提示/配置放入 `sani` 仓库。
