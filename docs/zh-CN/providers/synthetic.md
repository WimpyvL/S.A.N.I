...
summary: 在 SANI 中使用 Synthetic 的 Anthropic 兼容 API
title: Synthetic
x-i18n:
...
Synthetic 提供兼容 Anthropic 的端点。SANI 将其注册为 `synthetic` 提供商，并使用 Anthropic Messages API。
...
```bash
sani onboard --auth-choice synthetic-api-key
```
...
注意：SANI 的 Anthropic 客户端会自动在 base URL 后追加 `/v1`，因此请使用 `https://api.synthetic.new/anthropic`（而非 `/anthropic/v1`）。如果 Synthetic 更改了其 base URL，请覆盖 `models.providers.synthetic.baseUrl`。
