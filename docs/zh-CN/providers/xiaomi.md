---
read_when:
  - 你想在 SANI 中使用 Xiaomi MiMo 模型
  - 你需要设置 XIAOMI_API_KEY
summary: 在 SANI 中使用 Xiaomi MiMo (mimo-v2-flash)
title: Xiaomi MiMo
x-i18n:
...
Xiaomi MiMo 是 **MiMo** 模型的 API 平台。它提供与 OpenAI 和 Anthropic 格式兼容的 REST API，并使用 API 密钥进行身份验证。请在 [Xiaomi MiMo 控制台](https://platform.xiaomimimo.com/#/console/api-keys) 中创建你的 API 密钥。SANI 使用 `xiaomi` 提供商配合 Xiaomi MiMo API 密钥。
...
```bash
sani onboard --auth-choice xiaomi-api-key
# 或非交互式
sani onboard --auth-choice xiaomi-api-key --xiaomi-api-key "$XIAOMI_API_KEY"
```
