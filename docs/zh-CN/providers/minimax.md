---
read_when:
  - 你想在 SANI 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 SANI 中使用 MiniMax M2.1
title: MiniMax
x-i18n:
...
```bash
sani plugins enable minimax-portal-auth  # 如果已加载则跳过
sani gateway restart  # 如果 Gateway 网关已在运行则重启
...
详情参见 [MiniMax OAuth 插件 README](https://github.com/sani/sani/tree/main/extensions/minimax-portal-auth)。
...
- 使用 `sani models list` 和 `sani models set minimax/MiniMax-M2.1` 切换模型。
...
- 运行 `sani configure` 并选择 **MiniMax M2.1**，或
