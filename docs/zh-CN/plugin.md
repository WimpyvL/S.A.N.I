...
插件只是一个**小型代码模块**，用额外功能（命令、工具和 Gateway 网关 RPC）扩展 SANI。

大多数时候，当你想要一个尚未内置到核心 SANI 的功能（或你想将可选功能排除在主安装之外）时，你会使用插件。
...
- 从 2026.1.15 起 Microsoft Teams 仅作为插件提供；如果使用 Teams，请安装 `@sani/msteams`。
...
SANI 插件是通过 jiti 在运行时加载的 **TypeScript 模块**。**配置验证不会执行插件代码**；它使用插件清单和 JSON Schema。参见 [插件清单](/plugins/manifest)。
...
4. 捆绑扩展（随 SANI 一起发布，**默认禁用**）

- `<sani>/extensions/*`

捆绑插件必须通过 `plugins.entries.<id>.enabled` 或 `sani plugins enable <id>` 显式启用。已安装的插件默认启用，但可以用相同方式禁用。

每个插件必须在其根目录中包含 `sani.plugin.json` 文件。如果路径指向文件，则插件根目录是文件的目录，必须包含清单。

如果多个插件解析到相同的 id，上述顺序中的第一个匹配项获胜，较低优先级的副本被忽略。

### 包集合

插件目录可以包含带有 `sani.extensions` 的 `package.json`：
...
渠道插件可以通过 `sani.channel` 广播新手引导元数据，通过 `sani.install` 广播安装提示。这使核心目录保持无数据。
...
SANI 还可以合并**外部渠道目录**（例如，MPM 注册表导出）。将 JSON 文件放在以下位置之一：

- `~/.sani/mpm/plugins.json`
- `~/.sani/mpm/catalog.json`
- `~/.sani/plugins/catalog.json`

或将 `SANI_PLUGIN_CATALOG_PATHS`（或 `SANI_MPM_CATALOG_PATHS`）指向一个或多个 JSON 文件（逗号/分号/`PATH` 分隔）。每个文件应包含 `{ "entries": [ { "name": "@scope/pkg", "sani": { "channel": {...}, "install": {...} } } ] }`。
...
如果插件导出 `id`，SANI 会使用它，但当它与配置的 id 不匹配时会发出警告。
...
- 插件配置使用嵌入在 `sani.plugin.json`（`configSchema`）中的 JSON Schema 进行验证。
...
插件也可以注册自己的顶级命令（例如：`sani voicecall`）。
...
- 插件管理的钩子在 `sani hooks list` 中显示为 `plugin:<id>`。
- 你不能通过 `sani hooks` 启用/禁用插件管理的钩子；而是启用/禁用插件。

## 提供商插件（模型认证）

插件可以注册**模型提供商认证**流程，以便用户可以在 SANI 内运行 OAuth 或 API 密钥设置（无需外部脚本）。
...
- 主包：`sani`（本仓库）
- 插件：`@sani/*` 下的独立 npm 包（例如：`@sani/voice-call`）

发布契约：

- 插件 `package.json` 必须包含带有一个或多个入口文件的 `sani.extensions`。
- 入口文件可以是 `.js` 或 `.ts`（jiti 在运行时加载 TS）。
- `sani plugins install <npm-spec>` 使用 `npm pack`，提取到 `~/.sani/extensions/<id>/`，并在配置中启用它。
...
- 单独发布的插件应运行自己的 CI（lint/构建/测试）并验证 `sani.extensions` 指向构建的入口点（`dist/index.js`）。
