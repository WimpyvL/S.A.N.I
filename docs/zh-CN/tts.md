...
SANI 可以使用 ElevenLabs、OpenAI 或 Edge TTS 将出站回复转换为音频。它可以在任何 SANI 能发送音频的地方工作；Telegram 会显示圆形语音消息气泡。
...
Edge TTS **不**需要 API 密钥。如果没有找到 API 密钥，SANI 默认使用 Edge TTS（除非通过 `messages.tts.edge.enabled=false` 禁用）。
...
TTS 配置位于 `sani.json` 中的 `messages.tts` 下。完整 schema 在 [Gateway 网关配置](/gateway/configuration)中。
...
- 如果 `provider` **未设置**，SANI 优先选择 `openai`（如果有密钥），然后是 `elevenlabs`（如果有密钥），否则是 `edge`。
...
斜杠命令将本地覆盖写入 `prefsPath`（默认：`~/.sani/settings/tts.json`，可通过 `SANI_TTS_PREFS` 或 `messages.tts.prefsPath` 覆盖）。
...
- **Edge TTS**：使用 `edge.outputFormat`（默认 `audio-24khz-48kbitrate-mono-mp3`）。
  ...
  - 如果配置的 Edge 输出格式失败，SANI 会使用 MP3 重试。
...
Discord 注意：`/tts` 是 Discord 的内置命令，所以 SANI 在那里注册 `/voice` 作为原生命令。文本 `/tts ...` 仍然有效。
