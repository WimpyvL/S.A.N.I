...
SANI 可以在 Telegram 私信中使用 `sendMessageDraft` 流式传输部分回复。
...
- 如果 Telegram 拒绝 HTML 负载，SANI 会以纯文本重试相同的消息。

## 命令（原生 + 自定义）

SANI 在启动时向 Telegram 的机器人菜单注册原生命令（如 `/status`、`/reset`、`/model`）。
你可以通过配置向菜单添加自定义命令：
...
- 自定义命令**仅是菜单条目**；除非你在其他地方处理它们，否则 SANI 不会实现它们。
...
**隐私注意：** `@userinfobot` 是第三方机器人。如果你更倾向于其他方式，将机器人添加到群组，发送一条消息，然后使用 `sani logs --follow` 读取 `chat.id`，或使用 Bot API `getUpdates`。
...
- 群组升级为超级群组，Telegram 发出 `migrate_to_chat_id`（聊天 ID 更改）。SANI 可以自动迁移 `channels.telegram.groups`。
...
Telegram 论坛话题在每条消息中包含 `message_thread_id`。SANI：
...
私聊在某些边缘情况下可能包含 `message_thread_id`。SANI 保持私信会话键不变，但在存在线程 id 时仍将其用于回复/草稿流式传输。
...
2. 运行 `sani logs --follow` 并查找 `from.id`。
...
SANI 支持接收和发送 Telegram 贴纸，并具有智能缓存功能。

### 接收贴纸

当用户发送贴纸时，SANI 根据贴纸类型处理：
...
贴纸通过 AI 的视觉功能处理以生成描述。由于相同的贴纸经常重复发送，SANI 缓存这些描述以避免冗余的 API 调用。
...
**缓存位置：** `~/.sani/telegram/sticker-cache.json`
...
SANI 使用 Bot API `sendMessageDraft`（不是真实消息），然后将最终回复作为普通消息发送。
...
**反应工作原理：**
Telegram 反应作为**单独的 `message_reaction` 事件**到达，而不是消息负载中的属性。当用户添加反应时，SANI：
...
- Telegram 机器人必须在 `allowed_updates` 中明确请求 `message_reaction`（由 SANI 自动配置）
...
- 示例：`sani message send --channel telegram --target 123456789 --message "hi"`。
...
- `sani channels status` 在配置期望未提及群组消息时显示警告。
- `sani channels status --probe` 可以额外检查显式数字群组 ID 的成员资格（它无法审计通配符 `"*"` 规则）。
...
- 检查 Gateway 网关日志：`sani logs --follow`（查找"skipping group message"）
...
- 升级到规范化中止信号的 SANI 构建版本，或在可以升级之前在 Node 20 上运行 Gateway 网关。
