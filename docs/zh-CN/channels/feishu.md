...
如果您刚安装完 SANI，可以直接运行向导，根据提示添加飞书：
...
- `sani gateway status` - 查看网关运行状态
- `sani logs --follow` - 查看实时日志
...
- `sani gateway status` - 查看网关运行状态
- `sani gateway restart` - 重启网关以应用新配置
- `sani logs --follow` - 查看实时日志
...
1. 运行 `sani channels add` 添加了 Feishu 渠道
2. 网关处于启动状态（可通过 `sani gateway status` 检查状态）
...
```bash
sani pairing approve feishu <配对码>
```
...
- **批准配对**：
  ```bash
  sani pairing list feishu      # 查看待审批列表
  sani pairing approve feishu <CODE>  # 批准
  ```
...
1. 启动网关并在群组中 @机器人发消息
2. 运行 `sani logs --follow` 查看日志中的 `chat_id`
...
1. 启动网关并给机器人发消息
2. 运行 `sani logs --follow` 查看日志中的 `open_id`
...
| -------------------------- | ----------------- |
| `sani gateway status`  | 查看网关运行状态  |
| `sani gateway install` | 安装/启动网关服务 |
| `sani gateway stop`    | 停止网关服务      |
| `sani gateway restart` | 重启网关服务      |
| `sani logs --follow`   | 实时查看日志输出  |
...
3. 检查是否选择了 **长连接** 模式
4. 检查应用权限是否完整
5. 检查网关是否正在运行：`sani gateway status`
6. 查看实时日志：`sani logs --follow`
