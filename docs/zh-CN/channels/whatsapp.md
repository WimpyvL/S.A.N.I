...
2. 在 `~/.sani/sani.json` 中配置 WhatsApp。
3. 运行 `sani channels login` 扫描二维码（关联设备）。
4. 启动 Gateway 网关。
...
WhatsApp 需要真实手机号码进行验证。VoIP 和虚拟号码通常会被封锁。在 WhatsApp 上运行 SANI 有两种支持的方式：

### 专用号码（推荐）

为 SANI 使用**单独的手机号码**。最佳用户体验，清晰的路由，无自聊天怪异问题。理想设置：**备用/旧 Android 手机 + eSIM**。保持 Wi-Fi 和电源连接，通过二维码关联。

**WhatsApp Business：** 你可以在同一设备上使用不同号码的 WhatsApp Business。非常适合将个人 WhatsApp 分开——安装 WhatsApp Business 并在那里注册 SANI 号码。
...
快速备选方案：在**你自己的号码**上运行 SANI。给自己发消息（WhatsApp"给自己发消息"）进行测试，这样就不会打扰联系人。在设置和实验期间需要在主手机上阅读验证码。**必须启用自聊天模式。**
...
当设置了 `identity.name` 时，自聊天回复默认为 `[{identity.name}]`（否则为 `[sani]`），
...
- 早期 SANI 版本支持 Twilio 的 WhatsApp Business 集成。
...
- 登录命令：`sani channels login`（通过关联设备扫描二维码）。
- 多账户登录：`sani channels login --account <id>`（`<id>` = `accountId`）。
- 默认账户（省略 `--account` 时）：如果存在则为 `default`，否则为第一个配置的账户 id（排序后）。
- 凭证存储在 `~/.sani/credentials/whatsapp/<accountId>/creds.json`。
- 备份副本在 `creds.json.bak`（损坏时恢复）。
- 旧版兼容性：较旧的安装将 Baileys 文件直接存储在 `~/.sani/credentials/` 中。
- 登出：`sani channels logout`（或 `--account <id>`）删除 WhatsApp 认证状态（但保留共享的 `oauth.json`）。
...
- **私信策略**：`channels.whatsapp.dmPolicy` 控制直接聊天访问（默认：`pairing`）。
  - 配对：未知发送者会收到配对码（通过 `sani pairing approve whatsapp <code>` 批准；码在 1 小时后过期）。
...
如果你在**个人 WhatsApp 号码**上运行 SANI，请启用 `channels.whatsapp.selfChatMode`（见上面的示例）。
...
**当我关联 WhatsApp 时，SANI 会给随机联系人发消息吗？**
不会。默认私信策略是**配对**，因此未知发送者只会收到配对码，他们的消息**不会被处理**。SANI 只会回复它收到的聊天，或你明确触发的发送（智能体/CLI）。

**WhatsApp 上的配对是如何工作的？**
配对是未知发送者的私信门控：

- 来自新发送者的第一条私信返回一个短码（消息不会被处理）。
- 使用以下命令批准：`sani pairing approve whatsapp <code>`（使用 `sani pairing list whatsapp` 列出）。
- 码在 1 小时后过期；每个渠道的待处理请求上限为 3 个。

**多个人可以在一个 WhatsApp 号码上使用不同的 SANI 实例吗？**
...
- 最佳效果：OGG/Opus。SANI 将 `audio/ogg` 重写为 `audio/ogg; codecs=opus`。
...
- 日志文件：`/tmp/sani/sani-YYYY-MM-DD.log`（可配置）。
...
- 修复：在 Gateway 网关主机上运行 `sani channels login` 并扫描二维码（WhatsApp → 设置 → 关联设备）。

**已关联但断开连接 / 重连循环**

- 症状：`channels status` 显示 `running, disconnected` 或警告"Linked but disconnected"。
- 修复：`sani doctor`（或重启 Gateway 网关）。如果问题持续，通过 `channels login` 重新关联并检查 `sani logs --follow`。
