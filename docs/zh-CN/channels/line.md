...
LINE 通过 LINE Messaging API 连接到 SANI。该插件作为 webhook 接收器在 Gateway 网关上运行，使用你的 channel access token + channel secret 进行身份验证。
...
```bash
sani plugins install @sani/line
```

本地检出（从 git 仓库运行时）：

```bash
sani plugins install ./extensions/line
```
...
```bash
sani pairing list line
sani pairing approve line <CODE>
```
