...
- 通过 CLI 安装：`sani plugins install @sani/zalouser`
- 或从源码检出安装：`sani plugins install ./extensions/zalouser`
- 详情：[插件](/plugin)
...
- 配置向导可以提示输入群组允许列表。
- 启动时，SANI 将允许列表中的群组/用户名称解析为 ID 并记录映射；未解析的条目保持原样。
...
- 重新登录：`sani channels logout --channel zalouser && sani channels login --channel zalouser`
