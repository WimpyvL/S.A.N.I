---
read_when:
  - 你已安装 SANI 但 `sani` 提示"command not found"
...
SANI 的运行时基线要求为 **Node 22+**。

如果你能运行 `npm install -g sani@latest`，但之后看到 `sani: command not found`，这几乎总是 **PATH** 问题：npm 存放全局二进制文件的目录不在你 shell 的 PATH 中。
...
如果 `$(npm prefix -g)/bin`（macOS/Linux）或 `$(npm prefix -g)`（Windows）**未出现**在 `echo "$PATH"` 的输出中，你的 shell 就无法找到全局 npm 二进制文件（包括 `sani`）。
