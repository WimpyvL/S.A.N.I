...
read_when:
  - 更新 SANI
  - 更新后出现问题
summary: 安全更新 SANI（全局安装或源码），以及回滚策略
title: 更新
x-i18n:
...
SANI 发展迅速（尚未到"1.0"）。将更新视为发布基础设施：更新 → 运行检查 → 重启（或使用会重启的 `sani update`）→ 验证。

## 推荐：重新运行网站安装程序（原地升级）

**首选**的更新路径是重新运行网站上的安装程序。它会检测现有安装、原地升级，并在需要时运行 `sani doctor`。
...
- 对于**全局安装**，脚本底层使用 `npm install -g sani@latest`。
...
- 如果你的 Gateway 网关作为服务运行，`sani gateway restart` 优于杀死 PID。
...
- 安装依赖、构建、构建控制 UI，并运行 `sani doctor`。
- 默认重启 Gateway 网关（使用 `--no-restart` 跳过）。

如果你通过 **npm/pnpm** 安装（没有 git 元数据），`sani update` 将尝试通过你的包管理器更新。如果无法检测到安装，请改用"更新（全局安装）"。

## 更新（控制 UI / RPC）

控制 UI 有**更新并重启**（RPC：`update.run`）。它：

1. 运行与 `sani update` 相同的源码更新流程（仅限 git checkout）。
...
- 当你运行打包的 `sani` 二进制文件（[`sani.mjs`](https://github.com/sani/sani/blob/main/sani.mjs)）或使用 Node 运行 `dist/` 时，`pnpm build` 很重要。
- 如果你从仓库 checkout 运行而没有全局安装，CLI 命令使用 `pnpm sani ...`。
- 如果你直接从 TypeScript 运行（`pnpm sani ...`），通常不需要重新构建，但**配置迁移仍然适用** → 运行 doctor。
- 在全局和 git 安装之间切换很容易：安装另一种方式，然后运行 `sani doctor` 以便将 Gateway 网关服务入口点重写为当前安装。

## 始终运行：`sani doctor`

Doctor 是"安全更新"命令。它故意很无聊：修复 + 迁移 + 警告。

注意：如果你是**源码安装**（git checkout），`sani doctor` 会提供先运行 `sani update`。
...
- 检测并将旧版 Gateway 网关服务（launchd/systemd；旧版 schtasks）迁移到当前 SANI 服务。
...
- macOS launchd（应用捆绑的 LaunchAgent）：`launchctl kickstart -k gui/$UID/bot.molt.gateway`（使用 `bot.molt.<profile>`；旧版 `com.sani.*` 仍然有效）
- Linux systemd 用户服务：`systemctl --user restart sani-gateway[-<profile>].service`
- Windows（WSL2）：`systemctl --user restart sani-gateway[-<profile>].service`
  - `launchctl`/`systemctl` 仅在服务已安装时有效；否则运行 `sani gateway install`。
...
提示：要查看当前发布的版本，运行 `npm view sani version`。
...
- 再次运行 `sani doctor` 并仔细阅读输出（它通常会告诉你修复方法）。
