...
read_when:
  - 安装 SANI
  - 你想从 GitHub 安装
summary: 安装 SANI（推荐安装器、全局安装或从源代码安装）
title: 安装
x-i18n:
...
pnpm approve-builds -g                # 批准 sani、node-llama-cpp、sharp 等
pnpm add -g sani@latest           # 重新运行以执行 postinstall 脚本
```
...
提示：如果你还没有全局安装，请通过 `pnpm sani ...` 运行仓库命令。
...
- 运行新手引导：`sani onboard --install-daemon`
- 快速检查：`sani doctor`
- 检查 Gateway 网关健康状态：`sani status` + `sani health`
- 打开仪表板：`sani dashboard`
...
如果 `$(npm prefix -g)/bin`（macOS/Linux）或 `$(npm prefix -g)`（Windows）**不**在 `echo "$PATH"` 的输出中，你的 shell 无法找到全局 npm 二进制文件（包括 `sani`）。
