...
本页跟踪 SANI 的**形式化安全模型**（目前是 TLA+/TLC；根据需要会添加更多）。

> 注意：一些较旧的链接可能引用了以前的项目名称。

**目标（北极星）：** 提供机器检查的论证，证明 SANI 在明确假设下执行其
...
**目前还不是什么：** 证明"SANI 在所有方面都是安全的"或完整 TypeScript 实现是正确的。

## 模型存放位置

模型维护在一个单独的仓库中：[vignesh07/sani-formal-models](https://github.com/vignesh07/sani-formal-models)。
