...
read_when:
  ...
  - 你想了解浏览器接管的安全影响
summary: Chrome 扩展：让 SANI 驱动你现有的 Chrome 标签页
title: Chrome 扩展
x-i18n:
...
SANI Chrome 扩展让智能体控制你**现有的 Chrome 标签页**（你的正常 Chrome 窗口），而不是启动一个单独的 sani 管理的 Chrome 配置文件。
...
然后 SANI 通过正常的 `browser` 工具界面控制附加的标签页（选择正确的配置文件）。
...
扩展作为静态文件包含在 SANI 发布版（npm 包）中。没有单独的"构建"步骤。

升级 SANI 后：

- 重新运行 `sani browser extension install` 以刷新 SANI 状态目录下的已安装文件。
- Chrome → `chrome://extensions` → 点击扩展上的"重新加载"。

## 使用它（无需额外配置）

SANI 附带一个名为 `chrome` 的内置浏览器配置文件，它指向默认端口上的扩展中继。
...
`sani browser extension path` 打印包含扩展文件的**已安装**磁盘目录。

CLI 有意**不**打印 `node_modules` 路径。始终先运行 `sani browser extension install` 将扩展复制到 SANI 状态目录下的稳定位置。
