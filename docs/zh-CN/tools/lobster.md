...
read_when:
  - 你想要具有显式审批的确定性多步骤工作流
  - 你需要恢复工作流而不重新运行早期步骤
summary: SANI 的类型化工作流运行时，支持可恢复的审批关卡。
title: Lobster
x-i18n:
...
Lobster 是一个工作流外壳，让 SANI 能够将多步骤工具序列作为单个确定性操作运行，并带有显式审批检查点。
...
- **一次调用代替多次**：SANI 运行一次 Lobster 工具调用并获得结构化结果。
...
SANI 以**工具模式**启动本地 `lobster` CLI，并从 stdout 解析 JSON 信封。
...
Lobster 可以运行包含 `name`、`args`、`steps`、`env`、`condition` 和 `approval` 字段的 YAML/JSON 工作流文件。在 SANI 工具调用中，将 `pipeline` 设置为文件路径。
...
在运行 SANI Gateway 网关的**同一主机**上安装 Lobster CLI（参见 [Lobster 仓库](https://github.com/sani/lobster)），并确保 `lobster` 在 `PATH` 中。
...
插件工具（如 `lobster`），SANI 会保持核心工具启用。要限制核心
...
- **仅限本地子进程** — 插件本身不进行网络调用。
- **无密钥** — Lobster 不管理 OAuth；它调用管理 OAuth 的 SANI 工具。
