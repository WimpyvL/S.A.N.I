---
read_when:
  - 在 Raspberry Pi 上设置 SANI 时
  - 在 ARM 设备上运行 SANI 时
  - 构建低成本常驻个人 AI 时
summary: 在 Raspberry Pi 上运行 SANI（低成本自托管设置）
title: Raspberry Pi
x-i18n:
...
# 在 Raspberry Pi 上运行 SANI

## 目标

在 Raspberry Pi 上运行持久、常驻的 SANI Gateway 网关，**一次性成本约 $35-80**（无月费）。
...
大多数 SANI 功能在 ARM64 上可用，但某些外部二进制文件可能需要 ARM 构建：
