---
read_when:
  - 你想在 GCP 上 24/7 运行 SANI
  - 你想要在自己的 VM 上运行生产级、常驻的 Gateway 网关
  - 你想完全控制持久化、二进制文件和重启行为
summary: 在 GCP Compute Engine VM（Docker）上 24/7 运行 SANI Gateway 网关并持久化状态
title: GCP
x-i18n:
...
# 在 GCP Compute Engine 上运行 SANI（Docker，生产 VPS 指南）

## 目标

使用 Docker 在 GCP Compute Engine VM 上运行持久化的 SANI Gateway 网关，具有持久状态、内置二进制文件和安全的重启行为。

如果你想要"SANI 24/7 大约 $5-12/月"，这是在 Google Cloud 上的可靠设置。
...
- 安装 Docker（隔离的应用运行时）
- 在 Docker 中启动 SANI Gateway 网关
- 在主机上持久化 `~/.sani` + `~/.sani/workspace`（重启/重建后仍保留）
...
SANI 在 Docker 中运行，但 Docker 不是真实来源。
所有长期状态必须在重启、重建和重启后仍然存在。

| 组件             | 位置                              | 持久化机制    | 说明                        |
| ---------------- | --------------------------------- | ------------- | --------------------------- |
| Gateway 网关配置 | `/home/node/.sani/`           | 主机卷挂载    | 包括 `sani.json`、令牌  |
| 模型认证配置文件 | `/home/node/.sani/`           | 主机卷挂载    | OAuth 令牌、API 密钥        |
| Skill 配置       | `/home/node/.sani/skills/`    | 主机卷挂载    | Skill 级别状态              |
| 智能体工作区     | `/home/node/.sani/workspace/` | 主机卷挂载    | 代码和智能体产物            |
| WhatsApp 会话    | `/home/node/.sani/`           | 主机卷挂载    | 保留 QR 登录                |
| Gmail 密钥环     | `/home/node/.sani/`           | 主机卷 + 密码 | 需要 `GOG_KEYRING_PASSWORD` |
