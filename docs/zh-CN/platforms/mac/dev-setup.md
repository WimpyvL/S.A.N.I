...
read_when:
  - 设置 macOS 开发环境
summary: 为在 SANI macOS 应用上工作的开发者提供的设置指南
title: macOS 开发设置
x-i18n:
...
本指南涵盖从源代码构建和运行 SANI macOS 应用程序的必要步骤。
...
要构建 macOS 应用并将其打包到 `dist/SANI.app`，运行：
...
macOS 应用期望全局安装 `sani` CLI 来管理后台任务。
...
1. 重置 TCC 权限：
   ...
   ```
2. 如果这不起作用，在 [`scripts/package-mac-app.sh`](https://github.com/sani/sani/blob/main/scripts/package-mac-app.sh) 中临时更改 `BUNDLE_ID` 以强制 macOS 从"全新状态"开始。
