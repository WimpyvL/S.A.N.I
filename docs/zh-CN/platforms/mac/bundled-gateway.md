...
SANI.app 不再捆绑 Node/Bun 或 Gateway 网关运行时。macOS 应用期望有一个**外部**的 `sani` CLI 安装，不会将 Gateway 网关作为子进程启动，而是管理一个每用户的 launchd 服务来保持 Gateway 网关运行（或者如果已有本地 Gateway 网关正在运行，则连接到现有的）。

## 安装 CLI（本地模式必需）

你需要在 Mac 上安装 Node 22+，然后全局安装 `sani`：
...
- `bot.molt.gateway`（或 `bot.molt.<profile>`；旧版 `com.sani.*` 可能仍然存在）
...
- CLI 也可以安装它：`sani gateway install`。

行为：

- "SANI Active"启用/禁用 LaunchAgent。
