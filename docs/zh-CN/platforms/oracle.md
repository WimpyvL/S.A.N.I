---
read_when:
  - 在 Oracle Cloud 上设置 SANI
  - 寻找 SANI 的低成本 VPS 托管
  - 想要在小型服务器上 24/7 运行 SANI
summary: 在 Oracle Cloud 上运行 SANI（Always Free ARM）
title: Oracle Cloud
x-i18n:
...
# 在 Oracle Cloud（OCI）上运行 SANI

## 目标

在 Oracle Cloud 的 **Always Free** ARM 层上运行持久化的 SANI Gateway 网关。

Oracle 的免费层非常适合 SANI（特别是如果你已经有 OCI 账户），但有一些权衡：
...
这会启用 Tailscale SSH，所以你可以从 tailnet 上的任何设备通过 `ssh sani` 连接——不需要公网 IP。
...
**从现在开始，通过 Tailscale 连接：** `ssh ubuntu@sani`（或使用 Tailscale IP）。
...
此设置通常消除了纯粹为了阻止互联网范围的 SSH 暴力破解而需要额外的基于主机的防火墙规则的*需求*——但你仍应保持操作系统更新，运行 `sani security audit`，并验证你没有意外地在公共接口上监听。
...
- `~/.sani/` — 配置、凭证、会话数据
- `~/.sani/workspace/` — 工作区（SOUL.md、记忆、产物）
