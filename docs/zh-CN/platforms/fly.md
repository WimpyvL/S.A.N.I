...
**目标：** SANI Gateway 网关运行在 [Fly.io](https://fly.io) 机器上，具有持久存储、自动 HTTPS 和 Discord/渠道访问。
...
| `internal_port = 3000`         | 必须与 `--port 3000`（或 `SANI_GATEWAY_PORT`）匹配以进行 Fly 健康检查 |
| `memory = "2048mb"`            | 512MB 太小；推荐 2GB                                                      |
| `SANI_STATE_DIR = "/data"` | 在卷上持久化状态                                                          |
...
- 非 loopback 绑定（`--bind lan`）出于安全需要 `SANI_GATEWAY_TOKEN`。
- 像对待密码一样对待这些 token。
- **优先使用环境变量而不是配置文件**来存储所有 API 密钥和 token。这可以避免密钥出现在 `sani.json` 中，防止意外暴露或记录。
...
**注意：** 使用 `SANI_STATE_DIR=/data` 时，配置路径是 `/data/sani.json`。
...
或访问 `https://my-sani.fly.dev/`

粘贴你的 Gateway 网关 token（来自 `SANI_GATEWAY_TOKEN` 的那个）进行认证。
...
**修复：** 确保 `internal_port` 与 Gateway 网关端口匹配（设置 `--port 3000` 或 `SANI_GATEWAY_PORT=3000`）。
...
如果使用 `--allow-unconfigured`，Gateway 网关会创建最小配置。你在 `/data/sani.json` 的自定义配置应该在重启时被读取。
...
**修复：** 确保 `fly.toml` 中设置了 `SANI_STATE_DIR=/data` 并重新部署。
