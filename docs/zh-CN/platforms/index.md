...
SANI 核心使用 TypeScript 编写。**Node 是推荐的运行时**。
...
- 服务状态：`sani gateway status`

## Gateway 网关服务安装（CLI）

使用以下任一方式（均支持）：

- 向导（推荐）：`sani onboard --install-daemon`
- 直接安装：`sani gateway install`
- 配置流程：`sani configure` → 选择 **Gateway service**
- 修复/迁移：`sani doctor`（提供安装或修复服务）

服务目标取决于操作系统：

- macOS：LaunchAgent（`bot.molt.gateway` 或 `bot.molt.<profile>`；旧版 `com.sani.*`）
- Linux/WSL2：systemd 用户服务（`sani-gateway[-<profile>].service`）
