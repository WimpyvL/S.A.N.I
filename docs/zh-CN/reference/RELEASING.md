...
- [ ] 更新 CLI/版本字符串：[`src/cli/program.ts`](https://github.com/sani/sani/blob/main/src/cli/program.ts) 和 [`src/provider-web.ts`](https://github.com/sani/sani/blob/main/src/provider-web.ts) 中的 Baileys user agent。
- [ ] 确认包元数据（name、description、repository、keywords、license）以及 `bin` 映射指向 [`sani.mjs`](https://github.com/sani/sani/blob/main/sani.mjs) 作为 `sani`。
- [ ] 如果依赖项有变化，运行 `pnpm install` 确保 `pnpm-lock.yaml` 是最新的。

2. **构建和产物**

- [ ] 如果 A2UI 输入有变化，运行 `pnpm canvas:a2ui:bundle` 并提交更新后的 [`src/canvas-host/a2ui/a2ui.bundle.js`](https://github.com/sani/sani/blob/main/src/canvas-host/a2ui/a2ui.bundle.js)。
...
- [ ] `SANI_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke`（Docker 安装冒烟测试，快速路径；发布前必需）
  - 如果已知上一个 npm 发布版本有问题，为预安装步骤设置 `SANI_INSTALL_SMOKE_PREVIOUS=<last-good-version>` 或 `SANI_INSTALL_SMOKE_SKIP_PREVIOUS=1`。
- [ ]（可选）完整安装程序冒烟测试（添加非 root + CLI 覆盖）：`pnpm test:install:smoke`
- [ ]（可选）安装程序 E2E（Docker，运行 `curl -fsSL https://sani.ai/install.sh | bash`，新手引导，然后运行真实工具调用）：
...
- [ ] 生成 Sparkle appcast（通过 [`scripts/make_appcast.sh`](https://github.com/sani/sani/blob/main/scripts/make_appcast.sh) 生成 HTML 注释）并更新 `appcast.xml`。
...
- [ ] 按照 [macOS 发布](/platforms/mac/release) 获取确切命令和所需环境变量。
  ...
  - 如果进行公证，使用从 App Store Connect API 环境变量创建的 `sani-notary` 钥匙串配置文件（参见 [macOS 发布](/platforms/mac/release)）。
...
- [ ] 验证注册表：`npm view sani version`、`npm view sani dist-tags` 和 `npx -y sani@X.Y.Z --version`（或 `--help`）。

### 故障排除（来自 2.0.0-beta2 发布的笔记）

- **npm pack/publish 挂起或产生巨大 tarball**：`dist/SANI.app` 中的 macOS 应用包（和发布 zip）被扫入包中。通过 `package.json` 的 `files` 白名单发布内容来修复（包含 dist 子目录、docs、skills；排除应用包）。用 `npm pack --dry-run` 确认 `dist/SANI.app` 未列出。
...
- [ ] 为 `vX.Y.Z` 创建/刷新 GitHub 发布，**标题为 `sani X.Y.Z`**（不仅仅是标签）；正文应包含该版本的**完整**变更日志部分（亮点 + 更改 + 修复），内联显示（无裸链接），且**不得在正文中重复标题**。
- [ ] 附加产物：`npm pack` tarball（可选）、`SANI-X.Y.Z.zip` 和 `SANI-X.Y.Z.dSYM.zip`（如果生成）。
- [ ] 提交更新后的 `appcast.xml` 并推送（Sparkle 从 main 获取源）。
- [ ] 从干净的临时目录（无 `package.json`），运行 `npx -y sani@X.Y.Z send --help` 确认安装/CLI 入口点正常工作。
- [ ] 宣布/分享发布说明。

## 插件发布范围（npm）

我们只发布 `@sani/*` 范围下的**现有 npm 插件**。不在 npm 上的内置插件保持**仅磁盘树**（仍在 `extensions/**` 中发布）。

获取列表的流程：

1. `npm search @sani --json` 并捕获包名。
