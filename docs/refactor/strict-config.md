...
- Plugin manifests (`sani.plugin.json`) are required for all plugins.

## Plugin schema enforcement

- Each plugin provides a strict JSON Schema for its config (inline in the manifest).
- Plugin load flow:
  1. Resolve plugin manifest + schema (`sani.plugin.json`).
...
Everything else must hard-fail with: “Config invalid. Run `sani doctor --fix`.”
