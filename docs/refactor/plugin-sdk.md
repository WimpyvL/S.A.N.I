...
- Publish as `sani/plugin-sdk` (or export from core under `sani/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `SANIPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: SANIConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: SANIConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: SANIConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: SANIConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: SANIConfig): string;
  };
};
```
...
- Introduce `sani/plugin-sdk`.
- Add `api.runtime` to `SANIPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `saniRuntime: ">=2026.2.0"`).
