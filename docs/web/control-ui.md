...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/sani`)
...
If the page fails to load, start the Gateway first: `sani gateway`.
...
you revoke it with `sani devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.sani/sani.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. SANI
...
SANI **blocks** Control UI connections without device identity.
