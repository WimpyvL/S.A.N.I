...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in SANI
...
Running SANI via Node with `tsx` fails at startup with:

```
[sani] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `sani status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch sani.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node sani.mjs status` works on Node 25.
