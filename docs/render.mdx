...
Deploy SANI on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a
  href="https://render.com/deploy?repo=https://github.com/sani/sani"
  target="_blank"
  rel="noreferrer"
>
  Deploy to Render
...
repository configures everything needed to run SANI:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: SANI_STATE_DIR
        value: /data/.sani
      - key: SANI_WORKSPACE_DIR
        value: /data/workspace
      - key: SANI_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/sani`.
...
If you use the original SANI repository, Render will not auto-deploy your SANI. To update it, run a manual Blueprint sync from the dashboard.
...
For SANI, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any SANI host.
