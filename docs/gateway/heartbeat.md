...
- During heartbeat runs, SANI treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `sani sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, SANI skips the heartbeat run entirely (no model call).
...
```bash
sani system event --text "Check for urgent follow-ups" --mode now
```
