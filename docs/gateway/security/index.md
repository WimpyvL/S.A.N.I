...
## Quick check: `sani security audit`
...
```bash
sani security audit
sani security audit --deep
sani security audit --fix
```
...
- Tighten local perms (`~/.sani` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... _spicy_. Here’s how to not get pwned.

SANI is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, SANI also attempts a best-effort live Gateway probe.
...
`sani security audit` warns when this setting is enabled.
...
gateway:
  ...
  auth:
    mode: password
    password: ${SANI_GATEWAY_PASSWORD}
```
...
SANI stores session transcripts on disk under `~/.sani/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.sani` (see the audit section below). If you need
...
SANI can refresh the skills list mid-session:
...
- If you install plugins from npm (`sani plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.sani/extensions/<pluginId>/` (or `$SANI_STATE_DIR/extensions/<pluginId>/`).
  - SANI uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
sani pairing approve <channel> <code>
```
...
By default, SANI routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
SANI has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.sani/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.sani or your logs.”
...
4. **Re-run audit**
   - `sani security audit --deep` and confirm the report is clean.
...
- `~/.sani/sani.json`: `600` (user read/write only)
- `~/.sani`: `700` (user only)

`sani doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `SANI_GATEWAY_PORT`
...
The Gateway broadcasts its presence via mDNS (`_sani-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `SANI_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `sani doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `SANI_GATEWAY_PASSWORD`).

Rotation checklist (token/password):

1. Generate/set a new secret (`gateway.auth.token` or `SANI_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), SANI
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. SANI verifies the identity by resolving the
...
- SANI will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.sani/` (or `$SANI_STATE_DIR/`) may contain secrets or private data:

- `sani.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `sani status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@sani", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.sani/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `sani` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only SANI clients can connect.
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `sani gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `SANI_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/sani/sani-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.sani/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + SANI version
...
Found a vulnerability in SANI? Please report responsibly:

1. Email: security@sani.ai
