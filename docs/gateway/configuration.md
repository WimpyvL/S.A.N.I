---
summary: "All configuration options for ~/.sani/sani.json with examples"
read_when:
...
SANI reads an optional **JSON5** config from `~/.sani/sani.json` (comments + trailing commas allowed).

If the file is missing, SANI uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.sani/workspace`). You usually only need a config to:
...
SANI only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `sani doctor`, `sani logs`, `sani health`, `sani status`, `sani service`, `sani help`).
- Run `sani doctor` to see the exact issues.
- Run `sani doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `sani config set`. Keep a backup of `~/.sani/sani.json`.
...
```bash
sani gateway call config.get --params '{}' # capture payload.hash
sani gateway call config.apply --params '{
...
```bash
sani gateway call config.get --params '{}' # capture payload.hash
sani gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.sani/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@sani", "reisponde"] },
      },
    ],
  },
  channels: {
...
// ~/.sani/agents.json5
{
  ...
  list: [{ id: "main", workspace: "~/.sani/workspace" }],
}
```
...
{ "$include": "/etc/sani/base.json5" } // absolute
...
```json5
// ~/.sani/clients/mueller/agents.json5
[
...
```json5
// ~/.sani/clients/mueller/broadcast.json5
{
...
SANI reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, SANI runs your login shell and imports only the missing expected keys (never overrides).
...
- `SANI_LOAD_SHELL_ENV=1`
- `SANI_SHELL_ENV_TIMEOUT_MS=15000`
...
SANI stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.sani/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.sani/credentials/oauth.json` (or `$SANI_STATE_DIR/credentials/oauth.json`)
...
- `~/.sani/agent/*` (migrated by `sani doctor` into `~/.sani/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `SANI_OAUTH_DIR`
- Agent dir (default agent root override): `SANI_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, SANI imports `oauth.json` entries into `auth-profiles.json`.
...
If set, SANI derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/sani/sani-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/sani/sani.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.sani/credentials/whatsapp/biz
          // authDir: "~/.sani/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `sani doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@sani", "sani"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@sani"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.sani/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.sani/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.sani/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.sani/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.sani/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.sani/workspace-home" },
      { id: "work", workspace: "~/.sani/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `sani.json`).
...
SANI starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["sani-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-sani",
...
SANI starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `sani plugins install @sani/mattermost` (or `./extensions/mattermost` from a git checkout).
...
SANI starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
SANI spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, SANI
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/sani" } },
}
```
...
When a file exceeds this limit, SANI logs a warning and injects a truncated
...
message envelopes). If unset, SANI uses the host timezone at runtime.
...
`[sani]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[sani]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[sani]"`, SANI will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, SANI generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.sani/settings/tts.json",
      elevenlabs: {
...
SANI also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, SANI currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `sani configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:sani`: all built-in SANI tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.sani/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.sani/sandboxes",
        docker: {
          image: "sani-sandbox:bookworm-slim",
          containerPrefix: "sani-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
        ...
        browser: {
          enabled: false,
          image: "sani-sandbox-browser:bookworm-slim",
          containerPrefix: "sani-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
SANI uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.sani/agents/<agentId>/agent/models.json` or by defining the same schema inside your
SANI config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, SANI writes/merges a `models.json` into
`~/.sani/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. SANI uses
...
- Shortcut: `sani onboard --auth-choice opencode-zen`.
...
Shortcut: `sani onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `sani onboard --auth-choice moonshot-api-key`.
- Model ref: `moonshot/kimi-k2.5`.
- For the China endpoint, either:
  - Run `sani onboard --auth-choice moonshot-api-key-cn` (wizard will set `https://api.moonshot.cn/v1`), or
...
- Set `KIMI_API_KEY` in the environment or use `sani onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `sani onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `sani onboard --auth-choice minimax-api`.
...
- Override the agent config root with `SANI_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.sani/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.sani/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.sani/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, SANI stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.sani/skills` (workspace skills
...
from `~/.sani/extensions`, `<workspace>/.sani/extensions`, plus any
...
### `browser` (sani-managed browser)

SANI can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for sani and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (SANI.app menubar, or `sani gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      sani: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/sani" }
...
- When a connection comes from one of these IPs, SANI uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `sani gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `SANI_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `SANI_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. SANI
...
- SANI.app watches `~/.sani/sani.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.sani/sani.json` (or `SANI_CONFIG_PATH`) and applies changes automatically.
...
- `~/.sani/sani.json` (or `SANI_CONFIG_PATH`)
...
- `SANI_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `sani --dev …` → uses `~/.sani-dev` + shifts ports from base `19001`
- `sani --profile <name> …` → uses `~/.sani-<name>` (port via config/env/flags)
...
Gmail helper config (used by `sani webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "sani@gmail.com",
...
- Set `SANI_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, SANI defaults `serve.path` to `/` so
...
Default root: `~/.sani/workspace/canvas`  
Default port: `18793` (chosen to avoid the sani browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__sani__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__sani__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.sani/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.sani/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.sani/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_sani-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `sani` (advertises `sani.local`). Override with `SANI_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_sani-gw._tcp` under `~/.sani/dns/` using the configured discovery domain (example: `sani.internal.`).
