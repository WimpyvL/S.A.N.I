---
summary: "Quick troubleshooting guide for common SANI failures"
read_when:
...
When SANI misbehaves, here's how to fix it.
...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------- |
| `sani status`                  | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview                       |
| `sani status --all`            | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail                                   | When you need to share a debug report             |
| `sani status --deep`           | Runs gateway health checks (incl. provider probes; requires reachable gateway)                         | When “configured” doesn’t mean “working”          |
| `sani gateway probe`           | Gateway discovery + reachability (local + remote targets)                                              | When you suspect you’re probing the wrong gateway |
| `sani channels status --probe` | Asks the running gateway for channel status (and optionally probes)                                    | When gateway is reachable but channels misbehave  |
| `sani gateway status`          | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error                      | When the service “looks loaded” but nothing runs  |
| `sani logs --follow`           | Live logs (best signal for runtime issues)                                                             | When you need the actual failure reason           |

**Sharing output:** prefer `sani status --all` (it redacts tokens). If you paste `sani status`, consider setting `SANI_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  sani models auth setup-token --provider anthropic
  ```
...
```bash
sani models auth paste-token --provider anthropic
sani models status
```
...
- Preferred: `sani logs --follow`
- File logs (always): `/tmp/sani/sani-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$SANI_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u sani-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  sani config set gateway.mode local
  ```
...
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  sani config set gateway.mode remote
  sani config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `sani setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.sani/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `sani doctor`
...
- then run `sani sandbox recreate --agent <id>` (or `--all`)
...
- `gateway.mode` must be `local` for `sani gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `sani gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `sani gateway status` and `sani doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `SANI_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `sani gateway status` shows a config mismatch**
...
- Fix: rerun `sani gateway install --force` from the same `--profile` / `SANI_STATE_DIR` you want the service to use.

**If `sani gateway status` reports service config issues**

- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `sani doctor` to update it (or `sani gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `SANI_GATEWAY_TOKEN`) and restart the service.

**If `sani gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/sani` on disk.
...
Symptoms: `pwd` or file tools show `~/.sani/sandboxes/...` even though you
...
SANI intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `sani models list` or
  `sani models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${SANI_CONFIG_PATH:-$HOME/.sani/sani.json}"
```
...
tail -f "$(ls -t /tmp/sani/sani-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
sani logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.sani/agents/<agentId>/sessions/
```
...
sani logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
sani gateway --verbose
```

If you’re logged out / unlinked:

```bash
sani channels logout
trash "${SANI_STATE_DIR:-$HOME/.sani}/credentials" # if logout can't cleanly remove everything
sani channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/sani/sani-*.log | head -1)" | tail -20
```

### High Memory Usage

SANI keeps conversation history in memory.
...
SANI now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
- `sani doctor` reports every invalid entry.
- `sani doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `sani logs`, `sani health`, `sani status`, `sani gateway status`, and `sani gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/sani/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `sani models status` (CLI).
...
5. Run `sani channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. SANI scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/sani/sani/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
sani gateway status
sani gateway stop
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.sani.* still works)
```
...
Ensure the global `sani` CLI is installed and matches the app version:

```bash
sani --version
npm install -g sani@<version>
```
...
#   ${SANI_CONFIG_PATH:-$HOME/.sani/sani.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
sani gateway --verbose
sani channels login --verbose
```
...
| Log                               | Location                                                                                                                                                                                                                                                                                                                    |
| --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gateway file logs (structured)    | `/tmp/sani/sani-YYYY-MM-DD.log` (or `logging.file`)                                                                                                                                                                                                                                                                 |
| Gateway service logs (supervisor) | macOS: `$SANI_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.sani/logs/...`; profiles use `~/.sani-<profile>/logs/...`)<br />Linux: `journalctl --user -u sani-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "SANI Gateway (<profile>)" /V /FO LIST` |
| Session files                     | `$SANI_STATE_DIR/agents/<agentId>/sessions/`                                                                                                                                                                                                                                                                            |
| Media cache                       | `$SANI_STATE_DIR/media/`                                                                                                                                                                                                                                                                                                |
| Credentials                       | `$SANI_STATE_DIR/credentials/`                                                                                                                                                                                                                                                                                          |
...
trash "${SANI_STATE_DIR:-$HOME/.sani}"
sani channels login         # re-pair WhatsApp
sani gateway restart           # or: sani gateway
```
...
1. Check logs first: `/tmp/sani/` (default: `sani-YYYY-MM-DD.log`, or your configured `logging.file`)
