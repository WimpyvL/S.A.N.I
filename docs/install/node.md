...
read_when:
  - "You installed SANI but `sani` is “command not found”"
...
SANI’s runtime baseline is **Node 22+**.

If you can run `npm install -g sani@latest` but later see `sani: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
