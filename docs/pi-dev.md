...
This guide summarizes a sane workflow for working on the pi integration in SANI.
...
- Trigger the agent directly:
  - `pnpm sani agent --message "Hello" --thinking low`
...
State lives under the SANI state directory. Default is `~/.sani`. If `SANI_STATE_DIR` is set, use that directory instead.

To reset everything:

- `sani.json` for config
...
- https://docs.sani.ai/testing
- https://docs.sani.ai/start/getting-started
