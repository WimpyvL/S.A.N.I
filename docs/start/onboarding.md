---
summary: "First-run onboarding flow for SANI (macOS app)"
read_when:
...
- Writes credentials to `~/.sani/credentials/oauth.json`
...
The app can install the global `sani` CLI via npm/pnpm so terminal
...
On the first agent run, SANI bootstraps a workspace (default `~/.sani/workspace`):
...
```bash
sani webhooks gmail setup --account you@gmail.com
```
