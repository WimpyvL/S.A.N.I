...
Fastest chat: open the Control UI (no channel setup needed). Run `sani dashboard`
and chat in the browser, or open `http://127.0.0.1:18789/` on the gateway host.
Docs: [Dashboard](/web/dashboard) and [Control UI](/web/control-ui).

Recommended path: use the **CLI onboarding wizard** (`sani onboard`). It sets up:
...
- **Recommended:** Brave Search API key for web search. Easiest path:
  `sani configure --section web` (stores `tools.web.search.apiKey`).
...
- OAuth credentials (legacy import): `~/.sani/credentials/oauth.json`
...
If you’re hacking on SANI itself, run from source:
...
If you don’t have a global install yet, run the onboarding step via `pnpm sani ...` from the repo.
`pnpm build` also bundles A2UI assets; if you need to run just that step, use `pnpm canvas:a2ui:bundle`.

Gateway (from this repo):

```bash
node sani.mjs gateway --port 18789 --verbose
```
...
If `sani health` shows “no auth configured”, go back to the wizard and set OAuth/key auth — the agent won’t be able to respond without it.

Tip: `sani status --all` is the best pasteable, read-only debug report.
Health probes: `sani health` (or `sani status --deep`) asks the running gateway for a health snapshot.
