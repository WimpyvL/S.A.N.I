---
title: Lobster
summary: "Typed workflow runtime for SANI with resumable approval gates."
description: Typed workflow runtime for SANI — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets SANI run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: SANI runs one Lobster tool call and gets a structured result.
...
SANI launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | sani.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
sani.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In SANI tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the SANI Gateway (see the [Lobster repo](https://github.com/sani/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), SANI keeps core tools enabled. To restrict core
...
→ sani calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ sani calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls SANI tools that do.
