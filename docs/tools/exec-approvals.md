...
- **gateway host** → `sani` process on the gateway machine
...
`~/.sani/exec-approvals.json` directly.

CLI: `sani approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
