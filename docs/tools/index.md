---
summary: "Agent tool surface for SANI (browser, canvas, nodes, message, cron) replacing legacy `sani-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `sani-*` skills
title: "Tools"
---

# Tools (SANI)

SANI exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `sani-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `sani.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, SANI logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`sani node run`).
...
- Requires a Brave API key (recommended: `sani configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated SANI-managed browser.
...
- Quick smoke: `sani nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `sani gateway` restart in-place)
