...
ClawHub is the **public skill registry for SANI**. It is a free service: all skills are public, open, and visible to everyone for sharing and reuse. A skill is just a folder with a `SKILL.md` file (plus supporting text files). You can browse skills in the web app or use the CLI to search, install, update, and publish skills.

Site: [clawhub.ai](https://clawhub.ai)

## What ClawHub is

- A public registry for SANI skills.
...
4. Users browse, download, and install skills in SANI.
...
If you want to add new capabilities to your SANI agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
4. Start a new SANI session so it picks up the new skill.
...
## How it fits into SANI

By default, the CLI installs skills into `./skills` under your current working directory. If a SANI workspace is configured, `clawhub` falls back to that workspace unless you override `--workdir` (or `CLAWHUB_WORKDIR`). SANI loads workspace skills from `<workspace>/skills` and will pick them up in the **next** session. If you already use `~/.sani/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches SANI how to perform a
...
Interested in becoming a moderator? Ask in the SANI Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to SANI workspace).
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/sani/skills` and `~/.sani/skills`). This is designed to find older skill installs without extra flags.
