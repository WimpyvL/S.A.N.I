---
summary: "Chrome extension: let SANI drive your existing Chrome tab"
read_when:
...
The SANI Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate sani-managed Chrome profile.
...
SANI then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the SANI release (npm package) as static files. There is no separate “build” step.

After upgrading SANI:

- Re-run `sani browser extension install` to refresh the installed files under your SANI state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

SANI ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want SANI to control.
...
- `ON`: attached; SANI can drive that tab.
...
Debugging: `sani sandbox explain`
...
`sani browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `sani browser extension install` first to copy the extension to a stable location under your SANI state directory.
...
- **This is not isolated** like the dedicated sani-managed profile.
