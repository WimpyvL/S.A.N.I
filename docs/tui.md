...
```bash
sani tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `sani tui --deliver`
...
- Check the Gateway logs: `sani logs --follow`.
- Confirm the agent can run: `sani status` and `sani models status`.
...
- No agents in picker: check `sani agents list` and your routing config.
