---
summary: "Use Venice AI privacy-focused models in SANI"
read_when:
  - You want privacy-focused inference in SANI
...
```bash
sani onboard --auth-choice venice-api-key
```
...
```bash
sani chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, SANI shows all available Venice models. Pick based on your needs:
...
```bash
sani models set venice/claude-opus-45
sani models set venice/llama-3.3-70b
```

List all available models:

```bash
sani models list | grep venice
```

## Configure via `sani configure`

1. Run `sani configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
SANI automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
sani chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
sani chat --model venice/claude-opus-45

# Use uncensored model
sani chat --model venice/venice-uncensored

# Use vision model with image
sani chat --model venice/qwen3-vl-235b-a22b

# Use coding model
sani chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `sani models list` to see currently available models. Some models may be temporarily offline.
