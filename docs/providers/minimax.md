---
summary: "Use MiniMax M2.1 in SANI"
read_when:
  - You want MiniMax models in SANI
...
```bash
sani plugins enable minimax-portal-auth  # skip if already loaded.
sani gateway restart  # restart if gateway is already running
sani onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/sani/sani/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `sani.json`:
...
- Use `sani models list` and `sani models set minimax/MiniMax-M2.1` to switch.
...
- Running `sani configure` and selecting **MiniMax M2.1**, or
