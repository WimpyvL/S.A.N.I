---
summary: "SANI on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up SANI on a Raspberry Pi
  - Running SANI on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# SANI on Raspberry Pi

## Goal

Run a persistent, always-on SANI Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most SANI features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled sani

# Enable if not
sudo systemctl enable sani
...
```bash
# Check logs
journalctl -u sani --no-pager -n 100

# Common fix: rebuild
cd ~/sani  # if using hackable install
npm run build
sudo systemctl restart sani
```
