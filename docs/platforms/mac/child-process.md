...
service via the external `sani` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`SANI_PROFILE`; legacy `com.sani.*` is supported).
