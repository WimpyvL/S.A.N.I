...
SANI integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- SANI owns lifecycle when `channels.signal.autoStart=true`.
...
- SANI spawns `imsg rpc` as a child process (legacy iMessage integration).
