...
SANI features that can generate provider usage or paid API calls.
...
- `sani status --usage` and `sani channels list` show provider **usage windows**
...
SANI can pick up credentials from:
...
`sani models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
