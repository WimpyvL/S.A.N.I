...
read_when:
  - Designing workspace memory (~/.sani/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep SANI integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.sani/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.sani/workspace` as canonical human-readable memory.
...
```
~/.sani/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in SANI**, but keep a separable core library.

### Why integrate into SANI?
...
- You want the agent itself to call the tools:
  - `sani memory recall "…" --k 25 --since 30d`
  - `sani memory reflect --since 7d`
...
Pragmatic take for `~/.sani/workspace`:
