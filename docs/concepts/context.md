...
read_when:
  - You want to understand what “context” means in SANI
...
“Context” is **everything SANI sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (SANI-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How SANI builds the system prompt

The system prompt is **SANI-owned** and rebuilt each run. It includes:
...
By default, SANI injects a fixed set of workspace files (if present):
