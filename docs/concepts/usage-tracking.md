...
- `/usage cost` in chats: local cost summary aggregated from SANI session logs.
- CLI: `sani status --usage` prints a full per-provider breakdown.
- CLI: `sani channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
