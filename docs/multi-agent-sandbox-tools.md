...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `sani sandbox explain`.
...
Legacy `agent.*` configs are migrated by `sani doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=sani-sbx-"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${SANI_STATE_DIR:-$HOME/.sani}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
