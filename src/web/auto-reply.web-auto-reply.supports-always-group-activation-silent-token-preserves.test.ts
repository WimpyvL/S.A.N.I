...
beforeEach(async () => {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "sani-web-home-"));
...
const makeSessionStore = async (
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "sani-session-"));
...
describe("web auto-reply", () => {
  ...
  it("supports always-on group activation with silent token and clears pending history", async () => {
    ...
    setLoadConfigMock(() => ({
      messages: {
        groupChat: { mentionPatterns: ["@sani"] },
      },
  ...
  it("ignores JID mentions in self-chat mode (group chats)", async () => {
    ...
    // Text-based mentionPatterns still work (user can type "sani" explicitly).
    await capturedOnMessage?.({
      body: "sani ping",
      from: "123@g.us",
      conversationId: "123@g.us",
  ...
  it("emits heartbeat logs with connection metadata", async () => {
    vi.useFakeTimers();
    const logPath = `/tmp/sani-heartbeat-${crypto.randomUUID()}.log`;
  ...
  it("logs outbound replies to file", async () => {
    const logPath = `/tmp/sani-log-test-${crypto.randomUUID()}.log`;
    setLoggerOverride({ level: "trace", file: logPath });
