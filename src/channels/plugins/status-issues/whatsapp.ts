  ...
  for (const entry of accounts) {
    ...
    if (!linked) {
      issues.push({
        ...
        fix: `Run: ${formatCliCommand("sani channels login")} (scan QR on the gateway host).`,
      });
      continue;
    }
    ...
    if (running && !connected) {
      issues.push({
        ...
        fix: `Run: ${formatCliCommand("sani doctor")} (or restart the gateway). If it persists, relink via channels login and check logs.`,
      });
    }
  }
  return issues;
