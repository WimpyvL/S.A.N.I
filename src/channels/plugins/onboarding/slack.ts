...
function setSlackDmPolicy(cfg: SANIConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "SANI";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for SANI`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: SANIConfig, allowFrom: string[]): SANIConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
