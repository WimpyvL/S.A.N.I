...
function setSignalDmPolicy(cfg: SANIConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
export const signalOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    await prompter.note(
      [
        'Link device with: signal-cli link -n "SANI"',
        "Scan QR in Signal → Linked Devices",
        `Then run: ${formatCliCommand("sani gateway call channels.status --params '{\"probe\":true}'")}`,
        `Docs: ${formatDocsLink("/signal", "signal")}`,
      ].join("\n"),
      "Signal next steps",
    );
