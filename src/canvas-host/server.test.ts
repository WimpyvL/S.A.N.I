...
describe("canvas host", () => {
  it("injects live reload script", () => {
    ...
    expect(out).toContain("saniCanvasA2UIAction");
    expect(out).toContain("saniSendUserAction");
  });

  it("creates a default index.html when missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "sani-canvas-"));
    ...
    try {
      ...
      expect(html).toContain("saniSendUserAction");
  ...
  it("serves the gateway-hosted A2UI scaffold", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.saniA2UI = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/__sani__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("sani-a2ui-host");
      expect(html).toContain("saniCanvasA2UIAction");

      const bundleRes = await fetch(
        `http://127.0.0.1:${server.port}/__sani__/a2ui/a2ui.bundle.js`,
      );
      const js = await bundleRes.text();
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("saniA2UI");
    } finally {
      await server.close();
      if (createdBundle) {
