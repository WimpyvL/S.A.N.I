...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.saniCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.saniCanvasA2UIAction &&
        typeof window.saniCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.saniSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("sani:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android SANI node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.saniSendUserAction === "function"
        ? window.saniSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.SANI_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.SANI_SKIP_CANVAS_HOST)) {
    return true;
...
function resolveDefaultCanvasRoot(): string {
  const candidates = [path.join(os.homedir(), ".sani", "canvas")];
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>SANI Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
