...
vi.mock("../config/sessions.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    resolveStorePath: vi.fn(() => "/tmp/sani-sessions.json"),
...
describe("discord tool result dispatch", () => {
  it(
    ...
    async () => {
      ...
      const cfg = {
        ...
        session: { store: "/tmp/sani-sessions.json" },
        channels: {
          discord: {
        ...
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bsani\\b"] },
        },
    ...
    await handler(
      {
        message: {
          ...
          referencedMessage: {
            ...
            mentionedUsers: [],
            mentionedRoles: [],
            author: { id: "bot-id", bot: true, username: "SANI" },
          },
        },
        author: { id: "u1", bot: false, username: "Ada" },
        member: { nickname: "Ada" },
  ...
  it("treats forum threads as distinct sessions without channel payloads", async () => {
    ...
    const cfg = {
      agent: { model: "anthropic/claude-opus-4-5", workspace: "/tmp/sani" },
      session: { store: "/tmp/sani-sessions.json" },
      channels: {
        discord: {
          dm: { enabled: true, policy: "open" },
