...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "sani-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".sani", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<SANIConfig> = {},
): SANIConfig {
  const base: SANIConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "sani"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".sani", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
