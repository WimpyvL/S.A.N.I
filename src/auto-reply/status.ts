...
type AgentConfig = Partial<NonNullable<NonNullable<SANIConfig["agents"]>["defaults"]>>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.sani/sessions/<SessionId>.jsonl)
  if (!sessionId) {
    return undefined;
  }
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🦞 SANI ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: SANIConfig): string {
  const lines = ["ℹ️ Help", ""];
