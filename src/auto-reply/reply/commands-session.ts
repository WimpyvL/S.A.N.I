...
import { scheduleGatewaySigusr1Restart, triggerSaniRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting SANI in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerSaniRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting SANI via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
