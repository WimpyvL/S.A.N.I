...
function resolveForegroundMs(cfg: SANIConfig): number {
...
function formatElevatedUnavailableMessage(params: {
  ...
  if (params.sessionKey) {
    lines.push(
      `See: ${formatCliCommand(`sani sandbox explain --session ${params.sessionKey}`)}`,
    );
  }
  return lines.join("\n");
}
...
export async function handleBashChatCommand(params: {
  ...
  if (params.cfg.commands?.bash !== true) {
    return {
      text: "⚠️ bash is disabled. Set commands.bash=true to enable. Docs: https://docs.sani.ai/tools/slash-commands#config",
    };
  }
