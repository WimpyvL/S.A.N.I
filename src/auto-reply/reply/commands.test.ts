...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "sani-commands-"));
...
function buildParams(commandBody: string, cfg: SANIConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
