...
import type { SANIConfig } from "../config/types.js";
import type {
...
export function isCommandEnabled(cfg: SANIConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: SANIConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as SANIConfig),
...
export function getCommandDetection(_cfg?: SANIConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: SANIConfig) {
