...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.SANI_STATE_DIR;
  previousConfigPath = process.env.SANI_CONFIG_PATH;
  previousSkipBrowserControl = process.env.SANI_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.SANI_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.SANI_SKIP_CANVAS_HOST;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "sani-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.SANI_STATE_DIR = path.join(tempHome, ".sani");
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "sani-test-"))
    : path.join(tempHome, ".sani-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.SANI_STATE_DIR = previousStateDir;
    }
    if (previousConfigPath === undefined) {
      delete process.env.SANI_CONFIG_PATH;
    } else {
      process.env.SANI_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.SANI_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.SANI_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.SANI_SKIP_GMAIL_WATCHER;
    } else {
      process.env.SANI_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.SANI_SKIP_CANVAS_HOST;
    } else {
      process.env.SANI_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.SANI_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.SANI_GATEWAY_TOKEN = fallbackToken;
  }
