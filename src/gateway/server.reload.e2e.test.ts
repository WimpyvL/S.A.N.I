...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.SANI_SKIP_CHANNELS;
    prevSkipGmail = process.env.SANI_SKIP_GMAIL_WATCHER;
    process.env.SANI_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.SANI_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.SANI_SKIP_GMAIL_WATCHER;
    } else {
      process.env.SANI_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
