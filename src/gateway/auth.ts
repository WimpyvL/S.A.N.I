...
export function resolveGatewayAuth(params: {
  ...
  const token =
    authConfig.token ?? env.SANI_GATEWAY_TOKEN ?? env.CLAWDBOT_GATEWAY_TOKEN ?? undefined;
  const password =
    authConfig.password ??
    env.SANI_GATEWAY_PASSWORD ??
...
export function assertGatewayAuthConfigured(auth: ResolvedGatewayAuth): void {
  if (auth.mode === "token" && !auth.token) {
    ...
    throw new Error(
      "gateway auth mode is token, but no token was configured (set gateway.auth.token or SANI_GATEWAY_TOKEN)",
    );
  }
