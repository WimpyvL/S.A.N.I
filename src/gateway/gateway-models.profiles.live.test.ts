...
import type { SANIConfig, ModelProviderConfig } from "../config/types.js";
import { resolveSANIAgentDir } from "../agents/agent-paths.js";
...
import { ensureSANIModelsJson } from "../agents/models-config.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.SANI_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.SANI_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.SANI_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<SANIConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.SANI_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.SANI_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveSANIAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "sani-live-state-"));
  process.env.SANI_STATE_DIR = tempStateDir;
  ...
  process.env.SANI_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.sani-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveSANIAgentDir();
  const sanitizedCfg: SANIConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "SANI live tool probe (local, safe): " +
    ...
    process.env.SANI_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.SANI_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureSANIModelsJson(cfg);
      ...
      const rawModels = process.env.SANI_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.sani-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
