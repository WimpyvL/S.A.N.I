...
import { ensureSANICliOnPath } from "../infra/path-env.js";
...
export async function startGatewayServer(
  ...
  process.env.SANI_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "SANI_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "SANI_RAW_STREAM_PATH",
  ...
  if (configSnapshot.legacyIssues.length > 0) {
    ...
    if (!migrated) {
      throw new Error(
        `Legacy config entries detected but auto-migration failed. Run "${formatCliCommand("sani doctor")}" to migrate.`,
      );
    }
  ...
  if (configSnapshot.exists && !configSnapshot.valid) {
    ...
    throw new Error(
      `Invalid config at ${configSnapshot.path}.\n${issues}\nRun "${formatCliCommand("sani doctor")}" to repair, then retry.`,
    );
  }
