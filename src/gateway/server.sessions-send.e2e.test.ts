...
import { createSANITools } from "../agents/sani-tools.js";
...
beforeAll(async () => {
  ...
  prevGatewayToken = process.env.SANI_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  process.env.SANI_GATEWAY_PORT = String(gatewayPort);
  process.env.SANI_GATEWAY_TOKEN = "test-token";
...
afterAll(async () => {
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.SANI_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createSANITools().find((candidate) => candidate.name === "sessions_send");
    if (!tool) {
