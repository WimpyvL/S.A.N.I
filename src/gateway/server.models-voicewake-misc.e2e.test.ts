...
describe("gateway server models + voicewake", () => {
  const setTempHome = (homeDir: string) => {
    ...
    process.env.SANI_STATE_DIR = path.join(homeDir, ".sani");
  ...
  test(
    ...
    { timeout: 60_000 },
    async () => {
      const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "sani-home-"));
      const restoreHome = setTempHome(homeDir);

      const initial = await rpcReq<{ triggers: string[] }>(ws, "voicewake.get");
      expect(initial.ok).toBe(true);
      expect(initial.payload?.triggers).toEqual(["sani", "claude", "computer"]);
      ...
      const onDisk = JSON.parse(
        await fs.readFile(path.join(homeDir, ".sani", "settings", "voicewake.json"), "utf8"),
...
describe("gateway server misc", () => {
  test("hello-ok advertises the gateway port for canvas host", async () => {
    ...
    const prevCanvasPort = process.env.SANI_CANVAS_HOST_PORT;
    process.env.SANI_GATEWAY_TOKEN = "secret";
    testTailnetIPv4.value = "100.64.0.1";
    testState.gatewayBind = "lan";
    const canvasPort = await getFreePort();
    testState.canvasHostPort = canvasPort;
    process.env.SANI_CANVAS_HOST_PORT = String(canvasPort);
    ...
    if (prevCanvasPort === undefined) {
      ...
      process.env.SANI_CANVAS_HOST_PORT = prevCanvasPort;
    }
  });
  ...
  test("auto-enables configured channel plugins on startup", async () => {
    const configPath = process.env.SANI_CONFIG_PATH;
    if (!configPath) {
      throw new Error("Missing SANI_CONFIG_PATH");
    }
