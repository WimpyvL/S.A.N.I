...
describe("gateway server chat", () => {
  test("handles chat send and history flows", async () => {
    ...
    try {
      ...
      const sendPolicyDir = await fs.mkdtemp(path.join(os.tmpdir(), "sani-gw-"));
      ...
      const agentBlockedDir = await fs.mkdtemp(path.join(os.tmpdir(), "sani-gw-"));
      ...
      const historyDir = await fs.mkdtemp(path.join(os.tmpdir(), "sani-gw-"));
  ...
  test("routes chat.send slash commands without agent runs", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "sani-gw-"));
    try {
