...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.SANI_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "sani-gw-mock-home-"));
      ...
      process.env.SANI_SKIP_BROWSER_CONTROL_SERVER = "1";

      const token = `test-${randomUUID()}`;
      process.env.SANI_GATEWAY_TOKEN = token;

      const workspaceDir = path.join(tempHome, "sani");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.sani-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".sani");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "sani.json");
      ...
      try {
        ...
        process.env.SANI_CONFIG_PATH = prev.configPath;
        process.env.SANI_GATEWAY_TOKEN = prev.token;
        process.env.SANI_SKIP_CHANNELS = prev.skipChannels;
        process.env.SANI_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.SANI_SKIP_CRON = prev.skipCron;
        process.env.SANI_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.SANI_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "sani-wizard-home-"));
    process.env.HOME = tempHome;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.SANI_STATE_DIR = prev.stateDir;
      process.env.SANI_CONFIG_PATH = prev.configPath;
      process.env.SANI_GATEWAY_TOKEN = prev.token;
      process.env.SANI_SKIP_CHANNELS = prev.skipChannels;
      process.env.SANI_SKIP_GMAIL_WATCHER = prev.skipGmail;
