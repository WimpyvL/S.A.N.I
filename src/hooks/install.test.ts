...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `sani-hook-install-${randomUUID()}`);
  ...
  it("rejects hook packs with reserved ids", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@evil/.",
        version: "0.0.1",
        sani: { hooks: ["./hooks/reserved-hook"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
