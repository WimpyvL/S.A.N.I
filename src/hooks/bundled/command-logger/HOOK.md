...
homepage: https://docs.sani.ai/hooks#command-logger
metadata:
  {
    "sani":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.sani/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.sani/logs/commands.log ~/.sani/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/sani`:
   ```
   /home/username/.sani/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
