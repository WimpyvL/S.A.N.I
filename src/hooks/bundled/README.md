...
This directory contains hooks that ship with SANI. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.sani.ai/hooks/soul-evil
...
Hooks can be configured in `~/.sani/sani.json`:
...
2. Restart gateway: `pkill -9 -f 'sani.*gateway' && pnpm sani gateway`
3. Enable the hook: `sani hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.sani.ai/hooks
