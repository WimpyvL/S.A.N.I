...
`;
    ...
    expect(parsed.sani.emoji).toBe("💾");
    expect(parsed.sani.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.sani.emoji).toBe("📝");
    expect(parsed.sani.events).toEqual(["command"]);
    expect(parsed.sani.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.sani.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"sani": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"sani": {"events": ["test"]}}');
  });
...
describe("resolveSANIMetadata", () => {
  it("extracts sani metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveSANIMetadata(frontmatter);
  ...
  it("returns undefined when sani key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        sani: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with SANI" },
            { id: "npm", kind: "npm", package: "@sani/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@sani/hook");
  });
    ...
    "sani":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with SANI" }],
      },
  }
---
...
`;
    ...
    const sani = resolveSANIMetadata(frontmatter);
    expect(sani).toBeDefined();
    expect(sani?.emoji).toBe("💾");
    expect(sani?.events).toEqual(["command:new"]);
    expect(sani?.requires?.config).toEqual(["workspace.dir"]);
    expect(sani?.install?.[0].kind).toBe("bundled");
  });
