...
async function makeTempDir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "sani-hooks-e2e-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("hooks install (e2e)", () => {
  ...
  beforeEach(async () => {
    ...
    prevStateDir = process.env.SANI_STATE_DIR;
    prevBundledDir = process.env.SANI_BUNDLED_HOOKS_DIR;
    process.env.SANI_STATE_DIR = path.join(baseDir, "state");
    process.env.SANI_BUNDLED_HOOKS_DIR = path.join(baseDir, "bundled-none");
    vi.resetModules();
  });
  ...
  afterEach(async () => {
    ...
    if (prevBundledDir === undefined) {
      delete process.env.SANI_BUNDLED_HOOKS_DIR;
    } else {
      process.env.SANI_BUNDLED_HOOKS_DIR = prevBundledDir;
    }
  ...
  it("installs a hook pack and triggers the handler", async () => {
    ...
    await fs.writeFile(
      ...
      [
        ...
        'metadata: {"sani":{"events":["command:new"]}}',
        "---",
        "",
        "# Hello Hook",
        "",
