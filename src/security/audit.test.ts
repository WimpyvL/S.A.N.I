...
describe("security audit", () => {
  ...
  it("treats Windows ACL-only perms as secure", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "sani-security-audit-win-"));
    const stateDir = path.join(tmp, "state");
    await fs.mkdir(stateDir, { recursive: true });
    const configPath = path.join(stateDir, "sani.json");
  ...
  it("flags Windows ACLs when Users can read the state dir", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "sani-security-audit-win-open-"));
    const stateDir = path.join(tmp, "state");
    await fs.mkdir(stateDir, { recursive: true });
    const configPath = path.join(stateDir, "sani.json");
    await fs.writeFile(configPath, "{}\n", "utf-8");
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: SANIConfig = { session: { dmScope: "main" } };
  ...
  it("flags Discord native commands without a guild user allowlist", async () => {
    const prevStateDir = process.env.SANI_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "sani-security-audit-discord-"));
    process.env.SANI_STATE_DIR = tmp;
    ...
    try {
      ...
      if (prevStateDir == null) {
        delete process.env.SANI_STATE_DIR;
      } else {
        process.env.SANI_STATE_DIR = prevStateDir;
      }
    }
  });
  ...
  it("does not flag Discord slash commands when dm.allowFrom includes a Discord snowflake id", async () => {
    ...
    const tmp = await fs.mkdtemp(
      path.join(os.tmpdir(), "sani-security-audit-discord-allowfrom-snowflake-"),
    );
    process.env.SANI_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
  ...
  it("flags Discord slash commands when access-group enforcement is disabled and no users allowlist exists", async () => {
    const prevStateDir = process.env.SANI_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "sani-security-audit-discord-open-"));
    process.env.SANI_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: SANIConfig = {
  ...
  it("flags Slack slash commands without a channel users allowlist", async () => {
    const prevStateDir = process.env.SANI_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "sani-security-audit-slack-"));
    process.env.SANI_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: SANIConfig = {
  ...
  it("flags Slack slash commands when access-group enforcement is disabled", async () => {
    const prevStateDir = process.env.SANI_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "sani-security-audit-slack-open-"));
    process.env.SANI_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: SANIConfig = {
  ...
  it("flags Telegram group commands without a sender allowlist", async () => {
    const prevStateDir = process.env.SANI_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "sani-security-audit-telegram-"));
    process.env.SANI_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: SANIConfig = {
  ...
  it("adds a warning when deep probe fails", async () => {
    const cfg: SANIConfig = { gateway: { mode: "local" } };
  ...
  it("warns when hooks token reuses the gateway env token", async () => {
    const prevToken = process.env.SANI_GATEWAY_TOKEN;
    process.env.SANI_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: SANIConfig = {
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.SANI_GATEWAY_TOKEN = prevToken;
      }
    }
  });
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      includeFilesystem: false,
      includeChannelSecurity: false,
      stateDir: "/Users/test/Dropbox/.sani",
      configPath: "/Users/test/Dropbox/.sani/sani.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "sani-security-audit-"));
    const stateDir = path.join(tmp, "state");
    ...
    try {
      const cfg: SANIConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "sani.json"),
      });
  ...
  describe("maybeProbeGateway auth selection", () => {
    const originalEnvToken = process.env.SANI_GATEWAY_TOKEN;
    const originalEnvPassword = process.env.SANI_GATEWAY_PASSWORD;

    beforeEach(() => {
      delete process.env.SANI_GATEWAY_TOKEN;
      delete process.env.SANI_GATEWAY_PASSWORD;
    });
    ...
    afterEach(() => {
      if (originalEnvToken == null) {
        ...
        process.env.SANI_GATEWAY_TOKEN = originalEnvToken;
      }
      if (originalEnvPassword == null) {
        delete process.env.SANI_GATEWAY_PASSWORD;
      } else {
        process.env.SANI_GATEWAY_PASSWORD = originalEnvPassword;
      }
    });
    ...
    it("prefers env token over local config token", async () => {
      process.env.SANI_GATEWAY_TOKEN = "env-token";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: SANIConfig = {
        gateway: {
          mode: "local",
    ...
    it("prefers env password over remote password", async () => {
      process.env.SANI_GATEWAY_PASSWORD = "env-pass";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: SANIConfig = {
        gateway: {
          mode: "remote",
