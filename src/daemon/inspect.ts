...
const EXTRA_MARKERS = ["sani", "clawdbot", "moltbot"] as const;
const execFileAsync = promisify(execFile);

export function renderGatewayServiceCleanupHints(
  env: Record<string, string | undefined> = process.env as Record<string, string | undefined>,
): string[] {
  const profile = env.SANI_PROFILE;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["sani_service_marker"];
  const kindKeys = ["sani_service_kind"];
...
function isSANIGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ai.sani.");
}

function isSANIGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (!name.startsWith("sani-gateway")) {
    return false;
  }
  return contents.toLowerCase().includes("gateway");
}

function isSANIGatewayTaskName(name: string): boolean {
  const normalized = name.trim().toLowerCase();
  if (!normalized) {
    return false;
  }
  const defaultName = resolveGatewayWindowsTaskName().toLowerCase();
  return normalized === defaultName || normalized.startsWith("sani gateway");
}
...
async function scanLaunchdDir(params: {
  ...
  for (const entry of entries) {
    ...
    if (marker === "sani" && isSANIGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "sani" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  ...
  for (const entry of entries) {
    ...
    if (marker === "sani" && isSANIGatewaySystemdService(name, contents)) {
      continue;
    }
    results.push({
      platform: "linux",
