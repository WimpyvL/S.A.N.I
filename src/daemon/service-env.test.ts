...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.SANI_GATEWAY_PORT).toBe("18789");
    expect(env.SANI_GATEWAY_TOKEN).toBe("secret");
    expect(env.SANI_SERVICE_MARKER).toBe("sani");
    expect(env.SANI_SERVICE_KIND).toBe("gateway");
    expect(typeof env.SANI_SERVICE_VERSION).toBe("string");
    expect(env.SANI_SYSTEMD_UNIT).toBe("sani-gateway.service");
    if (process.platform === "darwin") {
      expect(env.SANI_LAUNCHD_LABEL).toBe("ai.sani.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", SANI_PROFILE: "work" },
      port: 18789,
    });
    expect(env.SANI_SYSTEMD_UNIT).toBe("sani-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.SANI_LAUNCHD_LABEL).toBe("ai.sani.work");
    }
  });
});
