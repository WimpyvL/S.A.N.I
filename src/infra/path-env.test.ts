...
import { ensureSANICliOnPath } from "./path-env.js";

describe("ensureSANICliOnPath", () => {
  it("prepends the bundled app bin dir when a sibling sani exists", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "sani-path-"));
    try {
      const appBinDir = path.join(tmp, "AppBin");
      await fs.mkdir(appBinDir, { recursive: true });
      const cliPath = path.join(appBinDir, "sani");
      await fs.writeFile(cliPath, "#!/bin/sh\necho ok\n", "utf-8");
      await fs.chmod(cliPath, 0o755);

      const originalPath = process.env.PATH;
      const originalFlag = process.env.SANI_PATH_BOOTSTRAPPED;
      process.env.PATH = "/usr/bin";
      delete process.env.SANI_PATH_BOOTSTRAPPED;
      try {
        ensureSANICliOnPath({
          execPath: cliPath,
          cwd: tmp,
        ...
        if (originalFlag === undefined) {
          ...
          process.env.SANI_PATH_BOOTSTRAPPED = originalFlag;
        }
      }
    } finally {
  ...
  it("prepends mise shims when available", async () => {
    ...
    try {
      const appBinDir = path.join(tmp, "AppBin");
      await fs.mkdir(appBinDir, { recursive: true });
      const appCli = path.join(appBinDir, "sani");
      await fs.writeFile(appCli, "#!/bin/sh\necho ok\n", "utf-8");
      await fs.chmod(appCli, 0o755);

      const localBinDir = path.join(tmp, "node_modules", ".bin");
      await fs.mkdir(localBinDir, { recursive: true });
      const localCli = path.join(localBinDir, "sani");
