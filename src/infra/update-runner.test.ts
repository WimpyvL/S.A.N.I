...
describe("runGatewayUpdate", () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sani-update-"));
  });
  ...
  it("skips git update when worktree is dirty", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "sani", version: "1.0.0" }),
      "utf-8",
    );
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ name: "sani", version: "1.0.0", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
    ...
    const { runner, calls } = createRunner({
      ...
      "pnpm sani doctor --non-interactive": { stdout: "" },
    });
  ...
  it("skips update when no git root", async () => {
    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ name: "sani", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
  ...
  it("updates global npm installs when detected", async () => {
    ...
    const pkgRoot = path.join(nodeModules, "sani");
    ...
    const runCommand = async (argv: string[]) => {
      ...
      if (key === "npm i -g sani@latest") {
        await fs.writeFile(
          path.join(pkgRoot, "package.json"),
          JSON.stringify({ name: "sani", version: "2.0.0" }),
          "utf-8",
        );
    ...
    expect(calls.some((call) => call === "npm i -g sani@latest")).toBe(true);
  });
  ...
  it("uses update channel for global npm installs when tag is omitted", async () => {
    ...
    expect(result.before?.version).toBe("1.0.0");
    expect(result.after?.version).toBe("2.0.0");
    expect(calls.some((call) => call === "npm i -g sani@beta")).toBe(true);
  });

  it("cleans stale npm rename dirs before global update", async () => {
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "sani");
    const staleDir = path.join(nodeModules, ".sani-stale");
  ...
  it("updates global bun installs when detected", async () => {
    ...
    try {
      ...
      const pkgRoot = path.join(bunGlobalRoot, "sani");
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.writeFile(
        path.join(pkgRoot, "package.json"),
        JSON.stringify({ name: "sani", version: "1.0.0" }),
      ...
      expect(result.before?.version).toBe("1.0.0");
      expect(result.after?.version).toBe("2.0.0");
      expect(calls.some((call) => call === "bun add -g sani@latest")).toBe(true);
    } finally {
  ...
  it("rejects git roots that are not a sani checkout", async () => {
    await fs.mkdir(path.join(tempDir, ".git"));
    ...
    expect(result.status).toBe("error");
    expect(result.reason).toBe("not-sani-root");
