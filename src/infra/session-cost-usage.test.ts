...
describe("session cost usage", () => {
  it("aggregates daily totals with log cost and pricing fallback", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "sani-cost-"));
    ...
    const originalState = process.env.SANI_STATE_DIR;
    process.env.SANI_STATE_DIR = root;
    try {
      ...
      if (originalState === undefined) {
        ...
        process.env.SANI_STATE_DIR = originalState;
      }
    }
  });

  it("summarizes a single session file", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "sani-cost-session-"));
