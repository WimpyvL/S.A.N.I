...
export function triggerSaniRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.SANI_SYSTEMD_UNIT || process.env.SANI_SYSTEMD_UNIT,
        process.env.SANI_PROFILE || process.env.SANI_PROFILE,
      );
  ...
  const label =
    process.env.SANI_LAUNCHD_LABEL ||
    process.env.SANI_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.SANI_PROFILE || process.env.SANI_PROFILE);
