...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sani-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "sani doctor",
        managerScriptArgs(manager, "sani", ["doctor", "--non-interactive"]),
        gitRoot,
        { SANI_UPDATE_IN_PROGRESS: "1" },
      ),
    );
