...
function candidateBinDirs(opts: EnsureSaniPathOpts): string[] {
  ...
  // Bundled macOS app: `sani` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "sani");
    if (isExecutable(siblingCli)) {
      candidates.push(execDir);
    } else {
      const legacySiblingCli = path.join(execDir, "sani");
      if (isExecutable(legacySiblingCli)) {
        candidates.push(execDir);
      }
    }
  } catch {
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/sani` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "sani")) || isExecutable(path.join(localBinDir, "sani"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `sani` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureSaniCliOnPath(opts: EnsureSaniPathOpts = {}) {
  if (isTruthyEnvValue(process.env.SANI_PATH_BOOTSTRAPPED) || isTruthyEnvValue(process.env.SANI_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.SANI_PATH_BOOTSTRAPPED = "1";
...
/** @deprecated Use ensureSaniCliOnPath */
export const ensureSANICliOnPath = ensureSaniCliOnPath;
