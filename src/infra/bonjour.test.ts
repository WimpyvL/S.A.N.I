...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    process.env.SANI_MDNS_HOSTNAME = "test-host";
    process.env.SANI_MDNS_HOSTNAME = "test-host";
    ...
    expect(gatewayCall?.[0]?.type).toBe("sani-gw");
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("sani (SANI)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("sani");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("sani.local");
