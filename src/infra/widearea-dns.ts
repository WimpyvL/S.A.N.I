...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate =
    params?.configDomain ?? env.SANI_WIDE_AREA_DOMAIN ?? env.SANI_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*(?:sani|sani)-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "sani";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "sani");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "sani-gw");
  ...
  records.push(`_sani-gw._tcp IN PTR ${instanceLabel}._sani-gw._tcp`);
  records.push(`${instanceLabel}._sani-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._sani-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; sani-content-hash: ${contentHash}\n${contentBody}`;
}
