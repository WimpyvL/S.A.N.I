...
export async function autoMigrateLegacyStateDir(params: {
  ...
  if (env.SANI_STATE_DIR?.trim() || env.SANI_STATE_DIR?.trim()) {
    return { migrated: false, skipped: true, changes: [], warnings: [] };
  }
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        warnings.push(
          `Rollback failed; set SANI_STATE_DIR=${targetDir} to avoid split state: ${String(rollbackErr)}`,
        );
...
export async function autoMigrateLegacyState(params: {
  ...
  if (env.SANI_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim()) {
    return {
