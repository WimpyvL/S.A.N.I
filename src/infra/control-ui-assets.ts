...
import { resolveSANIPackageRoot, resolveSANIPackageRootSync } from "./sani-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveSANIPackageRoot({ argv1: normalized });
  if (!packageRoot) {
    return null;
  }
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveSANIPackageRootSync({
    argv1,
