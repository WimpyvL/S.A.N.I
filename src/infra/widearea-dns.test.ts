...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    const txt = renderWideAreaGatewayZoneText({
      domain: "sani.internal.",
      serial: 2025121701,
      gatewayPort: 18789,
      displayName: "Mac Studio (SANI)",
    ...
    expect(txt).toContain(`$ORIGIN sani.internal.`);
    ...
    expect(txt).toContain(`_sani-gw._tcp IN PTR studio-london._sani-gw._tcp`);
    expect(txt).toContain(`studio-london._sani-gw._tcp IN SRV 0 0 18789 studio-london`);
    expect(txt).toContain(`displayName=Mac Studio (SANI)`);
    expect(txt).toContain(`gatewayPort=18789`);
    expect(txt).toContain(`sshPort=22`);
    expect(txt).toContain(`cliPath=/opt/homebrew/bin/sani`);
  });
