...
function createProfileContext(
  ...
  const ensureBrowserAvailable = async (): Promise<void> => {
    ...
    if (isExtension) {
      ...
      throw new Error(
        `Chrome extension relay is running, but no tab is connected. Click the SANI Chrome extension icon on a tab to attach it (profile "${profile.name}").`,
      );
    }
    ...
    if (!httpReachable) {
      ...
      const launched = await launchSANIChrome(current.resolved, profile);
    ...
    if (!profileState.running) {
      throw new Error(
        `Port ${profile.cdpPort} is in use for profile "${profile.name}" but not by sani. ` +
    ...
    await stopSANIChrome(profileState.running);
    setProfileRunning(null);

    const relaunched = await launchSANIChrome(current.resolved, profile);
      ...
      if (profile.driver === "extension") {
        throw new Error(
          `tab not found (no attached Chrome tabs for profile "${profile.name}"). ` +
            "Click the SANI Browser Relay toolbar icon on the tab you want to control (badge ON).",
        );
      }
