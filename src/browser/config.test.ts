...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    const sani = resolveProfile(resolved, "sani");
    expect(sani?.driver).toBe("sani");
    expect(sani?.cdpPort).toBe(18800);
    expect(sani?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from SANI_GATEWAY_PORT when unset", () => {
    const prev = process.env.SANI_GATEWAY_PORT;
    process.env.SANI_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const sani = resolveProfile(resolved, "sani");
      expect(sani?.cdpPort).toBe(19012);
      expect(sani?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.SANI_GATEWAY_PORT;
      } else {
        process.env.SANI_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const sani = resolveProfile(resolved, "sani");
      expect(sani?.cdpPort).toBe(19022);
      expect(sani?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.SANI_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "sani");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        sani: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("sani");
  });
});
