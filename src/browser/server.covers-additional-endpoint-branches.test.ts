  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        defaultProfile: "sani",
        profiles: {
          sani: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchSANIChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
    launchCalls.push({ port: profile.cdpPort });
    reachable = true;
    return {
      pid: 123,
      exe: { kind: "chrome", path: "/fake/chrome" },
      userDataDir: "/tmp/sani",
      cdpPort: profile.cdpPort,
      startedAt: Date.now(),
      proc,
    };
  }),
  resolveSANIUserDataDir: vi.fn(() => "/tmp/sani"),
  stopSANIChrome: vi.fn(async () => {
    reachable = false;
  }),
}));
...
describe("browser control server", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.SANI_GATEWAY_PORT;
    process.env.SANI_GATEWAY_PORT = String(testPort - 2);
  ...
  afterEach(async () => {
    ...
    if (prevGatewayPort === undefined) {
      ...
      process.env.SANI_GATEWAY_PORT = prevGatewayPort;
    }
...
describe("backward compatibility (profile parameter)", () => {
  ...
  it("GET / without profile uses default profile", async () => {
    ...
    // Should use default profile (sani)
    expect(status.profile).toBe("sani");
  });
  ...
  it("POST /start without profile uses default profile", async () => {
    ...
    expect(result.profile).toBe("sani");
  });
  ...
  it("GET /profiles returns list of profiles", async () => {
    ...
    // Should at least have the default sani profile
    expect(result.profiles.some((p) => p.name === "sani")).toBe(true);
  });

  it("GET /tabs?profile=sani returns tabs for specified profile", async () => {
    const { startBrowserControlServerFromConfig } = await import("./server.js");
    await startBrowserControlServerFromConfig();
    const base = `http://127.0.0.1:${testPort}`;
    ...
    const result = (await realFetch(`${base}/tabs?profile=sani`).then((r) => r.json())) as {
      running: boolean;
      tabs: unknown[];
    };
    expect(result.running).toBe(true);
  ...
  it("POST /tabs/open?profile=sani opens tab in specified profile", async () => {
    const { startBrowserControlServerFromConfig } = await import("./server.js");
    await startBrowserControlServerFromConfig();
    const base = `http://127.0.0.1:${testPort}`;
    ...
    const result = (await realFetch(`${base}/tabs/open?profile=sani`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ url: "https://example.com" }),
    }).then((r) => r.json())) as { targetId?: string };
