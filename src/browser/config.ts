import type { BrowserConfig, BrowserProfileConfig, SANIConfig } from "../config/config.js";
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_SANI_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "sani" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_SANI_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_SANI_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an SANI browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for sani/sani CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: SANIConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_SANI_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "sani";
