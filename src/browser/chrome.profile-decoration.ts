...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".sani-profile-decorated");
}
 ...
 */
export function decorateSANIProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_SANI_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_SANI_BROWSER_COLOR).toUpperCase();
