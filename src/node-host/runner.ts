...
const execHostEnforced = process.env.SANI_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.SANI_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureSANICliOnPath({ pathEnv: process.env.PATH ?? "" });
