...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.SANI_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.SANI_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.SANI_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[sani] ${parsed.error}`);
    process.exit(2);
  }
