...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "SANI_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "sani-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.SANI_STATE_DIR = path.join(home, ".sani");
    ...
    await fs.mkdir(path.join(home, ".sani"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.sani${path.sep}media`);
