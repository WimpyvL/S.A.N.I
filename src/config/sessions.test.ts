...
describe("sessions", () => {
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-sani",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-sani#general");
  });
  ...
  it("updateLastRoute persists channel and target", async () => {
    ...
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "sani-sessions-"));
  ...
  it("derives session transcripts dir from SANI_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { SANI_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.SANI_STATE_DIR = "/custom/state";
    try {
