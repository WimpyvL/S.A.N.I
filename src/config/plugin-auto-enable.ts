import type { SANIConfig } from "./config.js";
...
function isTelegramConfigured(cfg: SANIConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: SANIConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: SANIConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: SANIConfig): boolean {
...
function isIMessageConfigured(cfg: SANIConfig): boolean {
...
function isWhatsAppConfigured(cfg: SANIConfig): boolean {
...
function isGenericChannelConfigured(cfg: SANIConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: SANIConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: SANIConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: SANIConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: SANIConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: SANIConfig, pluginId: string): SANIConfig {
...
function enablePluginEntry(cfg: SANIConfig, pluginId: string): SANIConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
