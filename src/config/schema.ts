...
import { SANISchema } from "./zod-schema.js";
...
export type ConfigSchema = ReturnType<typeof SANISchema.toJSONSchema>;
...
const FIELD_HELP: Record<string, string> = {
  "meta.lastTouchedVersion": "Auto-set when SANI writes the config.",
  ...
  "diagnostics.cacheTrace.filePath":
    "JSONL output path for cache trace logs (default: $SANI_STATE_DIR/logs/cache-trace.jsonl).",
  ...
  "agents.defaults.memorySearch.store.path":
    "SQLite index path (default: ~/.sani/memory/{agentId}.sqlite).",
  ...
  "memory.backend": 'Memory backend ("builtin" for SANI embeddings, "qmd" for QMD sidecar).',
  ...
  "plugins.installs":
    "CLI-managed install metadata (used by `sani plugins update` to locate install sources).",
...
const FIELD_PLACEHOLDERS: Record<string, string> = {
  ...
  "gateway.controlUi.basePath": "/sani",
  "gateway.controlUi.root": "dist/control-ui",
  "gateway.controlUi.allowedOrigins": "https://control.example.com",
  "channels.mattermost.baseUrl": "https://chat.example.com",
  "agents.list[].identity.avatar": "avatars/sani.png",
};
...
function buildBaseConfigSchema(): ConfigSchemaResponse {
  ...
  const schema = SANISchema.toJSONSchema({
    target: "draft-07",
    unrepresentable: "any",
  });
  schema.title = "SANIConfig";
