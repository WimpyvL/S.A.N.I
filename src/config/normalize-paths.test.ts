  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.sani/logs/sani.log" },
        hooks: {
          path: "~/.sani/hooks.json5",
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".sani", "logs", "sani.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".sani", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".sani", "telegram.token"),
      );
      ...
      expect(cfg.agents?.list?.[0]?.agentDir).toBe(path.join(home, ".sani", "agents", "main"));
