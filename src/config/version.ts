...
export function parseSANIVersion(raw: string | null | undefined): SANIVersion | null {
  if (!raw) {
    return null;
  }
...
export function compareSANIVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseSANIVersion(a);
  const parsedB = parseSANIVersion(b);
  if (!parsedA || !parsedB) {
    return null;
  }
