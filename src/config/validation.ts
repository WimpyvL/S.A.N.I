...
import type { SANIConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: SANIConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: SANIConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = SANISchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as SANIConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as SANIConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as SANIConfig)),
    ),
  };
}
