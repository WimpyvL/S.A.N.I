...
import type { SANIConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
import { compareSANIVersions } from "./version.js";
...
function coerceConfig(value: unknown): SANIConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as SANIConfig;
}
...
function stampConfigVersion(cfg: SANIConfig): SANIConfig {
...
function warnIfConfigFromFuture(cfg: SANIConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareSANIVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer SANI (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: SANIConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): SANIConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as SANIConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as SANIConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: SANIConfig) {
    clearConfigCache();
...
// module scope. `SANI_CONFIG_PATH` (and friends) are expected to work even
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.SANI_DISABLE_CONFIG_CACHE?.trim() || env.SANI_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): SANIConfig {
...
export async function writeConfigFile(cfg: SANIConfig): Promise<void> {
  clearConfigCache();
