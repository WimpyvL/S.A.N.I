...
describe("buildWorkspaceSkillsPrompt", () => {
  it("syncs merged skills into a target workspace", async () => {
    const sourceWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "sani-"));
    const targetWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "sani-"));
  ...
  it("filters skills based on env/config gates", async () => {
    ...
    try {
      await writeSkill({
        ...
        metadata:
          '{"sani":{"requires":{"env":["GEMINI_API_KEY"]},"primaryEnv":"GEMINI_API_KEY"}}',
        body: "# Nano Banana\n",
      });
