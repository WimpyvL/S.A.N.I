...
describe("session_status tool", () => {
  it("returns a status card for the current session", async () => {
    ...
    const tool = createSANITools({ agentSessionKey: "main" }).find(
    ...
    expect(details.statusText).toContain("SANI");
  ...
  it("blocks cross-agent session_status without agent-to-agent access", async () => {
    ...
    const tool = createSANITools({ agentSessionKey: "agent:main:main" }).find(
      (candidate) => candidate.name === "session_status",
    );
    expect(tool).toBeDefined();
    if (!tool) {
  ...
  it("scopes bare session keys to the requester agent", async () => {
    ...
    const tool = createSANITools({ agentSessionKey: "agent:support:main" }).find(
      (candidate) => candidate.name === "session_status",
    );
    expect(tool).toBeDefined();
    if (!tool) {
