...
const defaultTools = createSANICodingTools();

describe("createSANICodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sani-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createSANICodingTools();
    ...
    try {
      ...
      const contents = "Hello from sani read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "sani-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "sani-workspace"),
    ...
    const tools = createSANICodingTools({ sandbox });
