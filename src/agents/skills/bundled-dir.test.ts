...
describe("resolveBundledSkillsDir", () => {
  const originalOverride = process.env.SANI_BUNDLED_SKILLS_DIR;

  afterEach(() => {
    if (originalOverride === undefined) {
      delete process.env.SANI_BUNDLED_SKILLS_DIR;
    } else {
      process.env.SANI_BUNDLED_SKILLS_DIR = originalOverride;
    }
  });
  ...
  it("resolves bundled skills under a flattened dist layout", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "sani-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "sani" }));
