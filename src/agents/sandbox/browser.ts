...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    color: DEFAULT_SANI_BROWSER_COLOR,
    executablePath: undefined,
    headless: params.headless,
    noSandbox: false,
    attachOnly: true,
    defaultProfile: DEFAULT_SANI_BROWSER_PROFILE_NAME,
    profiles: {
      [DEFAULT_SANI_BROWSER_PROFILE_NAME]: {
...
export async function ensureSandboxBrowser(params: {
  ...
  if (!state.exists) {
    ...
    args.push("-e", `SANI_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
    args.push("-e", `SANI_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`);
    args.push("-e", `SANI_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    args.push("-e", `SANI_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `SANI_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
  ...
  const existingProfile = existing
    ? resolveProfile(existing.bridge.state.resolved, DEFAULT_SANI_BROWSER_PROFILE_NAME)
    : null;
  const shouldReuse =
