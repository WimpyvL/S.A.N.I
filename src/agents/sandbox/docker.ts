...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "sani.sandbox=1");
  args.push("--label", `sani.sessionKey=${params.scopeKey}`);
  args.push("--label", `sani.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `sani.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("sani.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`sani sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`sani sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("sani sandbox recreate --all");
}
