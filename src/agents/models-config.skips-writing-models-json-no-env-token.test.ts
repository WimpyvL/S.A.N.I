...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "sani-models-" });
}

const MODELS_CONFIG: SaniConfig = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        vi.resetModules();
        const { ensureSANIModelsJson } = await import("./models-config.js");

        const agentDir = path.join(home, "agent-empty");
        const result = await ensureSANIModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      vi.resetModules();
      const { ensureSANIModelsJson } = await import("./models-config.js");
      const { resolveSANIAgentDir } = await import("./agent-paths.js");

      await ensureSANIModelsJson(MODELS_CONFIG);

      const modelPath = path.join(resolveSANIAgentDir(), "models.json");
