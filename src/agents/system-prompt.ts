...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; SANI handles all routing internally.",
...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    "## Documentation",
    `SANI docs: ${docsPath}`,
    "Mirror: https://docs.sani.ai",
    "Source: https://github.com/sani/sani",
    "Community: https://discord.com/invite/clawd",
    "Find new skills: https://clawhub.com",
    "For SANI behavior, commands, config, or architecture: consult local docs first.",
    "When diagnosing issues, run `sani status` yourself when possible; only ask the user if you lack access (e.g., sandboxed).",
    "",
  ];
}
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running SANI process",
  ...
  if (promptMode === "none") {
    return "You are a personal assistant running inside SANI.";
  }

  const lines = [
    "You are a personal assistant running inside SANI.",
    "",
    "## Tooling",
    ...
    toolLines.length > 0
      ...
      : [
        ...
        "- browser: control SANI's dedicated browser",
        "- canvas: present/eval/snapshot the Canvas",
        "- nodes: list/describe/notify/camera/screen on paired nodes",
        "- cron: manage cron jobs and wake events (use for reminders; when scheduling a reminder, write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context in reminder text if appropriate)",
    ...
    "## SANI CLI Quick Reference",
    "SANI is controlled via subcommands. Do not invent commands.",
    "To manage the Gateway daemon service (start/stop/restart):",
    "- sani gateway status",
    "- sani gateway start",
    "- sani gateway stop",
    "- sani gateway restart",
    "If unsure, ask the user to run `sani help` (or `sani gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
    // Skip self-update for subagent/none modes
    hasGateway && !isMinimal ? "## SANI Self-Update" : "",
    hasGateway && !isMinimal
      ? [
        "Get Updates (self-update) is ONLY allowed when the user explicitly asks for it.",
        "Do not run config.apply or update.run unless the user explicitly requests an update or config change; if it's not explicit, ask first.",
        "Actions: config.get, config.schema, config.apply (validate + write full config, then restart), update.run (update deps or git, then restart).",
        "After restart, SANI pings the last active session automatically.",
      ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
    ...
    "These user-editable files are loaded by SANI and included below in Project Context.",
    "",
  ...
  if (!isMinimal) {
    lines.push(
      ...
      "HEARTBEAT_OK",
      'SANI treats a leading/trailing "HEARTBEAT_OK" as a heartbeat ack (and may discard it).',
