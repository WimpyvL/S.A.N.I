...
import { __testing, createSANICodingTools } from "./pi-tools.js";
...
describe("createSANICodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createSANICodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sani-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "sani-outside.txt");
