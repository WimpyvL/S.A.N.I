...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): SaniConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
describe("runEmbeddedPiAgent auth profile rotation", () => {
  it("rotates for auto-pinned profiles", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "sani-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "sani-workspace-"));
    try {
