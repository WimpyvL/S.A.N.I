...
type ModelsConfig = NonNullable<SaniConfig["models"]>;
...
export async function resolveImplicitCopilotProvider(params: {
  ...
  // Our Copilot auth lives in SANI's auth-profiles store instead, so we also
  // write a runtime-only auth.json entry for pi-coding-agent to pick up.
  //
  // This is safe because it's (1) within SANI's agent dir, (2) contains the
  // GitHub token (not the exchanged Copilot token), and (3) matches existing
  // patterns for OAuth-like providers in pi-coding-agent.
  // Note: we deliberately do not write pi-coding-agent's `auth.json` here.
  // SANI uses its own auth store and exchanges tokens at runtime.
  // `models list` uses SANI's auth heuristics for availability.
