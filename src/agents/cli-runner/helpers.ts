...
/**
 * Cleanup suspended SANI CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: SANIConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sani-cli-images-"));
  const paths: string[] = [];
