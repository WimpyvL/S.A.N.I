...
describe("getApiKeyForModel", () => {
  it("migrates legacy oauth.json into auth-profiles.json", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sani-oauth-"));

    try {
      process.env.SANI_STATE_DIR = tempDir;
      process.env.SANI_AGENT_DIR = path.join(tempDir, "agent");
      ...
      const store = ensureAuthProfileStore(process.env.SANI_AGENT_DIR, {
  ...
  it("suggests openai-codex when only Codex OAuth is configured", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sani-auth-"));
