...
type WebSearchConfig = NonNullable<SaniConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: SaniConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("sani configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.sani.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://sani.ai",
      "X-Title": "SANI Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [
