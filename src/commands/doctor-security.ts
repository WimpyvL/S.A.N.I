...
import type { SANIConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: SANIConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("sani security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("sani configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("sani config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("sani doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("sani config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
