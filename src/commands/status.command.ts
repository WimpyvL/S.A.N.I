...
export async function statusCommand(
  ...
  runtime.log(theme.heading("SANI status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("sani security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("sani security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://loop69.co.za/sani/faq");
  runtime.log("Troubleshooting: https://loop69.co.za/sani/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("sani status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("sani logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("sani status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("sani gateway probe")}`);
  }
}
