...
import { readConfigFileSnapshot, type SANIConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: SANIConfig, model: string): SANIConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("sani plugins install")}\`.`,
    );
  }
