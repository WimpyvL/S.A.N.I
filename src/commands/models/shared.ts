...
export async function updateConfig(
  mutator: (cfg: SANIConfig) => SANIConfig,
): Promise<SANIConfig> {
...
export function resolveModelTarget(params: { raw: string; cfg: SANIConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: SANIConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("sani agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
