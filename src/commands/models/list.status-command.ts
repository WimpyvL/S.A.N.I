...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveSANIAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("sani models auth setup-token")}\` or \`${formatCliCommand("sani configure")}\`.`
          : `Run \`${formatCliCommand("sani configure")}\` or set an API key env var.`;
