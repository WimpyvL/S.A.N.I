...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveSANIAgentDir: vi.fn().mockReturnValue("/tmp/sani-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/sani-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/sani-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveSANIAgentDir: mocks.resolveSANIAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveSANIAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/sani-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/sani-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/sani-agent-custom");
