...
import { resolveSaniPackageRoot as resolveSANIPackageRoot } from "../infra/sani-root.js";
...
function resolveMode(cfg: SANIConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  const root = await resolveSANIPackageRoot({
  ...
  let cfg: SANIConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("sani configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("sani config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("sani setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("sani doctor --fix")}" to apply changes.`);
  }
