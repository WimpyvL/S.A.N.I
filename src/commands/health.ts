...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.SANI_DEBUG_HEALTH)) {
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: SANIConfig },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.SANI_DEBUG_HEALTH);
    if (opts.verbose) {
