...
describe("normalizeLegacyConfigValues", () => {
  ...
  beforeEach(() => {
    previousOauthDir = process.env.SANI_OAUTH_DIR;
    tempOauthDir = fs.mkdtempSync(path.join(os.tmpdir(), "sani-oauth-"));
    process.env.SANI_OAUTH_DIR = tempOauthDir;
  });
  ...
  afterEach(() => {
    if (previousOauthDir === undefined) {
      ...
      process.env.SANI_OAUTH_DIR = previousOauthDir;
    }
    if (tempOauthDir) {
  ...
  it("copies legacy ack reaction when authDir override exists", () => {
    const customDir = fs.mkdtempSync(path.join(os.tmpdir(), "sani-wa-auth-"));
    try {
