...
const SEARCH_TOOL = "https://docs.sani.ai/mcp.SearchSANI";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.sani.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('sani docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.sani.ai/");
      runtime.log(`Search: ${formatCliCommand('sani docs "your query"')}`);
    }
    return;
  }
