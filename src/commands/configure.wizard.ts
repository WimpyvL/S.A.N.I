    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from sani.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "SANI update wizard" : "SANI configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: SANIConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid && snapshot.issues.length > 0) {
        note(
          [
            ...
            "",
            "Docs: https://docs.sani.ai/gateway/configuration",
          ].join("\n"),
          "Config issues",
        );
      }
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("sani doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: baseConfig.gateway?.auth?.token ?? process.env.SANI_GATEWAY_TOKEN,
      password: baseConfig.gateway?.auth?.password ?? process.env.SANI_GATEWAY_PASSWORD,
    });
    ...
    if (opts.sections) {
      ...
      if (selected.includes("health")) {
        ...
        const token = nextConfig.gateway?.auth?.token ?? process.env.SANI_GATEWAY_TOKEN;
        const password =
          nextConfig.gateway?.auth?.password ?? process.env.SANI_GATEWAY_PASSWORD;
    ...
    const newPassword = nextConfig.gateway?.auth?.password ?? process.env.SANI_GATEWAY_PASSWORD;
    const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.SANI_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.SANI_GATEWAY_TOKEN;
