...
function stripUnknownConfigKeys(config: SANIConfig): {
  config: SANIConfig;
  removed: string[];
} {
  const parsed = SANISchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: SANIConfig) {
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".sani");
  const targetPath = path.join(targetDir, "sani.json");
  try {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("sani doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("sani doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "sani doctor --fix" to remove these keys.');
    }
  }
