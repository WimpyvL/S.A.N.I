...
beforeEach(() => {
  ...
  resolveSANIPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.SANI_STATE_DIR;
  originalUpdateInProgress = process.env.SANI_UPDATE_IN_PROGRESS;
  process.env.SANI_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "sani-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.SANI_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.SANI_UPDATE_IN_PROGRESS;
  } else {
    process.env.SANI_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveSANIPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadSANIPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/sani-root.js", () => ({
  resolveSANIPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "sani-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.SANI_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when SANI_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/sani.json",
      exists: true,
      raw: "{}",
    ...
    process.env.SANI_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
