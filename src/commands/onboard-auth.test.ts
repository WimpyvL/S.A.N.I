...
const requireAgentDir = () => {
  const agentDir = process.env.SANI_AGENT_DIR;
  if (!agentDir) {
    throw new Error("SANI_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.SANI_STATE_DIR;
  const previousAgentDir = process.env.SANI_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.SANI_AGENT_DIR;
    } else {
      process.env.SANI_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.SANI_OAUTH_DIR;
  });

  it("writes auth-profiles.json under SANI_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "sani-oauth-"));
    process.env.SANI_STATE_DIR = tempStateDir;
    process.env.SANI_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.SANI_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to SANI_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "sani-minimax-"));
    process.env.SANI_STATE_DIR = tempStateDir;
    process.env.SANI_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.SANI_AGENT_DIR;
