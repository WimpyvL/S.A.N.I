import type { SANIConfig, ConfigFileSnapshot } from "../../config/types.js";
...
export function resolveTargets(cfg: SANIConfig, explicitUrl?: string): GatewayStatusTarget[] {
...
export function resolveAuthForTarget(
  ...
  const envToken = process.env.SANI_GATEWAY_TOKEN?.trim() || "";
  const envPassword = process.env.SANI_GATEWAY_PASSWORD?.trim() || "";
  const cfgToken =
...
export function buildNetworkHints(cfg: SANIConfig) {
