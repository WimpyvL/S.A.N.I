...
function resolveSandboxDockerImage(cfg: SANIConfig): string {
  const image = cfg.agents?.defaults?.sandbox?.docker?.image?.trim();
  return image ? image : DEFAULT_SANDBOX_IMAGE;
}

function resolveSandboxBrowserImage(cfg: SANIConfig): string {
  const image = cfg.agents?.defaults?.sandbox?.browser?.image?.trim();
  return image ? image : DEFAULT_SANDBOX_BROWSER_IMAGE;
}

function updateSandboxDockerImage(cfg: SANIConfig, image: string): SANIConfig {
  return {
    ...cfg,
    agents: {
      ...cfg.agents,
...
function updateSandboxBrowserImage(cfg: SANIConfig, image: string): SANIConfig {
  return {
    ...cfg,
    agents: {
      ...cfg.agents,
...
export function noteSandboxScopeWarnings(cfg: SANIConfig) {
