...
function findOtherStateDirs(stateDir: string): string[] {
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      const candidates = [".sani"].map((dir) => path.resolve(root, entry.name, dir));
...
export async function noteStateIntegrity(
  ...
  const defaultStateDir = path.join(homedir(), ".sani");
...
export function noteWorkspaceBackupTip(workspaceDir: string) {
  ...
  note(
    [
      ...
      "- Keep ~/.sani out of git; it contains credentials and session history.",
