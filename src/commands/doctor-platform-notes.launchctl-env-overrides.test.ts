...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "SANI_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("SANI_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv SANI_GATEWAY_TOKEN");
    expect(message).not.toContain("SANI_GATEWAY_PASSWORD");
  });
