...
beforeAll(() => {
  previousProfile = process.env.SANI_PROFILE;
  process.env.SANI_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.SANI_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/sani-root.js", () => ({
  resolveSANIPackageRoot: vi.fn().mockResolvedValue("/tmp/sani"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/sani/pnpm-lock.yaml",
      markerPath: "/tmp/sani/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("SANI status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("sani status --all") ||
          l.includes("sani --profile isolated status --all") ||
          l.includes("sani status --all") ||
          l.includes("sani --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.SANI_GATEWAY_TOKEN = "abcd1234";
    try {
