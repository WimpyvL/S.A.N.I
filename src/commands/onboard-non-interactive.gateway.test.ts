...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    password: process.env.SANI_GATEWAY_PASSWORD,
  };
  ...
  const initStateDir = async (prefix: string) => {
    ...
    process.env.SANI_STATE_DIR = stateDir;
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "sani-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.SANI_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "sani");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.SANI_STATE_DIR = stateDir;
    process.env.SANI_CONFIG_PATH = path.join(stateDir, "sani.json");
