...
export async function maybeRepairGatewayDaemon(params: {
    ...
    if (params.cfg.gateway?.mode !== "remote") {
      ...
      if (install) {
        ...
        const { programArguments, workingDirectory, environment } = await buildGatewayInstallPlan({
          env: process.env,
          port,
          token: params.cfg.gateway?.auth?.token ?? process.env.SANI_GATEWAY_TOKEN,
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.SANI_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("sani gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
