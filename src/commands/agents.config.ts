...
export function listAgentEntries(cfg: SANIConfig): AgentEntry[] {
...
function resolveAgentName(cfg: SANIConfig, agentId: string) {
...
function resolveAgentModel(cfg: SANIConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: SANIConfig): AgentSummary[] {
