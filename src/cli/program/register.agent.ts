...
${formatHelpExamples([
          ['sani agent --to +15555550123 --message "status update"', "Start a new session."],
          ['sani agent --agent ops --message "Summarize logs"', "Use a specific agent."],
          [
            'sani agent --session-id 1234 --message "Summarize inbox" --thinking medium',
            "Target a session with explicit thinking level.",
          ],
          [
            'sani agent --to +15555550123 --message "Trace logs" --verbose on --json',
            "Enable verbose logging and JSON output.",
          ],
          ['sani agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
          [
            'sani agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
            "Send reply to a different channel/target.",
          ],
        ])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "loop69.co.za/sani/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "loop69.co.za/sani/cli/agents")}\n`,
    );
...
${formatHelpExamples([
          ...
          [
            "sani agents set-identity --identity-file ~/.sani/workspace/IDENTITY.md --agent main",
