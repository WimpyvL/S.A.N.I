...
const SANDBOX_EXAMPLES = {
  main: [
    ["sani sandbox list", "List all sandbox containers."],
    ["sani sandbox list --browser", "List only browser containers."],
    ["sani sandbox recreate --all", "Recreate all containers."],
    ["sani sandbox recreate --session main", "Recreate a specific session."],
    ["sani sandbox recreate --agent mybot", "Recreate agent containers."],
    ["sani sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["sani sandbox list", "List all sandbox containers."],
    ["sani sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["sani sandbox recreate --all", "Recreate all containers."],
    ["sani sandbox recreate --session main", "Recreate a specific session."],
    ["sani sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["sani sandbox recreate --browser --all", "Recreate only browser containers."],
    ["sani sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["sani sandbox explain", "Show effective sandbox config."],
    ["sani sandbox explain --session agent:main:main", "Explain a specific session."],
    ["sani sandbox explain --agent work", "Explain an agent sandbox."],
    ["sani sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.sani.ai/cli/sandbox")}\n`,
    )
    .action(() => {
