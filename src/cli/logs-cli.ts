...
function createLogWriters() {
  const writer = createSafeStreamWriter({
    ...
    onBrokenPipe: (err, stream) => {
      ...
      const message = `sani logs: output ${target} closed (${code}). Stopping tail.`;
      try {
...
function emitGatewayError(
  ...
  const hint = `Hint: run \`${formatCliCommand("sani doctor")}\`.`;
...
export function registerLogsCli(program: Command) {
  const logs = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/logs", "docs.sani.ai/cli/logs")}\n`,
    );
