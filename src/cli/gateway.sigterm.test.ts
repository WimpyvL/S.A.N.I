...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "sani-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...
      SANI_SKIP_GMAIL_WATCHER: "1",
      SANI_SKIP_CRON: "1",
      SANI_SKIP_BROWSER_CONTROL_SERVER: "1",
      SANI_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "sani-entry-bootstrap.mjs");
