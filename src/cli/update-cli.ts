...
import { resolveSaniPackageRoot } from "../infra/sani-root.js";
...
const STEP_LABELS: Record<string, string> = {
  ...
  "sani doctor": "Running doctor checks",
...
const UPDATE_QUIPS = [
  ...
  "The lobster has molted. Harder shell.",
...
const DEFAULT_PACKAGE_NAME = "sani";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME, "sani"]);
const CLI_NAME = resolveCliName();
const SANI_REPO_URL = "https://github.com/sani/sani.git"; // Keep URL until renamed
const DEFAULT_GIT_DIR = path.join(os.homedir(), ".sani");
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("sani@")) {
    return trimmed.slice("sani@".length);
  }
  if (trimmed.startsWith("sani@")) {
    return trimmed.slice("sani@".length);
  }
...
async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "sani.mjs");
  if (!(await pathExists(binPath))) {
    const legacyBinPath = path.join(root, "sani.mjs");
    if (!(await pathExists(legacyBinPath))) {
      return;
    }
  }
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("sani completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
function resolveGitInstallDir(): string {
  const override =
    process.env.SANI_GIT_DIR?.trim() || process.env.SANI_GIT_DIR?.trim();
  if (override) {
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", SANI_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `SANI_GIT_DIR points at a non-git directory: ${params.dir}. Set SANI_GIT_DIR to an empty folder or a sani checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`SANI_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  defaultRuntime.log(theme.heading("Sani update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating Sani..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this SANI install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("sani doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("sani gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g sani@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g sani@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.SANI_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.SANI_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("sani gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("sani doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("sani gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("sani gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `sani update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `SANI_GIT_DIR points at a non-git directory: ${gitDir}. Set SANI_GIT_DIR to an empty folder or a sani checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via SANI_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || !ok) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update SANI to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["sani update", "Update a source checkout (git)"],
        ["sani update --channel beta", "Switch to beta channel (git + npm)"],
        ["sani update --channel dev", "Switch to dev channel (git + npm)"],
        ["sani update --tag beta", "One-off update to a dist-tag or version"],
        ["sani update --no-restart", "Update without restarting the service"],
        ["sani update --json", "Output result as JSON"],
        ["sani update --yes", "Non-interactive (accept downgrade prompts)"],
        ["sani update wizard", "Interactive update wizard"],
        ["sani --update", "Shorthand for sani update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run sani update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "loop69.co.za/sani/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["sani update status", "Show channel + version status."],
          ["sani update status --json", "JSON output."],
          ["sani update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "loop69.co.za/sani/cli/update")}`,
    )
    .action(async (opts) => {
      try {
