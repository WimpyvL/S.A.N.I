...
function applySlotSelectionForPlugin(
  config: SANIConfig,
  pluginId: string,
): { config: SANIConfig; warnings: string[] } {
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    .command("plugins")
    .description("Manage SANI plugins/extensions")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.sani.ai/cli/plugins")}\n`,
    );
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.sani.ai/plugin");
      lines.push("");
