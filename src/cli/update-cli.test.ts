...
describe("update-cli", () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    const { resolveSANIPackageRoot } = await import("../infra/sani-root.js");
    const { readConfigFileSnapshot } = await import("../config/config.js");
    const { checkUpdateStatus, fetchNpmTagVersion, resolveNpmChannelTag } =
      await import("../infra/update-check.js");
    const { runCommandWithTimeout } = await import("../process/exec.js");
    vi.mocked(resolveSANIPackageRoot).mockResolvedValue(process.cwd());
  ...
  it("defaults to stable channel for package installs when unset", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sani-update-"));
    try {
      await fs.writeFile(
      ...
      vi.mocked(resolveSANIPackageRoot).mockResolvedValue(tempDir);
      vi.mocked(checkUpdateStatus).mockResolvedValue({
        root: tempDir,
        installKind: "package",
        packageManager: "npm",
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sani-update-wizard-"));
    const previousGitDir = process.env.SANI_GIT_DIR;
    try {
      setTty(true);
      process.env.SANI_GIT_DIR = tempDir;
      ...
      process.env.SANI_GIT_DIR = previousGitDir;
      await fs.rm(tempDir, { recursive: true, force: true });
    }
  });
});
