...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("sani node install"), formatCliCommand("sani node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("sani node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    SANI_LOG_PREFIX: baseEnv.SANI_LOG_PREFIX ?? "node",
