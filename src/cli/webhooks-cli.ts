...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "loop69.co.za/sani/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + SANI hooks")
    ...
    .option("--hook-url <url>", "SANI hook URL")
    .option("--hook-token <token>", "SANI hook token")
