...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    SANI_STATE_DIR: process.env.SANI_STATE_DIR,
    SANI_CONFIG_PATH: process.env.SANI_CONFIG_PATH,
    SANI_GATEWAY_PORT: process.env.SANI_GATEWAY_PORT,
    SANI_PROFILE: process.env.SANI_PROFILE,
  };

  beforeEach(() => {
    process.env.SANI_STATE_DIR = "/tmp/sani-cli-state";
    process.env.SANI_CONFIG_PATH = "/tmp/sani-cli-state/sani.json";
    delete process.env.SANI_GATEWAY_PORT;
    delete process.env.SANI_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.SANI_STATE_DIR !== undefined) {
      process.env.SANI_STATE_DIR = originalEnv.SANI_STATE_DIR;
    } else {
      delete process.env.SANI_STATE_DIR;
    }

    if (originalEnv.SANI_CONFIG_PATH !== undefined) {
      process.env.SANI_CONFIG_PATH = originalEnv.SANI_CONFIG_PATH;
    } else {
      delete process.env.SANI_CONFIG_PATH;
    }

    if (originalEnv.SANI_GATEWAY_PORT !== undefined) {
      process.env.SANI_GATEWAY_PORT = originalEnv.SANI_GATEWAY_PORT;
    } else {
      delete process.env.SANI_GATEWAY_PORT;
    }

    if (originalEnv.SANI_PROFILE !== undefined) {
      process.env.SANI_PROFILE = originalEnv.SANI_PROFILE;
    } else {
      delete process.env.SANI_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        SANI_PROFILE: "dev",
        SANI_STATE_DIR: "/tmp/sani-daemon-state",
        SANI_CONFIG_PATH: "/tmp/sani-daemon-state/sani.json",
        SANI_GATEWAY_PORT: "19001",
      },
