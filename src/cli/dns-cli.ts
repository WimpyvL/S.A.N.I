...
export function registerDnsCli(program: Command) {
  const dns = program
    ...
    .addHelpText(
      "after",
      () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/dns", "docs.sani.ai/cli/dns")}\n`,
    );
  ...
  dns
    ...
    .option("--domain <domain>", "Wide-area discovery domain (e.g. sani.internal)")
    .option(
      "--apply",
    ...
    .action(async (opts) => {
      ...
      defaultRuntime.log("");
      defaultRuntime.log(theme.heading("Recommended ~/.sani/sani.json:"));
      defaultRuntime.log(
        JSON.stringify(
          {
            gateway: { bind: "auto" },
      ...
      if (zoneFileNeedsBootstrap(zonePath)) {
        ...
        const zoneLines = [
          `; created by sani dns setup (will be overwritten by the gateway when wide-area discovery is enabled)`,
      ...
      if (cfg.discovery?.wideArea?.enabled !== true) {
        ...
        defaultRuntime.log(
          theme.muted(
            "Note: enable discovery.wideArea.enabled in ~/.sani/sani.json on the gateway and restart the gateway so it writes the DNS-SD zone.",
          ),
        );
      }
    });
