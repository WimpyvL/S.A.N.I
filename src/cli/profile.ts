...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.sani${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.SANI_PROFILE = profile;

  const stateDir = env.SANI_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.SANI_STATE_DIR?.trim()) {
    env.SANI_STATE_DIR = stateDir;
  }

  if (!env.SANI_CONFIG_PATH?.trim()) {
    env.SANI_CONFIG_PATH = path.join(stateDir, "sani.json");
  }

  if (profile === "dev" && !env.SANI_GATEWAY_PORT?.trim()) {
    env.SANI_GATEWAY_PORT = "19001";
  }
}
