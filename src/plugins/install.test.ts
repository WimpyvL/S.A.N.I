...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `sani-plugin-install-${randomUUID()}`);
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.sani/extensions and uses unscoped id", async () => {
  ...
  it("rejects packages without sani.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@sani/nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    expect(result.error).toContain("sani.extensions");
  });
});
