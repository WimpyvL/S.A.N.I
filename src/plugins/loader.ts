...
import { discoverSANIPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: SANIPluginDefinition;
  register?: SANIPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as SANIPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as SANIPluginDefinition;
...
export function loadSANIPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverSANIPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "sani/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: SANIPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as SANIPluginModule;
    } catch (err) {
