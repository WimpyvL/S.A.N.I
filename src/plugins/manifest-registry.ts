...
import { discoverSANIPlugins, type PluginCandidate } from "./discovery.js";
...
function resolveManifestCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.SANI_PLUGIN_MANIFEST_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseManifestCache(env: NodeJS.ProcessEnv): boolean {
  const disabled = env.SANI_DISABLE_PLUGIN_MANIFEST_CACHE?.trim();
  if (disabled) {
    return false;
  }
