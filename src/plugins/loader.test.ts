...
import { loadSANIPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.SANI_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `sani-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "sani.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.SANI_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadSANIPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.SANI_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadSANIPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadSANIPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
