import type { SaniApp } from "./app.ts";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as SaniApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as SaniApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as SaniApp);
  }
  if (host.tab === "agents") {
    const app = host as unknown as SaniApp;
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as SaniApp);
    await loadDevices(host as unknown as SaniApp);
    await loadConfig(host as unknown as SaniApp);
    await loadExecApprovals(host as unknown as SaniApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as SaniApp);
    await loadConfig(host as unknown as SaniApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as SaniApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as SaniApp, { reset: true });
...
export function inferBasePath() {
  ...
  const configured = window.__SANI_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as SaniApp, false),
    loadPresence(host as unknown as SaniApp),
    loadSessions(host as unknown as SaniApp),
    loadCronStatus(host as unknown as SaniApp),
    loadDebug(host as unknown as SaniApp),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as SaniApp, true),
    loadConfigSchema(host as unknown as SaniApp),
    loadConfig(host as unknown as SaniApp),
  ]);
}
...
export async function loadCron(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as SaniApp, false),
    loadCronStatus(host as unknown as SaniApp),
    loadCronJobs(host as unknown as SaniApp),
  ]);
}
