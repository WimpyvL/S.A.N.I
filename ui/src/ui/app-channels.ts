...
export async function handleWhatsAppStart(host: SaniApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: SaniApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: SaniApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: SaniApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: SaniApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: SaniApp): string {
...
export function handleNostrProfileCancel(host: SaniApp) {
...
export function handleNostrProfileToggleAdvanced(host: SaniApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: SaniApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: SaniApp) {
  const state = host.nostrProfileFormState;
