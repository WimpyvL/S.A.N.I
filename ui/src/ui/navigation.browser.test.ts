...
const originalConnect = SaniApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("sani-app") as unknown as SaniApp;
...
beforeEach(() => {
  SaniApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__SANI_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  SaniApp.prototype.connect = originalConnect;
  window.__SANI_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/sani/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/sani");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/sani/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__SANI_CONTROL_UI_BASE_PATH__ = "/sani";
    const app = mountApp("/sani/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/sani");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/sani/sessions");
  });
