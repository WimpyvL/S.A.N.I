    ...
    <div class="shell ${isChat ? "shell--chat" : ""} ${chatFocus ? "shell--chat-focus" : ""} ${state.settings.navCollapsed ? "shell--nav-collapsed" : ""} ${state.onboarding ? "shell--onboarding" : ""}">
        ...
        <div class="nav-group nav-group--links">
          ...
          <div class="nav-group__items">
            <a
              ...
              href="https://docs.loop69.co.za/sani"
              target="_blank"
              rel="noreferrer"
      ...
      ? renderOverview({
        ...
        onSessionKeyChange: (next) => {
          ...
          state.chatMessage = "";
          (state as unknown as SaniApp).resetToolStream();
          state.applySettings({
            ...state.settings,
            sessionKey: next,
      ...
      ? renderChat({
        ...
        onSessionKeyChange: (next) => {
          ...
          (state as unknown as SaniApp).chatStreamStartedAt = null;
          state.chatQueue = [];
          (state as unknown as SaniApp).resetToolStream();
          (state as unknown as SaniApp).resetChatScroll();
          state.applySettings({
            ...state.settings,
            sessionKey: next,
            lastActiveSessionKey: next,
        ...
        onChatScroll: (event) => (state as unknown as SaniApp).handleChatScroll(event),
        onDraftChange: (next) => (state.chatMessage = next),
        attachments: state.chatAttachments,
        onAttachmentsChange: (next) => (state.chatAttachments = next),
        onSend: () => (state as unknown as SaniApp).handleSendChat(),
        canAbort: Boolean(state.chatRunId),
        onAbort: () => void (state as unknown as SaniApp).handleAbortChat(),
        onQueueRemove: (id) => (state as unknown as SaniApp).removeQueuedMessage(id),
        onNewSession: () =>
          (state as unknown as SaniApp).handleSendChat("/new", { restoreDraft: true }),
        showNewMessages: state.chatNewMessagesBelow,
        onScrollToBottom: () => state.scrollToBottom(),
        // Sidebar props for tool output viewing
        sidebarOpen: (state as unknown as SaniApp).sidebarOpen,
        sidebarContent: (state as unknown as SaniApp).sidebarContent,
        sidebarError: (state as unknown as SaniApp).sidebarError,
        splitRatio: (state as unknown as SaniApp).splitRatio,
        onOpenSidebar: (content: string) =>
          (state as unknown as SaniApp).handleOpenSidebar(content),
        onCloseSidebar: () => (state as unknown as SaniApp).handleCloseSidebar(),
        onSplitRatioChange: (ratio: number) =>
          (state as unknown as SaniApp).handleSplitRatioChange(ratio),
      ...
      ? renderConfig({
        ...
        searchQuery: (state as unknown as SaniApp).configSearchQuery,
        activeSection: (state as unknown as SaniApp).configActiveSection,
        activeSubsection: (state as unknown as SaniApp).configActiveSubsection,
        onRawChange: (next) => {
          state.configRaw = next;
        },
        onFormModeChange: (mode) => (state.configFormMode = mode),
        onFormPatch: (path, value) =>
          updateConfigFormValue(state as unknown as SaniApp, path, value),
        onSearchChange: (query) =>
          ((state as unknown as SaniApp).configSearchQuery = query),
        onSectionChange: (section) => {
          (state as unknown as SaniApp).configActiveSection = section;
          (state as unknown as SaniApp).configActiveSubsection = null;
        },
        onSubsectionChange: (section) =>
          ((state as unknown as SaniApp).configActiveSubsection = section),
        onReload: () => loadConfig(state as unknown as SaniApp),
        onSave: () => saveConfig(state as unknown as SaniApp),
        onApply: () => applyConfig(state as unknown as SaniApp),
        onUpdate: () => runUpdate(state as unknown as SaniApp),
      })
      : nothing
    }
      ...
      ? renderLogs({
        ...
        onExport: (lines, label) =>
          (state as unknown as SaniApp).exportLogs(lines, label),
        onScroll: (event) => (state as unknown as SaniApp).handleLogsScroll(event),
      })
      : nothing
    }
      </main>
