---
description: Update Sani from upstream when branch has diverged (ahead/behind)
---

# Sani Upstream Sync Workflow
...
# Move old version
mv /Applications/Sani.app /tmp/Sani-backup.app

# Install new build
cp -R dist/Sani.app /Applications/
...
ps aux | grep -E "(sani|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm sani agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
pnpm sani agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"
