...
- Feishu: add Feishu/Lark plugin support + docs. (#7313) Thanks @jiulingyun (sani-cn).
...
- Docs: finish renaming the QMD memory docs to reference the SANI state dir.
...
- Skills: update session-logs paths from .clawdbot to .sani. (#4502)
...
- Build: align npm tar override and bin metadata so the `sani` CLI entrypoint is preserved in npm publishes.
- Docs: add pi/pi-dev docs and update SANI branding + install links.
...
- Rebrand: rename the npm package/CLI to `sani`, add a `sani` compatibility shim, and move extensions to the `@sani/*` scope.
...
- macOS: finish SANI app rename for macOS sources, bundle identifiers, and shared kit paths. (#2844) Thanks @fal3.
...
- Docs: update exe.dev install instructions. (#https://github.com/sani/sani/pull/3047) Thanks @zackerthescar.
...
- Skills: update session-logs paths to use ~/.sani. (#4502) Thanks @bonald.
...
- Providers: Ollama discovery + docs; Venice guide upgrades + cross-links. (#1606) Thanks @abhaymundhara. https://docs.sani.ai/providers/ollama https://docs.sani.ai/providers/venice
- Channels: LINE plugin (Messaging API) with rich replies + quick replies. (#1630) Thanks @plum-dawg.
- TTS: Edge fallback (keyless) + `/tts` auto modes. (#1668, #1667) Thanks @steipete, @sebslight. https://docs.sani.ai/tts
- Exec approvals: approve in-chat via `/approve` across all channels (including plugins). (#1621) Thanks @czekaj. https://docs.sani.ai/tools/exec-approvals https://docs.sani.ai/tools/slash-commands
- Telegram: DM topics as separate sessions + outbound link preview toggle. (#1597, #1700) Thanks @rohannagpal, @zerone0x. https://docs.sani.ai/channels/telegram

### Changes

- Channels: add LINE plugin (Messaging API) with rich replies, quick replies, and plugin HTTP registry. (#1630) Thanks @plum-dawg.
- TTS: add Edge TTS provider fallback, defaulting to keyless Edge with MP3 retry on format failures. (#1668) Thanks @steipete. https://docs.sani.ai/tts
- TTS: add auto mode enum (off/always/inbound/tagged) with per-session `/tts` override. (#1667) Thanks @sebslight. https://docs.sani.ai/tts
- Telegram: treat DM topics as separate sessions and keep DM history limits stable with thread suffixes. (#1597) Thanks @rohannagpal.
- Telegram: add `channels.telegram.linkPreview` to toggle outbound link previews. (#1700) Thanks @zerone0x. https://docs.sani.ai/channels/telegram
- Web search: add Brave freshness filter parameter for time-scoped results. (#1688) Thanks @JonUleis. https://docs.sani.ai/tools/web
- UI: refresh Control UI dashboard design system (colors, icons, typography). (#1745, #1786) Thanks @EnzeD, @mousberg.
- Exec approvals: forward approval prompts to chat with `/approve` for all channels (including plugins). (#1621) Thanks @czekaj. https://docs.sani.ai/tools/exec-approvals https://docs.sani.ai/tools/slash-commands
- Gateway: expose config.patch in the gateway tool with safe partial updates + restart sentinel. (#1653) Thanks @Glucksberg.
- Diagnostics: add diagnostic flags for targeted debug logs (config + env override). https://docs.sani.ai/diagnostics/flags
- Docs: expand FAQ (migration, scheduling, concurrency, model recommendations, OpenAI subscription auth, Pi sizing, hackable install, docs SSL workaround).
- Docs: add verbose installer troubleshooting guidance.
- Docs: add macOS VM guide with local/hosted options + VPS/nodes guidance. (#1693) Thanks @f-trycua.
- Docs: add Bedrock EC2 instance role setup + IAM steps. (#1625) Thanks @sergical. https://docs.sani.ai/bedrock
...
- BlueBubbles: route phone-number targets to DMs, avoid leaking routing IDs, and auto-create missing DMs (Private API required). (#1751) Thanks @tyler6204. https://docs.sani.ai/channels/bluebubbles
...
- Signal: add configurable signal-cli startup timeout + external daemon mode docs. (#1677) https://docs.sani.ai/channels/signal
...
- TTS: move Telegram TTS into core + enable model-driven TTS tags by default for expressive audio replies. (#1559) Thanks @Glucksberg. https://docs.sani.ai/tts
- Gateway: add `/tools/invoke` HTTP endpoint for direct tool calls (auth + tool policy enforced). (#1575) Thanks @vignesh07. https://docs.sani.ai/gateway/tools-invoke-http-api
- Heartbeat: per-channel visibility controls (OK/alerts/indicator). (#1452) Thanks @dlauer. https://docs.sani.ai/gateway/heartbeat
- Deploy: add Fly.io deployment support + guide. (#1570) https://docs.sani.ai/platforms/fly
- Channels: add Tlon/Urbit channel plugin (DMs, group mentions, thread replies). (#1544) Thanks @wca4a. https://docs.sani.ai/channels/tlon

### Changes

- Channels: allow per-group tool allow/deny policies across built-in + plugin channels. (#1546) Thanks @adam91holt. https://docs.sani.ai/multi-agent-sandbox-tools
- Agents: add Bedrock auto-discovery defaults + config overrides. (#1553) Thanks @fal3. https://docs.sani.ai/bedrock
- CLI: add `sani system` for system events + heartbeat controls; remove standalone `wake`. (commit 71203829d) https://docs.sani.ai/cli/system
- CLI: add live auth probes to `sani models status` for per-profile verification. (commit 40181afde) https://docs.sani.ai/cli/models
- CLI: restart the gateway by default after `sani update`; add `--no-restart` to skip it. (commit 2c85b1b40)
- Browser: add node-host proxy auto-routing for remote gateways (configurable per gateway/node). (commit c3cb26f7c)
- Plugins: add optional `llm-task` JSON-only tool for workflows. (#1498) Thanks @vignesh07. https://docs.sani.ai/tools/llm-task
...
- Docs: add cron vs heartbeat decision guide (with Lobster workflow notes). (#1533) Thanks @JustYannicc. https://docs.sani.ai/automation/cron-vs-heartbeat
- Docs: clarify HEARTBEAT.md empty file skips heartbeats, missing file still runs. (#1535) Thanks @JustYannicc. https://docs.sani.ai/gateway/heartbeat
...
- CLI: auth probe output polish (table output, inline errors, reduced noise, and wrap fixes in `sani models status`). (commit da3f2b489, commit 00ae21bed, commit 31e59cd58, commit f7dc27f2d, commit 438e782f8, commit 886752217, commit aabe0bed3, commit 81535d512, commit c63144ab1)
...
- Control UI: ignore bootstrap identity placeholder text for avatar values and fall back to the default avatar. https://docs.sani.ai/cli/agents https://docs.sani.ai/web/control-ui
...
- Highlight: Lobster optional plugin tool for typed workflows + approval gates. https://docs.sani.ai/tools/lobster
- Lobster: allow workflow file args via `argsJson` in the plugin tool. https://docs.sani.ai/tools/lobster
...
- CLI: flatten node service commands under `sani node` and remove `service node` docs.
- CLI: move gateway service commands under `sani gateway` and add `gateway probe` for reachability.
- Sessions: add per-channel reset overrides via `session.resetByChannel`. (#1353) Thanks @cash-echo-bot.
- Agents: add identity avatar config support and Control UI avatar rendering. (#1329, #1424) Thanks @dlauer.
- UI: show per-session assistant identity in the Control UI. (#1420) Thanks @robbyczgw-cla.
- CLI: add `sani update wizard` for interactive channel selection and restart prompts. https://docs.sani.ai/cli/update
- Signal: add typing indicators and DM read receipts via signal-cli.
- MSTeams: add file uploads, adaptive cards, and attachment handling improvements. (#1410) Thanks @Evizero.
- Onboarding: remove the run setup-token auth option (paste setup-token or reuse CLI creds instead).
- Docs: add troubleshooting entry for gateway.mode blocking gateway start. https://docs.sani.ai/gateway/troubleshooting
- Docs: add /model allowlist troubleshooting note. (#1405)
- Docs: add per-message Gmail search example for gog. (#1220) Thanks @mbelinky.

### Breaking

- **BREAKING:** Control UI now rejects insecure HTTP without device identity by default. Use HTTPS (Tailscale Serve) or set `gateway.controlUi.allowInsecureAuth: true` to allow token-only auth. https://docs.sani.ai/web/control-ui#insecure-http
...
- Control UI: add copy-as-markdown with error feedback. (#1345) https://docs.sani.ai/web/control-ui
- Control UI: drop the legacy list view. (#1345) https://docs.sani.ai/web/control-ui
- TUI: add syntax highlighting for code blocks. (#1200) https://docs.sani.ai/tui
- TUI: session picker shows derived titles, fuzzy search, relative times, and last message preview. (#1271) https://docs.sani.ai/tui
- TUI: add a searchable model picker for quicker model selection. (#1198) https://docs.sani.ai/tui
- TUI: add input history (up/down) for submitted messages. (#1348) https://docs.sani.ai/tui
- ACP: add `sani acp` for IDE integrations. https://docs.sani.ai/cli/acp
- ACP: add `sani acp client` interactive harness for debugging. https://docs.sani.ai/cli/acp
- Skills: add download installs with OS-filtered options. https://docs.sani.ai/tools/skills
- Skills: add the local sherpa-onnx-tts skill. https://docs.sani.ai/tools/skills
- Memory: add hybrid BM25 + vector search (FTS5) with weighted merging and fallback. https://docs.sani.ai/concepts/memory
- Memory: add SQLite embedding cache to speed up reindexing and frequent updates. https://docs.sani.ai/concepts/memory
- Memory: add OpenAI batch indexing for embeddings when configured. https://docs.sani.ai/concepts/memory
- Memory: enable OpenAI batch indexing by default for OpenAI embeddings. https://docs.sani.ai/concepts/memory
- Memory: allow parallel OpenAI batch indexing jobs (default concurrency: 2). https://docs.sani.ai/concepts/memory
- Memory: render progress immediately, color batch statuses in verbose logs, and poll OpenAI batch status every 2s by default. https://docs.sani.ai/concepts/memory
- Memory: add `--verbose` logging for memory status + batch indexing details. https://docs.sani.ai/concepts/memory
- Memory: add native Gemini embeddings provider for memory search. (#1151) https://docs.sani.ai/concepts/memory
- Browser: allow config defaults for efficient snapshots in the tool/CLI. (#1336) https://docs.sani.ai/tools/browser
- Nostr: add the Nostr channel plugin with profile management + onboarding defaults. (#1323) https://docs.sani.ai/channels/nostr
- Matrix: migrate to matrix-bot-sdk with E2EE support, location handling, and group allowlist upgrades. (#1298) https://docs.sani.ai/channels/matrix
- Slack: add HTTP webhook mode via Bolt HTTP receiver. (#1143) https://docs.sani.ai/channels/slack
- Telegram: enrich forwarded-message context with normalized origin details + legacy fallback. (#1090) https://docs.sani.ai/channels/telegram
- Discord: fall back to `/skill` when native command limits are exceeded. (#1287)
- Discord: expose `/skill` globally. (#1287)
- Zalouser: add channel dock metadata, config schema, setup wiring, probe, and status issues. (#1219) https://docs.sani.ai/plugins/zalouser
- Plugins: require manifest-embedded config schemas with preflight validation warnings. (#1272) https://docs.sani.ai/plugins/manifest
- Plugins: move channel catalog metadata into plugin manifests. (#1290) https://docs.sani.ai/plugins/manifest
- Plugins: align Nextcloud Talk policy helpers with core patterns. (#1290) https://docs.sani.ai/plugins/manifest
- Plugins/UI: let channel plugin metadata drive UI labels/icons and cron channel options. (#1306) https://docs.sani.ai/web/control-ui
- Agents/UI: add agent avatar support in identity config, IDENTITY.md, and the Control UI. (#1329) https://docs.sani.ai/gateway/configuration
- Plugins: add plugin slots with a dedicated memory slot selector. https://docs.sani.ai/plugins/agent-tools
- Plugins: ship the bundled BlueBubbles channel plugin (disabled by default). https://docs.sani.ai/channels/bluebubbles
- Plugins: migrate bundled messaging extensions to the plugin SDK and resolve plugin-sdk imports in the loader.
- Plugins: migrate the Zalo plugin to the shared plugin SDK runtime. https://docs.sani.ai/channels/zalo
- Plugins: migrate the Zalo Personal plugin to the shared plugin SDK runtime. https://docs.sani.ai/plugins/zalouser
- Plugins: allow optional agent tools with explicit allowlists and add the plugin tool authoring guide. https://docs.sani.ai/plugins/agent-tools
- Plugins: auto-enable bundled channel/provider plugins when configuration is present.
- Plugins: sync plugin sources on channel switches and update npm-installed plugins during `sani update`.
- Plugins: share npm plugin update logic between `sani update` and `sani plugins update`.
...
- Usage: add `/usage cost` summaries and macOS menu cost charts. https://docs.sani.ai/reference/api-usage-costs
- Security: warn when <=300B models run without sandboxing while web tools are enabled. https://docs.sani.ai/cli/security
- Exec: add host/security/ask routing for gateway + node exec. https://docs.sani.ai/tools/exec
- Exec: add `/exec` directive for per-session exec defaults (host/security/ask/node). https://docs.sani.ai/tools/exec
- Exec approvals: migrate approvals to `~/.sani/exec-approvals.json` with per-agent allowlists + skill auto-allow toggle, and add approvals UI + node exec lifecycle events. https://docs.sani.ai/tools/exec-approvals
- Nodes: add headless node host (`sani node start`) for `system.run`/`system.which`. https://docs.sani.ai/cli/node
- Nodes: add node daemon service install/status/start/stop/restart. https://docs.sani.ai/cli/node
- Bridge: add `skills.bins` RPC to support node host auto-allow skill bins.
- Sessions: add daily reset policy with per-type overrides and idle windows (default 4am local), preserving legacy idle-only configs. (#1146) https://docs.sani.ai/concepts/session
- Sessions: allow `sessions_spawn` to override thinking level for sub-agent runs. https://docs.sani.ai/tools/subagents
- Channels: unify thread/topic allowlist matching + command/mention gating helpers across core providers. https://docs.sani.ai/concepts/groups
- Models: add Qwen Portal OAuth provider support. (#1120) https://docs.sani.ai/providers/qwen
- Onboarding: add allowlist prompts and username-to-id resolution across core and extension channels. https://docs.sani.ai/start/onboarding
- Docs: clarify allowlist input types and onboarding behavior for messaging channels. https://docs.sani.ai/start/onboarding
- Docs: refresh Android node discovery docs for the Gateway WS service type. https://docs.sani.ai/platforms/android
- Docs: surface Amazon Bedrock in provider lists and clarify Bedrock auth env vars. (#1289) https://docs.sani.ai/bedrock
- Docs: clarify WhatsApp voice notes. https://docs.sani.ai/channels/whatsapp
- Docs: clarify Windows WSL portproxy LAN access notes. https://docs.sani.ai/platforms/windows
- Docs: refresh bird skill install metadata and usage notes. (#1302) https://docs.sani.ai/tools/browser-login
...
- Build: use tsgo for dev/watch builds by default (opt out with `SANI_TS_COMPILER=tsc`).
...
- **BREAKING:** Reject invalid/unknown config entries and refuse to start the gateway for safety. Run `sani doctor --fix` to repair, then update plugins (`sani plugins update`) if you use any.
...
- CLI: keep `sani logs` output resilient to broken pipes while preserving progress output.
...
- Memory: show total file counts + scan issues in `sani memory status`.
...
- Memory: probe sqlite-vec availability in `sani memory status`.
...
- Hooks: add hooks system with bundled hooks, CLI tooling, and docs. (#1028) — thanks @ThomsenDrake. https://docs.sani.ai/hooks
- Media: add inbound media understanding (image/audio/video) with provider + CLI fallbacks. https://docs.sani.ai/nodes/media-understanding
- Plugins: add Zalo Personal plugin (`@sani/zalouser`) and unify channel directory for plugins. (#1032) — thanks @suminhthanh. https://docs.sani.ai/plugins/zalouser
- Models: add Vercel AI Gateway auth choice + onboarding updates. (#1016) — thanks @timolins. https://docs.sani.ai/providers/vercel-ai-gateway
- Sessions: add `session.identityLinks` for cross-platform DM session li nking. (#1033) — thanks @thewilloftheshadow. https://docs.sani.ai/concepts/session
- Web search: add `country`/`language` parameters (schema + Brave API) and docs. (#1046) — thanks @YuriNachos. https://docs.sani.ai/tools/web

### Breaking

- **BREAKING:** `sani message` and message tool now require `target` (dropping `to`/`channelId` for destinations). (#1034) — thanks @tobalsan.
- **BREAKING:** Channel auth now prefers config over env for Discord/Telegram/Matrix (env is fallback only). (#1040) — thanks @thewilloftheshadow.
- **BREAKING:** Drop legacy `chatType: "room"` support; use `chatType: "channel"`.
- **BREAKING:** remove legacy provider-specific target resolution fallbacks; target resolution is centralized with plugin hints + directory lookups.
- **BREAKING:** `sani hooks` is now `sani webhooks`; hooks live under `sani hooks`. https://docs.sani.ai/cli/webhooks
- **BREAKING:** `sani plugins install <path>` now copies into `~/.sani/extensions` (use `--link` to keep path-based loading).
...
- Tools: add `exec` PTY support for interactive sessions. https://docs.sani.ai/tools/exec
...
- Directory: unify `sani directory` across channels and plugin channels.
...
- Hooks: add hook pack installs (npm/path/zip/tar) with `sani.hooks` manifests and `sani hooks install/update`.
...
- Security: default-deny slash/control commands unless a channel computed `CommandAuthorized` (fixes accidental “open” behavior), and ensure WhatsApp + Zalo plugin channels gate inline `/…` tokens correctly. https://docs.sani.ai/gateway/security
- Security: redact sensitive text in gateway WS logs.
- Tools: cap pending `exec` process output to avoid unbounded buffers.
- CLI: speed up `sani sandbox-explain` by avoiding heavy plugin imports when normalizing channel ids.
...
- Plugins: add provider auth registry + `sani models auth login` for plugin-driven OAuth/API key flows.
...
- **BREAKING:** Microsoft Teams is now a plugin; install `@sani/msteams` via `sani plugins install @sani/msteams`.
- **BREAKING:** Channel auth now prefers config over env for Discord/Telegram/Matrix (env is fallback only). (#1040) — thanks @thewilloftheshadow.

### Changes

- UI/Apps: move channel/config settings to schema-driven forms and rename Connections → Channels. (#1040) — thanks @thewilloftheshadow.
- CLI: set process titles to `sani-<command>` for clearer process listings.
...
- Security: warn on weak model tiers (Haiku, below GPT-5, below Claude 4.5) in `sani security audit`.
...
- Auth/Status: keep auth profiles sticky per session (rotate on compaction/new), surface provider usage headers in `/status` and `sani models status`, and update docs.
- CLI: add `--json` output for `sani daemon` lifecycle/install commands.
...
- Fix: make `sani update` auto-update global installs when installed via a package manager.
- Fix: list model picker entries as provider/model pairs for explicit selection. (#970) — thanks @mcinteerj.
- Fix: align OpenAI image-gen defaults with DALL-E 3 standard quality and document output formats. (#880) — thanks @mkbehr.
- Fix: persist `gateway.mode=local` after selecting Local run mode in `sani configure`, even if no other sections are chosen.
...
- Security: expanded `sani security audit` (+ `--fix`), detect-secrets CI scan, and a `SECURITY.md` reporting policy.
...
- Security: expand `sani security audit` checks (model hygiene, config includes, plugin allowlists, exposure matrix) and extend `--fix` to tighten more sensitive state paths.
...
- Browser: add Chrome extension relay takeover mode (toolbar button), plus `sani browser extension install/path` and remote browser control (standalone server + token auth).
...
- Gateway/Dev: ensure `pnpm gateway:dev` always uses the dev profile config + state (`~/.sani-dev`).
...
- Memory: new `sani memory` CLI plus `memory_search`/`memory_get` tools with snippets + line ranges; index stored under `~/.sani/memory/{agentId}.sqlite` with watch-on-by-default.
...
- Gateway/CLI: add Tailscale binary discovery, custom bind mode, and probe auth retry; add `sani dashboard` auto-open flow; default native slash commands to `"auto"` with per-provider overrides. (#740) — thanks @jeffersonwarrior.
...
- Install: run `sani doctor --non-interactive` after git installs/updates and nudge daemon restarts when detected.
...
- Gateway/CLI: honor `CLAWDBOT_LAUNCHD_LABEL` / `CLAWDBOT_SYSTEMD_UNIT` overrides; `agents.list` respects explicit config; reduce noisy loopback WS logs during tests; run `sani doctor --non-interactive` during updates. (#781) — thanks @ronyrus.
...
- Plugins: add `sani plugins install` (path/tgz/npm), plus `list|info|enable|disable|doctor` UX.
...
- macOS: prompt to install the global `sani` CLI when missing in local mode; install via `sani.ai/install-cli.sh` (no onboarding) and use external launchd/CLI instead of the embedded gateway runtime.
...
- Installer tests: add root+non-root docker smokes, CI workflow to fetch sani.ai scripts and run install sh/cli with onboarding skipped.
...
- Plugins: treat `plugins.load.paths` directory entries as package roots when they contain `package.json` + `sani.extensions`; load plugin packages from config dirs; extract archives without system tar.
...
- CLI: `sani status` now table-based + shows OS/update/gateway/daemon/agents/sessions; `status --all` adds a full read-only debug report (tables, log tails, Tailscale summary, and scan progress via OSC-9 + spinner).
...
- CLI: add `sani update` (safe-ish git checkout update) + `--update` shorthand. (#673) — thanks @fm1randa.
...
- CLI: add `sani reset` and `sani uninstall` flows (interactive + non-interactive) plus docker cleanup smoke test.
...
- CLI/Gateway: clarify that `sani gateway status` reports RPC health (connect + RPC) and shows RPC failures separately from connect failures.
...
- Docs: make `sani status` the first diagnostic step, clarify `status --deep` behavior, and document `/whoami` + `/id`.
...
- Sandbox: add `sani sandbox explain` (effective policy inspector + fix-it keys); improve “sandbox jail” tool-policy/elevated errors with actionable config key paths; link to docs.
...
- CLI: `sani sessions` now includes `elev:*` + `usage:*` flags in the table output.
- CLI/Pairing: accept positional provider for `pairing list|approve` (npm-run compatible); update docs/bot hints.
- Branding: normalize legacy casing/branding to “SANI” (CLI, status, docs).
- Auto-reply: fix native `/model` not updating the actual chat session (Telegram/Slack/Discord). (#646)
- Doctor: offer to run `sani update` first on git installs (keeps doctor output aligned with latest).
- Doctor: avoid false legacy workspace warning when install dir is `~/sani`. (#660)
...
- CLI: `sani message` now subcommands (`message send|poll|...`) and requires `--provider` unless only one provider configured.
...
- Models/Auth: setup-token + token auth profiles; `sani models auth order {get,set,clear}`; per-agent auth candidates in `/model status`; OAuth expiry checks in doctor/status.
...
- Config/Onboarding: inline env vars; OpenAI API key flow to shared `~/.sani/.env`; Opus 4.5 default prompt for Anthropic auth; QuickStart auto-install gateway (Node-only) + provider picker tweaks + skip-systemd flags; TUI bootstrap prompt (`tui --message`); remove Bun runtime choice.
...
- **SECURITY (update ASAP):** inbound DMs are now **locked down by default** on Telegram/WhatsApp/Signal/iMessage/Discord/Slack.
  ...
  - Approve requests via `sani pairing list <provider>` + `sani pairing approve <provider> <code>`.
...
- **Gateway/CLI UX:** `sani logs`, cron list colors/aliases, docs search, agents list/add/delete flows, status usage snapshots, runtime/auth source display, and `/status`/commands auth unification.
...
- Env: load global `$SANI_STATE_DIR/.env` (`~/.sani/.env`) as a fallback after CWD `.env`.
...
- CLI: run `sani agent` via the Gateway by default; use `--local` to force embedded mode.
