...
import type { SANIConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("sani-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("sani-bin-sherpa-");
      const modelDir = await makeTempDir("sani-sherpa-model-");
      ...
      const cfg: SANIConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("sani-bin-whispercpp-");
      const modelDir = await makeTempDir("sani-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("sani-bin-gemini-");
      ...
      const cfg: SANIConfig = { tools: { media: { image: {} } } };
