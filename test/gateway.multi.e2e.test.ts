...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `sani-e2e-${name}-`));
  const configDir = path.join(homeDir, ".sani");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          SANI_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        SANI_GATEWAY_TOKEN: inst.gatewayToken,
        SANI_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          SANI_GATEWAY_PORT: String(gwA.port),
          SANI_GATEWAY_TOKEN: gwA.gatewayToken,
          SANI_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          SANI_GATEWAY_PORT: String(gwB.port),
          SANI_GATEWAY_TOKEN: gwB.gatewayToken,
          SANI_GATEWAY_PASSWORD: "",
        }),
