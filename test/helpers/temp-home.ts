...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("SANI_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.SANI_STATE_DIR = path.join(base, ".sani");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "sani-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".sani", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
