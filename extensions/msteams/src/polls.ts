...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["saniPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["sani", "pollId"]) ??
    readNestedString(value, ["sani", "poll", "id"]) ??
    readNestedString(value, ["data", "saniPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "sani", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          ...
          value: { saniPollId: pollId, pollId },
        },
      },
    ],
  };
