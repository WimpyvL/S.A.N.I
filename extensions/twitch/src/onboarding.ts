...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: SANI_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: SANIConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var SANI_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.SANI_TWITCH_ACCESS_TOKEN?.trim();
