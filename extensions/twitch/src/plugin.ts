/**
 * Twitch channel plugin for SANI.
 *
...
import { buildChannelConfigSchema } from "sani/plugin-sdk";
import type {
 ...
 * for SANI. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: SANIConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: SANIConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: SANIConfig): boolean => {
