...
const liveEnabled = HAS_OPENAI_KEY && process.env.SANI_LIVE_TEST === "1";
...
describe("memory plugin e2e", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sani-memory-test-"));
...
describeLive("memory plugin live tests", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sani-memory-live-"));
    dbPath = path.join(tmpDir, "lancedb");
  });
