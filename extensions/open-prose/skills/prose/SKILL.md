...
metadata: { "sani": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == SANI `sessions_spawn`
- **File I/O** == SANI `read`/`write`
- **Remote fetch** == SANI `web_fetch` (or `exec` with curl when POST is required)
