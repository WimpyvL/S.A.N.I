import type { PluginRuntime } from "sani/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.SANI_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "sani-nostr-"));
  process.env.SANI_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.SANI_STATE_DIR?.trim() || env.SANI_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".sani");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.SANI_STATE_DIR = previous;
    }
