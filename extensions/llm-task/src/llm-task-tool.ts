...
// NOTE: This extension is intended to be bundled with SANI.
// When running from source (tests/dev), SANI internals live under src/.
...
export function createLlmTaskTool(api: SANIPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via sani.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sani-llm-task-"));
