...
`sani.invoke --each`) without adding new SANI code per workflow.
...
This extension depends on SANI internal modules (the embedded agent runner).
It is intended to ship as a **bundled** SANI extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.sani/extensions` as a standalone plugin directory.
