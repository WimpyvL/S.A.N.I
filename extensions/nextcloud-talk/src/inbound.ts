...
export async function handleNextcloudTalkInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as SANIConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as SANIConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as SANIConfig);
