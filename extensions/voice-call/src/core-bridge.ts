...
function resolveSANIRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.SANI_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set SANI_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveSANIRoot(), "dist", "extensionAPI.js");
