...
SANI extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
sani channels login --channel zalouser
...
```bash
sani message send --channel zalouser --target <threadId> --message "Hello from SANI!"
```
...
```bash
sani channels login --channel zalouser              # Login via QR
sani channels login --channel zalouser --account work
sani channels status --probe
sani channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
sani directory self --channel zalouser
sani directory peers list --channel zalouser --query "name"
sani directory groups list --channel zalouser --query "work"
sani directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
sani message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
sani message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `sani channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
sani channels login --channel zalouser --account work
sani message send --channel zalouser --account work --target <id> --message "Hello"
