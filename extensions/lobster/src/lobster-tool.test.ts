...
import type { SANIPluginApi, SANIPluginToolContext } from "../../../src/plugins/types.js";
import { createLobsterTool } from "./lobster-tool.js";

async function writeFakeLobsterScript(scriptBody: string, prefix = "sani-lobster-plugin-") {
...
function fakeApi(overrides: Partial<SANIPluginApi> = {}): SANIPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<SANIPluginToolContext> = {}): SANIPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: SANIPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
