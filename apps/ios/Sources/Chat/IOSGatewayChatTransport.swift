...
struct IOSGatewayChatTransport: SANIChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> SANIChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(SANIChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> SANIChatHistoryPayload {
        ...
        return try JSONDecoder().decode(SANIChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [SANIChatAttachmentPayload]) async throws -> SANIChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [SANIChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(SANIChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(SANIGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: SANIGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: SANIChatEventPayload.self)
                        {
