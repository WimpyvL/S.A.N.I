...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = SANICanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = SANICanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = SANICanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = SANICanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = SANICanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__sani__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: SANILocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for sani:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case SANILocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case SANICanvasCommand.present.rawValue,
                 SANICanvasCommand.hide.rawValue,
                 SANICanvasCommand.navigate.rawValue,
                 SANICanvasCommand.evalJS.rawValue,
                 SANICanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case SANICanvasA2UICommand.reset.rawValue,
                 SANICanvasA2UICommand.push.rawValue,
                 SANICanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleCanvasA2UIInvoke(req)
            case SANICameraCommand.list.rawValue,
                 SANICameraCommand.snap.rawValue,
                 SANICameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case SANIScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: SANINodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: SANINodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(SANILocationGetParams.self, from: req.paramsJSON)) ??
            SANILocationGetParams()
        ...
        let payload = SANILocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SANICanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(SANICanvasPresentParams.self, from: req.paramsJSON)) ??
                SANICanvasPresentParams()
        ...
        case SANICanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case SANICanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(SANICanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case SANICanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(SANICanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case SANICanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(SANICanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case SANICanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              ...
              if (!host) return JSON.stringify({ ok: false, error: "missing saniA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SANICanvasA2UICommand.push.rawValue, SANICanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == SANICanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(SANICanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try SANICanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(SANICanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try SANICanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.saniA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case SANICameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case SANICameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(SANICameraSnapParams.self, from: req.paramsJSON)) ??
                SANICameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case SANICameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(SANICameraClipParams.self, from: req.paramsJSON)) ??
                SANICameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(SANIScreenRecordParams.self, from: req.paramsJSON)) ??
            SANIScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> SANILocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return SANILocationMode(rawValue: raw) ?? .off
    }
