...
import "@a2ui/lit/ui";
import { themeContext } from "@sani/a2ui-theme-context";
...
class SANIA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  static styles = css`
    :host {
      ...
      padding:
        var(--sani-a2ui-inset-top, 0px)
        var(--sani-a2ui-inset-right, 0px)
        var(--sani-a2ui-inset-bottom, 0px)
        var(--sani-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--sani-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--sani-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--sani-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--sani-a2ui-empty-top, var(--sani-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  connectedCallback() {
    ...
    globalThis.saniA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["sani:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    globalThis.__saniLastA2UIAction = userAction;

    const handler =
      globalThis.webkit?.messageHandlers?.saniCanvasA2UIAction ??
      globalThis.saniCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.saniCanvasA2UIAction) {
...
if (!customElements.get("sani-a2ui-host")) {
  customElements.define("sani-a2ui-host", SANIA2UIHost);
}
