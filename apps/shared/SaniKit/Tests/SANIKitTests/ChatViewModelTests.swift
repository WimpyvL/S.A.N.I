...
private final class TestChatTransport: @unchecked Sendable, SANIChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [SANIChatHistoryPayload]
    private let sessionsResponses: [SANIChatSessionsListResponse]

    private let stream: AsyncStream<SANIChatTransportEvent>
    private let continuation: AsyncStream<SANIChatTransportEvent>.Continuation

    init(
        historyResponses: [SANIChatHistoryPayload],
        sessionsResponses: [SANIChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<SANIChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> SANIChatHistoryPayload {
        ...
        return self.historyResponses.last ?? SANIChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [SANIChatAttachmentPayload]) async throws -> SANIChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return SANIChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> SANIChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? SANIChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: SANIChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = SANIChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = SANIChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { SANIChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = SANIChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = SANIChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = SANIChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { SANIChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
