...
enum SANIPlatformImageFactory {
    static func image(_ image: SANIPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
