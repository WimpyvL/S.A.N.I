...
struct ChatMessageBubble: View {
    let message: SANIChatMessage
    let style: SANIChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? SANIChatTheme.userText : SANIChatTheme.assistantText
    ...
    private var inlineAttachments: [SANIChatMessageContent] {
    ...
    private var toolCalls: [SANIChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [SANIChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? SANIChatTheme.userBubble
        }
        if self.style == .onboarding {
            return SANIChatTheme.onboardingAssistantBubble
        }
        return SANIChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return SANIChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: SANIChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? SANIChatTheme.userText : SANIChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: SANIChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(SANIChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [SANIChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: SANIChatTheme.assistantText)
            }
        }
    }
}
