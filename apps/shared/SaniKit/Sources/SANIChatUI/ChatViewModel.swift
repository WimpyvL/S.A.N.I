...
private let chatUILogger = Logger(subsystem: "ai.sani", category: "SANIChatUI")

@MainActor
@Observable
public final class SANIChatViewModel {
    public private(set) var messages: [SANIChatMessage] = []
    ...
    public var attachments: [SANIPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [SANIChatPendingToolCall] = []
    public private(set) var sessions: [SANIChatSessionEntry] = []
    private let transport: any SANIChatTransport
    ...
    private var pendingToolCallsById: [String: SANIChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any SANIChatTransport) {
    ...
    public var sessionChoices: [SANIChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [SANIChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [SANIChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: SANIPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [SANIChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: SANIChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [SANIChatMessage]) -> [SANIChatMessage] {
        var result: [SANIChatMessage] = []
    ...
    private static func dedupeKey(for message: SANIChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [SANIChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> SANIChatAttachmentPayload in
            SANIChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> SANIChatSessionEntry {
        SANIChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: SANIChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: SANIChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: SANIAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = SANIChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> SANIPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
