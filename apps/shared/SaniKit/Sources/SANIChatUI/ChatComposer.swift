...
@MainActor
struct SANIChatComposer: View {
    @Bindable var viewModel: SANIChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(SANIChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(SANIChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \SANIPendingAttachment.id)
                { (att: SANIPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            SANIPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(SANIChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(SANIChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(SANIChatTheme.subtleCard)
        .clipShape(Capsule())
    }
