...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func prefersSANIBinary() async throws {
        ...
        let saniPath = tmp.appendingPathComponent("node_modules/.bin/sani")
        try self.makeExec(at: saniPath)

        let cmd = CommandResolver.saniCommand(subcommand: "gateway", defaults: defaults, configRoot: [:])
        #expect(cmd.prefix(2).elementsEqual([saniPath.path, "gateway"]))
    }
    ...
    @Test func fallsBackToNodeAndScript() async throws {
        ...
        let scriptPath = tmp.appendingPathComponent("bin/sani.js")
        try self.makeExec(at: nodePath)
        try "#!/bin/sh\necho v22.0.0\n".write(to: nodePath, atomically: true, encoding: .utf8)
        try FileManager().setAttributes([.posixPermissions: 0o755], ofItemAtPath: nodePath.path)
        try self.makeExec(at: scriptPath)

        let cmd = CommandResolver.saniCommand(
            subcommand: "rpc",
            defaults: defaults,
            configRoot: [:],
    ...
    @Test func fallsBackToPnpm() async throws {
        ...
        let cmd = CommandResolver.saniCommand(subcommand: "rpc", defaults: defaults, configRoot: [:])

        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "sani", "rpc"]))
    }
    ...
    @Test func pnpmKeepsExtraArgsAfterSubcommand() async throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "sani", "health", "--json"]))
    ...
    @Test func buildsSSHCommandForRemoteMode() async throws {
        ...
        defaults.set("sani@example.com:2222", forKey: remoteTargetKey)
        defaults.set("/tmp/id_ed25519", forKey: remoteIdentityKey)
        defaults.set("/srv/sani", forKey: remoteProjectRootKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "sani@example.com")
        } else {
            #expect(Bool(false))
        }
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/sani'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("sani"))
    ...
    @Test func configRootLocalOverridesRemoteDefaults() async throws {
        ...
        #expect(cmd.first == saniPath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }
