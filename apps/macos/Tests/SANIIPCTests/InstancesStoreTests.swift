...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: SANIProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: SANIProtocol.AnyCodable] = [
            "presence": .init([SANIProtocol.AnyCodable(entry)]),
        ]
        let payload = SANIProtocol.AnyCodable(payloadMap)
