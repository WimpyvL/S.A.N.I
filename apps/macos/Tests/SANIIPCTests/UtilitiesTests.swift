...
@Suite(.serialized) struct UtilitiesTests {
    ...
    @Test func gatewayEntrypointPrefersDistOverBin() throws {
        ...
        let bin = tmp.appendingPathComponent("bin/sani.js")
    ...
    @Test func logLocatorPicksNewestLogFile() throws {
        let fm = FileManager()
        let dir = URL(fileURLWithPath: "/tmp/sani", isDirectory: true)
        try? fm.createDirectory(at: dir, withIntermediateDirectories: true)

        let older = dir.appendingPathComponent("sani-old-\(UUID().uuidString).log")
        let newer = dir.appendingPathComponent("sani-new-\(UUID().uuidString).log")
