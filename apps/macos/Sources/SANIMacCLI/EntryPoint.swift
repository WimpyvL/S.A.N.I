...
struct SANIMacCLI {
    static func main() async {
        ...
        default:
            fputs("sani-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      sani-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      sani-mac discover [--timeout <ms>] [--json] [--include-local]
      sani-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      sani-mac connect
      sani-mac connect --url ws://127.0.0.1:18789 --json
      sani-mac discover --timeout 3000 --json
      sani-mac wizard --mode local
    """)
}
