...
enum WideAreaGatewayDiscovery {
        ...
        context: DiscoveryContext = .live) -> [WideAreaGatewayBeacon]
    {
        ...
        let probeName = "_sani-gw._tcp.\(domainTrimmed)"
        ...
        for raw in ptrLines {
            ...
            let suffix = "._sani-gw._tcp.\(domainTrimmed)"
    ...
    {
        guard let domain = SANIBonjour.wideAreaGatewayServiceDomain else { return nil }
        let domainTrimmed = domain.trimmingCharacters(in: CharacterSet(charactersIn: "."))
        let probeName = "_sani-gw._tcp.\(domainTrimmed)"
