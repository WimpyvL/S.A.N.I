...
enum SANILogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = SANIOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = SANIFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        SANILogging.bootstrapIfNeeded()
        let label = SANILogging.makeLabel(subsystem: subsystem, category: category)
...
struct SANIOSLogHandler: LogHandler {
...
struct SANIFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = SANILogging.parseLabel(self.label)
