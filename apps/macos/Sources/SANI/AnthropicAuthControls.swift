...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: SANIOAuthStore.AnthropicOAuthStatus = SANIOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: SANIOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = SANIOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = SANIOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try SANIOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. SANI can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: SANIOAuthStore.AnthropicOAuthStatus,
