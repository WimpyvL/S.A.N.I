...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: SANIProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.sani", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: SANIKit.AnyCodable]()) {
                $0[$1.key] = SANIKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: SANIProtocol.AnyCodable?) -> [String: SANIProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: SANIProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: SANIKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: SANIProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("sani.control.heartbeat")
    static let controlAgentEvent = Notification.Name("sani.control.agent")
}
