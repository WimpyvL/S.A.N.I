...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.sani/sani.json. Prefer SANI_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.sani/sani.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.sani/sani.json. Restart the gateway to apply."
        }
