...
final class WorkActivityStore {
    ...
    func handleTool(
        ...
        name: String?,
        meta: String?,
        args: [String: SANIProtocol.AnyCodable]?)
    {
    ...
    private static func buildLabel(
        name: String?,
        meta: String?,
        args: [String: SANIProtocol.AnyCodable]?) -> String
    {
    ...
    private static func wrapToolArgs(_ args: [String: SANIProtocol.AnyCodable]?) -> SANIKit.AnyCodable? {
        guard let args else { return nil }
        let converted: [String: Any] = args.mapValues { self.unwrapJSONValue($0.value) }
        return SANIKit.AnyCodable(converted)
    }

    private static func unwrapJSONValue(_ value: Any) -> Any {
        if let dict = value as? [String: SANIProtocol.AnyCodable] {
            return dict.mapValues { self.unwrapJSONValue($0.value) }
        }
        if let array = value as? [SANIProtocol.AnyCodable] {
