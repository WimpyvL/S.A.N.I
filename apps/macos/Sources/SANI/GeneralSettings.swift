        ...
        ScrollView(.vertical) {
            VStack(alignment: .leading, spacing: 18) {
                VStack(alignment: .leading, spacing: 12) {
                    SettingsToggleRow(
                        title: "SANI active",
                        subtitle: "Pause to stop the SANI gateway; no messages will be processed.",
                    ...
                    SettingsToggleRow(
                        ...
                        subtitle: "Automatically start SANI after you sign in.",
                        binding: self.$state.launchAtLogin)

                    SettingsToggleRow(
                        title: "Show Dock icon",
                        subtitle: "Keep SANI visible in the Dock instead of menu-bar-only mode.",
                ...
                HStack {
                    Spacer()
                    Button("Quit SANI") { NSApp.terminate(nil) }
                ...
                DisclosureGroup(isExpanded: self.$showRemoteAdvanced) {
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        LabeledContent("CLI path") {
                            TextField("/Applications/SANI.app/.../sani", text: self.$state.remoteCliPath)
                                .textFieldStyle(.roundedBorder)
                                .frame(width: 280)
                        }
                    }
...
extension GeneralSettings {
    ...
    private func revealLogs() {
        ...
        Looked for sani logs in /tmp/sani/.
    ...
    private func applyDiscoveredGateway(_ gateway: GatewayDiscoveryModel.DiscoveredGateway) {
        ...
        if self.state.remoteTransport == .direct {
            ...
            SANIConfigFile.setRemoteGatewayUrl(host: host, port: gateway.gatewayPort)
        }
    }
}
...
extension GeneralSettings {
    static func exerciseForTesting() {
        ...
        state.remoteProjectRoot = "/tmp/sani"
        state.remoteCliPath = "/tmp/sani"
