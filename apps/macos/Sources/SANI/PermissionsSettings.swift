...
struct PermissionsSettings: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Text("Allow these so SANI can notify and capture when needed.")
                .padding(.top, 4)
...
private struct LocationAccessSettings: View {
    @AppStorage(locationModeKey) private var locationModeRaw: String = SANILocationMode.off.rawValue
    @AppStorage(locationPreciseKey) private var locationPreciseEnabled: Bool = true
    @State private var lastLocationModeRaw: String = SANILocationMode.off.rawValue
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Picker("", selection: self.$locationModeRaw) {
                Text("Off").tag(SANILocationMode.off.rawValue)
                Text("While Using").tag(SANILocationMode.whileUsing.rawValue)
                Text("Always").tag(SANILocationMode.always.rawValue)
            }
            .labelsHidden()
            .pickerStyle(.menu)
        ...
        .onChange(of: self.locationModeRaw) { _, newValue in
            ...
            guard let mode = SANILocationMode(rawValue: newValue) else { return }
            Task {
    ...
    private var locationMode: SANILocationMode {
        SANILocationMode(rawValue: self.locationModeRaw) ?? .off
    }

    private func requestLocationAuthorization(mode: SANILocationMode) async -> Bool {
