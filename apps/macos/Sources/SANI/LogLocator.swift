...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["SANI_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/sani")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("sani-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("sani-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/sani/ (rolling or stdout), or nil if none exist.
