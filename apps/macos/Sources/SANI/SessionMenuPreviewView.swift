...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.sani", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> SANISessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: SANISessionPreviewEntry,
    ...
    private static func cache(payload: SANISessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: SANIChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [SANIKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [SANIKit.AnyCodable]) -> [SANIChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(SANIChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: SANIChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: SANIChatMessage) -> Bool {
    ...
    private static func toolNames(for message: SANIChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: SANIChatMessage) -> String? {
