...
final class HealthStore {
    ...
    private static let logger = Logger(subsystem: "ai.sani", category: "health")
    ...
    var summaryLine: String {
        ...
        if link.summary.linked != true {
            if let fallback = self.resolveFallbackChannel(snap, excluding: link.id) {
                ...
                return "\(fallbackLabel) \(fallbackState) · Not linked — run sani login"
            }
            return "Not linked — run sani login"
        }
    ...
    var detailLine: String? {
        if let error = self.lastError, !error.isEmpty {
            ...
            if lower.contains("connection refused") {
                ...
                return "The gateway control port (\(host)) isn’t listening — restart SANI to bring it back."
            }
