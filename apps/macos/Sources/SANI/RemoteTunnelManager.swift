...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.sani", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous SANI run already has an SSH listener on the expected port (common after restarts),
