...
private let gatewayConnectionLogger = Logger(subsystem: "ai.sani", category: "gateway.connection")
...
actor GatewayConnection {
    ...
    private func sessionDefaultString(_ defaults: [String: SANIProtocol.AnyCodable]?, key: String) -> String {
...
extension GatewayConnection {
    ...
    func healthOK(timeoutMs: Int = 8000) async throws -> Bool {
        ...
        return (try? self.decoder.decode(SANIGatewayHealthOK.self, from: data))?.ok ?? true
    }
    ...
    func sessionsPreview(
        ...
        limit: Int? = nil,
        maxChars: Int? = nil,
        timeoutMs: Int? = nil) async throws -> SANISessionsPreviewPayload
    {
        let resolvedKeys = keys
            .map { self.canonicalizeSessionKey($0) }
            .filter { !$0.isEmpty }
        if resolvedKeys.isEmpty {
            return SANISessionsPreviewPayload(ts: 0, previews: [])
        }
    ...
    func chatHistory(
        sessionKey: String,
        limit: Int? = nil,
        timeoutMs: Int? = nil) async throws -> SANIChatHistoryPayload
    {
    ...
    func chatSend(
        ...
        attachments: [SANIChatAttachmentPayload],
        timeoutMs: Int = 30000) async throws -> SANIChatSendResponse
    {
        let resolvedKey = self.canonicalizeSessionKey(sessionKey)
        var params: [String: AnyCodable] = [
