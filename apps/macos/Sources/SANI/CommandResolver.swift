...
enum CommandResolver {
    private static let projectRootDefaultsKey = "sani.gatewayProjectRootPath"
    private static let helperName = "sani"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let saniEntry = root.appendingPathComponent("sani.mjs").path
        if FileManager().isReadableFile(atPath: saniEntry) { return saniEntry }
        let binEntry = root.appendingPathComponent("bin/sani.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/sani")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let saniPaths = self.saniManagedPaths(home: home)
        if !saniPaths.isEmpty {
            extras.insert(contentsOf: saniPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + saniPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func saniManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".sani"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func saniExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectSANIExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("sani.mjs").path,
            root.appendingPathComponent("bin/sani.js").path,
        ]
    ...
    static func hasAnySANIInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.saniExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let saniPath = self.projectSANIExecutable(projectRoot: root) {
                return [saniPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "sani", subcommand] + extraArgs
            }
            if let saniPath = self.saniExecutable(searchPaths: searchPaths) {
                return [saniPath, subcommand] + extraArgs
            }

            let missingEntry = """
            sani entrypoint missing (looked for dist/index.js or sani.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real sani CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/sani"
        ...
        \(cliSection)
        \(projectSection)
        if command -v sani >/dev/null 2>&1; then
          CLI="$(command -v sani)"
          sani \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/sani.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/sani.mjs"
            node "$PRJ/sani.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/sani.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/sani.js"
            node "$PRJ/bin/sani.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent sani"
          pnpm --silent sani \(quotedArgs);
        else
          echo "sani CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? SANIConfigFile.loadDict()
