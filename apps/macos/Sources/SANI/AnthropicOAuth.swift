...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (SANI token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: SANIOAuthStore.AnthropicOAuthStatus = SANIOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.sani", category: "anthropic-oauth")
...
enum SANIOAuthStore {
    ...
    private static let saniOAuthDirEnv = "SANI_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "SANI OAuth token file not found"
            case .unreadableFile: "SANI OAuth token file not readable"
            case .invalidJSON: "SANI OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in SANI OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "SANI OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.saniOAuthDirEnv]?
        ...
        let preferred = home.appendingPathComponent(".sani", isDirectory: true)
