...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.sani", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: SANIPeekabooBridgeServices?
    private static var saniSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("SANI", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = SANIPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.saniSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.saniSocketPath, privacy: .public)")
    }
...
@MainActor
private final class SANIPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.sani.peekaboo")
