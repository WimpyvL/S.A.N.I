...
private let webChatSwiftLogger = Logger(subsystem: "ai.sani", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: SANIChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> SANIChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(SANIChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> SANIChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                SANIGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<SANIChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any SANIChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = SANIChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: SANIChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<SANIChatView>) -> NSViewController
    {
