...
import ai.sani.android.ScreenCaptureRequester
...
class ScreenRecordManager(private val context: Context) {
  ...
  @Volatile private var permissionRequester: ai.sani.android.PermissionRequester? = null

  fun attachScreenCaptureRequester(requester: ScreenCaptureRequester) {
    screenCaptureRequester = requester
  }

  fun attachPermissionRequester(requester: ai.sani.android.PermissionRequester) {
  ...
  suspend fun record(paramsJson: String?): Payload =
    withContext(Dispatchers.Default) {
      ...
      val file = File.createTempFile("sani-screen-", ".mp4")
