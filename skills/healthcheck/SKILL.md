---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for SANI deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, SANI cron scheduling for periodic checks, or version status checks on a machine running SANI (laptop, workstation, Pi, VPS).
---
...
Assess and harden the host running SANI, then align it to a user-defined risk tolerance without breaking access. Use SANI security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim SANI changes the host firewall, SSH, or OS updates; it does not.
...
5. SANI gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if SANI can access sensitive data.
...
### 2) Run SANI security audits (read-only)

As part of the default read-only checks, run `sani security audit --deep`. Only offer alternatives if the user requests them:

1. `sani security audit` (faster, non-probing)
2. `sani security audit --json` (structured output)

Offer to apply SANI safe defaults (numbered):

1. `sani security audit --fix`

Be explicit that `--fix` only tightens SANI defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check SANI version/update status (read-only)

As part of the default read-only checks, run `sani update status`.
...
- Credential hygiene notes (location of SANI creds, prefer disk encryption)
...
After SANI install or first hardening pass, run at least one baseline audit and version check:

- `sani security audit`
- `sani security audit --deep`
- `sani update status`

Ongoing monitoring is recommended. Use the SANI cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `sani cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `sani update status`
...
Before creating, `sani cron list` and match on exact `name`. If found, `sani cron edit <id> ...`.
If not found, `sani cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `sani update status` (preferred for source checkouts and channels)
2. `npm view sani version` (published npm version)

## SANI command accuracy

Use only supported commands and flags:

- `sani security audit [--deep] [--fix] [--json]`
- `sani status` / `sani status --deep`
- `sani health --json`
- `sani update status`
- `sani cron add|list|runs|run`

Do not invent CLI flags or imply SANI enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by SANI compaction:
