...
This document describes how the SANI ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`sani acp` exposes an ACP agent over stdio and forwards prompts to a running
SANI Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a SANI Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `sani acp` over stdio.

Example config:

```bash
sani config set gateway.remote.url wss://gateway-host:18789
sani config set gateway.remote.token <token>
```

Example run:

```bash
sani acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
sani acp --session agent:main:main
sani acp --session agent:design:main
sani acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “SANI ACP” to start a thread.

## Execution Model

- ACP client spawns `sani acp` and speaks ACP messages over stdio.
...
```bash
sani acp --session agent:main:main
sani acp --session-label "support inbox"
sani acp --reset-session
```
...
`sani acp` resolves the Gateway URL and auth from CLI flags or config:
