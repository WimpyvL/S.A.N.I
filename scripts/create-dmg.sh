...
#
# Env:
#   DMG_VOLUME_NAME        default: CFBundleName (or "SANI")
...
APP_NAME=$(/usr/libexec/PlistBuddy -c "Print CFBundleName" "$APP_PATH/Contents/Info.plist" 2>/dev/null || echo "SANI")
...
DMG_TEMP="$(mktemp -d /tmp/sani-dmg.XXXXXX)"
...
if [[ "${SKIP_DMG_STYLE:-0}" != "1" ]]; then
  ...
  ICON_SRC="$ROOT_DIR/apps/macos/Sources/SANI/Resources/SANI.icns"
  if [[ -f "$ICON_SRC" ]]; then
...
hdiutil resize -limits "$DMG_RW_PATH" >/tmp/sani-dmg-limits.txt 2>/dev/null || true
MIN_SECTORS="$(tail -n 1 /tmp/sani-dmg-limits.txt 2>/dev/null | awk '{print $1}')"
rm -f /tmp/sani-dmg-limits.txt
