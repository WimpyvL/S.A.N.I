...
async function main() {
  ...
  const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "sani-zai-fallback-"));
  const stateDir = path.join(baseDir, "state");
  const configPath = path.join(baseDir, "sani.json");
  ...
  const sessionId =
    process.env.SANI_ZAI_FALLBACK_SESSION_ID ??
  ...
  const run1 = await runCommand(
    "run1",
    ["sani", "agent", "--local", "--session-id", sessionId, "--message", toolPrompt],
    envValidAnthropic,
  );
  if (run1.code !== 0) {
  ...
  const run2 = await runCommand(
    "run2",
    ["sani", "agent", "--local", "--session-id", sessionId, "--message", followupPrompt],
    envInvalidAnthropic,
  );
