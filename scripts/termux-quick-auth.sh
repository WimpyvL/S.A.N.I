...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/sani/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "SANI Re-Auth" -c "After getting key, run: ssh $SERVER '~/sani/scripts/mobile-reauth.sh'" --id sani-auth
        ;;
    *)
