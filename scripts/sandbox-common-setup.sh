...
BASE_IMAGE="${BASE_IMAGE:-sani-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-sani-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=sani.sandbox=1)
NOTE
