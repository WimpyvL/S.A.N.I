...
# Checks both Claude Code and SANI auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
SANI_AUTH="$HOME/.sani/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    sani models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    sani_status=$(check_sani_auth 2>/dev/null || true)

    claude_expires=0
    sani_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        sani_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        sani_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$SANI_AUTH" 2>/dev/null || echo "0")
    fi
    ...
    jq -n \
        ...
        '{
            ...
            sani: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$sani_status" == EXPIRED* ]] || [[ "$sani_status" == MISSING* ]]; then
        echo "SANI_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$sani_status" == EXPIRING* ]]; then
        echo "SANI_EXPIRING"
        exit 2
    else
        echo "OK"
...
echo ""
echo "SANI Auth (~/.sani/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$SANI_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$SANI_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'sani doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active sani >/dev/null 2>&1; then
    echo -e "SANI service: ${GREEN}running${NC}"
else
    echo -e "SANI service: ${RED}NOT running${NC}"
fi
