...
TRASH
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/sani-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/sani-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/sani-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$HOME/.sani/workspace"
    config_path="$HOME/.sani/sani.json"
    sessions_dir="$HOME/.sani/agents/main/sessions"
