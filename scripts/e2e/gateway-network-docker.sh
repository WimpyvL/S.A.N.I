...
IMAGE_NAME="sani-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="sani-net-e2e-$$"
GW_NAME="sani-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "SANI_GATEWAY_TOKEN=$TOKEN" \
	  -e "SANI_SKIP_CHANNELS=1" \
	  -e "SANI_SKIP_GMAIL_WATCHER=1" \
	  -e "SANI_SKIP_CRON=1" \
	  -e "SANI_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
