...
IMAGE_NAME="sani-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/sani-bin:$PATH"
  mkdir -p /tmp/sani-bin

  cat > /tmp/sani-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/sani-bin/systemctl

  cat > /tmp/sani-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
LOGINCTL
  ...
  npm_bin="/tmp/npm-prefix/bin/sani"
  npm_entry="/tmp/npm-prefix/lib/node_modules/sani/dist/index.js"
  git_entry="/app/dist/index.js"
  git_cli="/app/sani.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/sani-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/sani-gateway.service"
    if [ ! -f "$unit_path" ]; then
