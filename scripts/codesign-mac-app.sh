...
APP_BUNDLE="${1:-dist/SANI.app}"
IDENTITY="${SIGN_IDENTITY:-}"
TIMESTAMP_MODE="${CODESIGN_TIMESTAMP:-auto}"
DISABLE_LIBRARY_VALIDATION="${DISABLE_LIBRARY_VALIDATION:-0}"
SKIP_TEAM_ID_CHECK="${SKIP_TEAM_ID_CHECK:-0}"
ENT_TMP_BASE=$(mktemp -t sani-entitlements-base.XXXXXX)
ENT_TMP_APP_BASE=$(mktemp -t sani-entitlements-app-base.XXXXXX)
ENT_TMP_RUNTIME=$(mktemp -t sani-entitlements-runtime.XXXXXX)
...
# Sign main binary
if [ -f "$APP_BUNDLE/Contents/MacOS/SANI" ]; then
  echo "Signing main binary"; sign_item "$APP_BUNDLE/Contents/MacOS/SANI" "$APP_ENTITLEMENTS"
fi
