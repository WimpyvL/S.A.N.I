...
#
# Suggested cron: */30 * * * * /home/admin/sani/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send SANI notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., sani-alerts)
...
STATE_FILE="$HOME/.sani/auth-monitor-state"
...
send_notification() {
    ...
    # Send via SANI if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use sani
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via SANI to $NOTIFY_PHONE..."
            sani send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! SANI is down. Run: ssh l36 '~/sani/scripts/mobile-reauth.sh'" "urgent"
    exit 1
