...
function updateConfig(config: SANIConfig, enabled: boolean): SANIConfig {
  const value = enabled ? "true" : "false";
  const next: SANIConfig = { ...config };
  const env = { ...(next.env ?? {}) } as SANIConfig["env"] & Record<string, unknown>;
