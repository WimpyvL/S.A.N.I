...
# Build and bundle SANI into a minimal .app we can open.
# Outputs to dist/SANI.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/SANI.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="SANI"
BUNDLE_ID="${BUNDLE_ID:-ai.sani.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/sani/sani/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/SANI/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :SANIBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :SANIGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/SANI"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/SANI"
fi
chmod +x "$APP_ROOT/Contents/MacOS/SANI"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/SANI" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/SANI/Resources/SANI.icns" "$APP_ROOT/Contents/Resources/SANI.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/SANI/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying SANIKit resources"
SANIKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/SANIKit_SANIKit.bundle"
if [ -d "$SANIKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/SANIKit_SANIKit.bundle"
  cp -R "$SANIKIT_BUNDLE" "$APP_ROOT/Contents/Resources/SANIKit_SANIKit.bundle"
else
  echo "WARN: SANIKit resource bundle not found at $SANIKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running SANI"
killall -q SANI 2>/dev/null || true
