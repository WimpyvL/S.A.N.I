...
ZIP=${1:?"Usage: $0 SANI-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/sani/sani/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^SANI-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/sani/sani/releases/download/v${VERSION}/"}
