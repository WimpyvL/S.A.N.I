...
import { loadSANIPlugins } from "../src/plugins/loader.js";
import { wrapInboundMessage } from "../src/security/inbound-message.js";

async function main() {
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "sani-sani-"));
  ...
  loadSANIPlugins({ config, workspaceDir, cache: false });
