 ...
 * `sani update`. Use it to verify the completion installation flow
...
\`sani update\`, \`sani doctor\`, and \`sani onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# SANI Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`sani completion --install\` later to enable.`));
    return;
  }
