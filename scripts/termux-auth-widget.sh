...
# Check auth status
termux-toast "Checking SANI auth..."

STATUS=$(ssh "$SERVER" '$HOME/sani/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/sani/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|SANI_EXPIRING|CLAWDBOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/sani/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|SANI_EXPIRED|CLAWDBOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: SANI won't work until re-auth"
                ;;
        esac
        ;;
