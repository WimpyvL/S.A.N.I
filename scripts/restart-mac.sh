#!/usr/bin/env bash
# Reset SANI like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${SANI_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="SANI.app/Contents/MacOS/SANI"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/SANI"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/SANI"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/SANI"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.sani.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/sani-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${SANI_RESTART_LOG:-/tmp/sani-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${SANI_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.sani/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  SANI_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node sani.mjs daemon install --force --runtime node"
      log "  node sani.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.sani/disable-launchagent"
      log ""
...
kill_all_sani() {
  for _ in {1..10}; do
    ...
    if ! pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1 \
       ...
       && ! pgrep -x "SANI" >/dev/null 2>&1; then
      return 0
    fi
    sleep 0.3
  done
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.sani.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing SANI instances"
kill_all_sani
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product SANI"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/SANI.app" ]]; then
    APP_BUNDLE="/Applications/SANI.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/SANI.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/SANI.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/SANI.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set SANI_APP_BUNDLE to your installed SANI.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node sani.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node sani.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".sani", "sani.json"), "utf8");
