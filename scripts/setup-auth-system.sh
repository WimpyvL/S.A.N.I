#!/bin/bash
# Setup SANI Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'sani-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  SANI message: Send warning via SANI itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/sani-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/sani-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/sani-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now sani-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status sani-auth-monitor.timer"
echo ""
echo "Quick commands:"
