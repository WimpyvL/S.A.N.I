...
docker run --rm -t \
  ...
  -e SANI_LIVE_TEST=1 \
  -e SANI_LIVE_MODELS="${SANI_LIVE_MODELS:-${CLAWDBOT_LIVE_MODELS:-all}}" \
  -e SANI_LIVE_PROVIDERS="${SANI_LIVE_PROVIDERS:-${CLAWDBOT_LIVE_PROVIDERS:-}}" \
  -e SANI_LIVE_MODEL_TIMEOUT_MS="${SANI_LIVE_MODEL_TIMEOUT_MS:-${CLAWDBOT_LIVE_MODEL_TIMEOUT_MS:-}}" \
  -e SANI_LIVE_REQUIRE_PROFILE_KEYS="${SANI_LIVE_REQUIRE_PROFILE_KEYS:-${CLAWDBOT_LIVE_REQUIRE_PROFILE_KEYS:-}}" \
