...
INSTALL_URL="${SANI_INSTALL_URL:-https://sani.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${SANI_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${SANI_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="sani"
PACKAGE_NAME="${SANI_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (SANI_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${SANI_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${SANI_INSTALL_LATEST_OUT:-}"
fi
